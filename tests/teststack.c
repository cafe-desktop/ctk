...
{
  ctk_stack_set_visible_child (GTK_STACK (stack), GTK_WIDGET (data));
}

static void
set_visible_child_name (GtkWidget *button, gpointer data)
{
  ctk_stack_set_visible_child_name (GTK_STACK (stack), (const char *)data);
}

static void
toggle_hhomogeneous (GtkWidget *button, gpointer data)
{
  gboolean active = ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button));
  ctk_stack_set_hhomogeneous (GTK_STACK (stack), active);
}
...
{
  gboolean active = ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button));
  ctk_stack_set_vhomogeneous (GTK_STACK (stack), active);
}
...
{
  gboolean active = ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button));
  ctk_container_child_set (GTK_CONTAINER (stack), w1,
...
{
  int id = ctk_combo_box_get_active (GTK_COMBO_BOX (combo));
  ctk_stack_set_transition_type (GTK_STACK (stack), id);
}
  ...
  for (i = 1; i < G_N_ELEMENTS (seq); i++)
    {
      if (g_strcmp0 (vis, seq[i]) == 0)
        {
          ctk_stack_set_visible_child_full (stack, seq[i - 1], GTK_STACK_TRANSITION_TYPE_SLIDE_RIGHT);
          break;
        }
    }
}
  ...
  for (i = 0; i < G_N_ELEMENTS (seq) - 1; i++)
    {
      if (g_strcmp0 (vis, seq[i]) == 0)
        {
          ctk_stack_set_visible_child_full (stack, seq[i + 1], GTK_STACK_TRANSITION_TYPE_SLIDE_LEFT);
          break;
        }
    }
}
      ...
      gchar ** argv)
{
  ...
  ctk_box_pack_start (GTK_BOX (box), switcher, FALSE, FALSE, 0);

  stack = ctk_stack_new ();

  /* Make transitions longer so we can see that they work */
  ctk_stack_set_transition_duration (GTK_STACK (stack), 1500);

  ctk_widget_set_halign (stack, GTK_ALIGN_START);
  ...
  layout = ctk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
  ctk_box_pack_start (GTK_BOX (layout), sidebar, FALSE, FALSE, 0);
  ctk_box_pack_start (GTK_BOX (layout), stack, TRUE, TRUE, 0);

  ctk_container_add (GTK_CONTAINER (box), layout);

  ctk_stack_switcher_set_stack (GTK_STACK_SWITCHER (switcher), GTK_STACK (stack));

  w1 = ctk_text_view_new ();
  ctk_text_buffer_set_text (ctk_text_view_get_buffer (GTK_TEXT_VIEW (w1)),
			    "This is a\nTest\nBalh!", -1);

  ctk_container_add_with_properties (GTK_CONTAINER (stack), w1,
				     "name", "1",
				     "title", "1",
				     NULL);

  w2 = ctk_button_new_with_label ("Gazoooooooooooooooonk");
  ctk_container_add (GTK_CONTAINER (stack), w2);
  ctk_container_child_set (GTK_CONTAINER (stack), w2,
			   "name", "2",
			   "title", "2",
  ...
  ctk_stack_add_titled (GTK_STACK (stack), w3, "3", "3");
  ...
  for (i = 0; i < class->n_values; i++)
    ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (combo), class->values[i].value_nick);
