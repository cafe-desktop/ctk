...
kinetic_scrolling (void)
{
  ...
  for (i = 0; i < 80; i++)
    {
      ...
      ctk_grid_attach (GTK_GRID (button_grid), button, 0, i, 1, 1);
  ...
  ctk_scrolled_window_set_kinetic_scrolling (GTK_SCROLLED_WINDOW (swindow), TRUE);
  ctk_scrolled_window_set_capture_button_press (GTK_SCROLLED_WINDOW (swindow), TRUE);
  ctk_container_add (GTK_CONTAINER (swindow), button_grid);
  ctk_widget_show (button_grid);

  ctk_grid_attach (GTK_GRID (grid), swindow, 0, 1, 1, 1);
  ctk_widget_show (swindow);

  treeview = ctk_tree_view_new ();
  ctk_tree_view_enable_model_drag_source (GTK_TREE_VIEW (treeview),
                                          GDK_BUTTON1_MASK,
                                          row_targets,
                                          G_N_ELEMENTS (row_targets),
                                          GDK_ACTION_MOVE | GDK_ACTION_COPY);
  ctk_tree_view_enable_model_drag_dest (GTK_TREE_VIEW (treeview),
                                        row_targets,
                                        G_N_ELEMENTS (row_targets),
                                        GDK_ACTION_MOVE | GDK_ACTION_COPY);
  ...
  ctk_scrolled_window_set_kinetic_scrolling (GTK_SCROLLED_WINDOW (swindow), TRUE);
  ctk_scrolled_window_set_capture_button_press (GTK_SCROLLED_WINDOW (swindow), TRUE);
  ctk_container_add (GTK_CONTAINER (swindow), treeview);
  ctk_widget_show (treeview);

  ctk_grid_attach (GTK_GRID (grid), swindow, 1, 1, 1, 1);
  ...
  ctk_grid_attach (GTK_GRID (grid), swindow, 2, 1, 1, 1);
  ctk_widget_set_hexpand (swindow, TRUE);
  ctk_widget_set_vexpand (swindow, TRUE);
  ctk_widget_show (swindow);
