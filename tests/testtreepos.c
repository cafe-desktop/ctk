              ...
              GtkTreePath **path)
{
  ...
  if (ctk_tree_view_get_path_at_pos (tv, x, y, path, &col, &cell_x, &cell_y))
    {
      cells = ctk_cell_layout_get_cells (GTK_CELL_LAYOUT (col));
      ...
      ctk_widget_style_get (GTK_WIDGET (tv), "expander-size", &expander_size, NULL);
      expander_size += 4;
...
int main (int argc, char *argv[])
{
  ...
  ctk_tree_view_append_column (GTK_TREE_VIEW (tv), col);
