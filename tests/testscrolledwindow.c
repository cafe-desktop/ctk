			   ...
			   GtkViewport *viewport)
{
  ...
  ctk_scrollable_set_hscroll_policy (GTK_SCROLLABLE (viewport), policy);
}
			 ...
			 GtkViewport *viewport)
{
  ...
  ctk_scrollable_set_vscroll_policy (GTK_SCROLLABLE (viewport), policy);
}
         ...
         GtkListBox *listbox)
{
  ...
  ctk_container_add (GTK_CONTAINER (row), ctk_label_new ("test"));
            ...
            GtkListBox *listbox)
{
  ...
  children = ctk_container_get_children (GTK_CONTAINER (listbox));
  last = g_list_last (children);

  if (last)
    ctk_container_remove (GTK_CONTAINER (listbox), last->data);
...
scrollable_policy (void)
{
  ...
  ctk_container_add (GTK_CONTAINER (frame), swindow);
  ...
  ctk_label_set_max_width_chars  (GTK_LABEL (label), 100);

  ctk_widget_show (label);
  ctk_widget_show (viewport);
  ctk_container_add (GTK_CONTAINER (viewport), label);
  ctk_container_add (GTK_CONTAINER (swindow), viewport);

  /* Add controls here */
  expander = ctk_expander_new ("Controls");
  ctk_expander_set_expanded (GTK_EXPANDER (expander), TRUE);
  cntl = ctk_box_new (GTK_ORIENTATION_VERTICAL, 2);
  ctk_widget_show (cntl);
  ctk_widget_show (expander);
  ctk_container_add (GTK_CONTAINER (expander), cntl);
  ...
  ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (widget), "Minimum");
  ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (widget), "Natural");
  ctk_combo_box_set_active (GTK_COMBO_BOX (widget), 0);
  ctk_widget_show (widget);

  ctk_box_pack_start (GTK_BOX (hbox), widget, TRUE, TRUE, 0);
  ctk_box_pack_start (GTK_BOX (cntl), hbox, FALSE, FALSE, 0);
  ...
  ctk_widget_show (widget);
  ctk_box_pack_start (GTK_BOX (cntl), widget, TRUE, TRUE, 0);
  ...
  ctk_menu_button_set_popover (GTK_MENU_BUTTON (widget), popover);
  ctk_container_add (GTK_CONTAINER (widget), ctk_label_new ("Popover"));
  ctk_box_pack_start (GTK_BOX (cntl), widget, FALSE, FALSE, 0);
  ctk_widget_show_all (widget);

  vbox = ctk_box_new (GTK_ORIENTATION_VERTICAL, 6);
  ctk_container_add (GTK_CONTAINER (popover), vbox);
  ctk_widget_show (vbox);
  ...
  ctk_box_pack_end (GTK_BOX (vbox), swindow, FALSE, FALSE, 0);
  ctk_widget_show (swindow);
  ctk_widget_show (hbox);

  /* Listbox */
  listbox = ctk_list_box_new ();
  ctk_container_add (GTK_CONTAINER (swindow), listbox);
  ...
  g_object_bind_property (ctk_spin_button_get_adjustment (GTK_SPIN_BUTTON (widget)),
                          "value",
                          swindow,
                          "min-content-width",
