...
combo_changed_cb (GtkComboBoxText *combo,
		  gpointer user_data)
{
  gint active = ctk_combo_box_get_active (GTK_COMBO_BOX (combo));

  ctk_button_box_set_layout (GTK_BUTTON_BOX (bbox),
...
{
  g_object_ref (widget);
  ctk_container_remove (GTK_CONTAINER (old_parent), widget);
			...
			GtkWidget **buttons)
{
  ...
  style = ctk_button_box_get_layout (GTK_BUTTON_BOX (old_parent));
  ctk_button_box_set_layout (GTK_BUTTON_BOX (new_parent), style);
	   ...
	   GtkWidget *button)
{
  ...
  ctk_button_box_set_child_secondary (GTK_BUTTON_BOX (bbox),
				      button, active);
}
      ...
      char **argv)
{
  ...
  hbbox = ctk_button_box_new (GTK_ORIENTATION_HORIZONTAL);
  ctk_box_pack_start (GTK_BOX (vbox), hbbox, TRUE, TRUE, 5);
  
  for (i = 0; i < N_BUTTONS; i++) {
    buttons[i] = ctk_button_new_with_label (strings[i]);
    ctk_container_add (GTK_CONTAINER (hbbox), buttons[i]);
  }
  
  bbox = hbbox;

  ctk_button_box_set_layout (GTK_BUTTON_BOX (hbbox), GTK_BUTTONBOX_SPREAD);
  
  /* GtkVButtonBox */
  vbbox = ctk_button_box_new (GTK_ORIENTATION_VERTICAL);
  ctk_box_pack_start (GTK_BOX (vbox), vbbox, TRUE, TRUE, 5);
  ...
  ctk_box_pack_start (GTK_BOX (hbox), combo_types, TRUE, TRUE, 0);
  ...
  ctk_box_pack_start (GTK_BOX (hbox), combo_styles, TRUE, TRUE, 0);
  ...
  ctk_box_pack_start (GTK_BOX (hbox), option, FALSE, FALSE, 0);
