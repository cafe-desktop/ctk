...
inhibitor_toggled (GtkToggleButton *button, GtkApplication *app)
{
  ...
  reason = ctk_entry_get_text (GTK_ENTRY (inhibit_entry));

  flags = 0;
  if (ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (inhibit_logout)))
    flags |= GTK_APPLICATION_INHIBIT_LOGOUT;
  if (ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (inhibit_switch)))
    flags |= GTK_APPLICATION_INHIBIT_SWITCH;
  if (ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (inhibit_suspend)))
    flags |= GTK_APPLICATION_INHIBIT_SUSPEND;
  if (ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (inhibit_idle)))
  ...
  if (active)
    {
      ...
      cookie = ctk_application_inhibit (app, GTK_WINDOW (toplevel), flags, reason);
      ...
      if (cookie == 0)
      ...
      else
        {
          ...
          ctk_label_set_label (GTK_LABEL (inhibit_label), text);
          g_free (text);
        }
    }
  else
    {
      ...
      ctk_label_set_label (GTK_LABEL (inhibit_label), "");
    }
          ...
          gpointer        data)
{
  ...
  ctk_grid_attach (GTK_GRID (grid), inhibit_label, 1, 0, 1, 1);

  inhibit_logout = ctk_check_button_new_with_label ("Logout");
  ctk_grid_attach (GTK_GRID (grid), inhibit_logout, 1, 1, 1, 1);

  inhibit_switch = ctk_check_button_new_with_label ("User switching");
  ctk_grid_attach (GTK_GRID (grid), inhibit_switch, 1, 2, 1, 1);

  inhibit_suspend = ctk_check_button_new_with_label ("Suspend");
  ctk_grid_attach (GTK_GRID (grid), inhibit_suspend, 1, 4, 1, 1);

  inhibit_idle = ctk_check_button_new_with_label ("Idle");
  ctk_grid_attach (GTK_GRID (grid), inhibit_idle, 1, 5, 1, 1);

  inhibit_entry = ctk_entry_new ();
  ctk_grid_attach (GTK_GRID (grid), inhibit_entry, 1, 6, 1, 1);

  button = ctk_toggle_button_new_with_label ("Inhibit");
  g_signal_connect (button, "toggled",
                    G_CALLBACK (inhibitor_toggled), app);
  ctk_grid_attach (GTK_GRID (grid), button, 2, 6, 1, 1);
  ...
  ctk_application_add_window (app, GTK_WINDOW (win));
}
