           ...
           TestProperty   prop)
{
  ...
  ctk_scrolled_window_set_propagate_natural_width (GTK_SCROLLED_WINDOW (scrolledwindow), TRUE);
  ctk_scrolled_window_set_propagate_natural_height (GTK_SCROLLED_WINDOW (scrolledwindow), TRUE);
  ctk_scrolled_window_set_overlay_scrolling (GTK_SCROLLED_WINDOW (scrolledwindow), overlay);
  ctk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolledwindow), policy, policy);
  ctk_container_add (GTK_CONTAINER (scrolledwindow), box);
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL)
    {
      if (prop & MINIMUM_CONTENT)
        {
          ctk_scrolled_window_set_min_content_width (GTK_SCROLLED_WINDOW (scrolledwindow), MIN_SIZE);
      ...
      if (prop & MAXIMUM_CONTENT)
        {
          ctk_scrolled_window_set_max_content_width (GTK_SCROLLED_WINDOW (scrolledwindow), MAX_SIZE);
      ...
      if (!overlay && policy == GTK_POLICY_ALWAYS)
        {
          GtkWidget *scrollbar = ctk_scrolled_window_get_vscrollbar (GTK_SCROLLED_WINDOW (scrolledwindow));
  ...
  /* Testing the content-height property */
  else
    {
      if (prop & MINIMUM_CONTENT)
        {
          ctk_scrolled_window_set_min_content_height (GTK_SCROLLED_WINDOW (scrolledwindow), MIN_SIZE);
      ...
      if (prop & MAXIMUM_CONTENT)
        {
          ctk_scrolled_window_set_max_content_height (GTK_SCROLLED_WINDOW (scrolledwindow), MAX_SIZE);
      ...
      if (!overlay && policy == GTK_POLICY_ALWAYS)
        {
          GtkWidget *scrollbar = ctk_scrolled_window_get_hscrollbar (GTK_SCROLLED_WINDOW (scrolledwindow));
