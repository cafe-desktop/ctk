...
#undef RESET_BIT
      ...
      rv = CreateCursor (_cdk_app_hmodule, cursors[i].hotx, cursors[i].hoty,
			 w, h, and_plane, xor_plane);
    }
  else
    {
      rv = CreateCursor (_cdk_app_hmodule, 0, 0,
			 w, h, and_plane, xor_plane);
    }
...
win32_cursor_create_hcursor (Win32Cursor *cursor)
{
  ...
  switch (cursor->load_type)
    {
      case CDK_WIN32_CURSOR_LOAD_FROM_FILE:
        result = LoadImageW (NULL,
                             ...
                             cursor->load_flags);
        break;
      case CDK_WIN32_CURSOR_LOAD_FROM_RESOURCE_NULL:
        result = LoadImageA (NULL,
                             ...
                             cursor->load_flags);
        break;
      case CDK_WIN32_CURSOR_LOAD_FROM_RESOURCE_THIS:
        result = LoadImageA (_cdk_app_hmodule,
                             (const gchar *) cursor->resource_name,
                             IMAGE_CURSOR,
                             cursor->width,
                             cursor->height,
...
static Win32Cursor *
win32_cursor_new (CdkWin32CursorLoadType load_type,
...
win32_cursor_destroy (gpointer data)
{
   ...
   */
  if (cursor->load_type == CDK_WIN32_CURSOR_LOAD_FROM_FILE)
  ...
  while ((filename = g_dir_read_name (gdir)) != NULL)
    {
      ...
      cursor = win32_cursor_new (CDK_WIN32_CURSOR_LOAD_FROM_FILE,
                                 filenamew,
                                 size,
                                 size,
                                 LR_LOADFROMFILE | (size == 0 ? LR_DEFAULTSIZE : 0),
  ...
  for (i = 0; i < G_N_ELEMENTS (cursors); i++)
    {
      ...
      DestroyCursor (hcursor);
      cursor = win32_cursor_new (CDK_WIN32_CURSOR_LOAD_FROM_RESOURCE_NULL,
...
static HCURSOR
hcursor_from_type (CdkCursorType cursor_type)
{
  gint i = 0;
...
struct _CdkWin32CursorClass
{
  CdkCursorClass cursor_class;
};

G_DEFINE_TYPE (CdkWin32Cursor, cdk_win32_cursor, CDK_TYPE_CURSOR)

static void
_cdk_win32_cursor_finalize (GObject *object)
{
  CdkWin32Cursor *private = CDK_WIN32_CURSOR (object);
  ...
  G_OBJECT_CLASS (cdk_win32_cursor_parent_class)->finalize (object);
}
...
static HCURSOR
hcursor_from_theme (CdkDisplay  *display,
                    const gchar *name)
{
  ...
  theme = _cdk_win32_display_get_cursor_theme (win32_display);
...
static HCURSOR
hcursor_from_name (CdkDisplay  *display,
                   const gchar *name)
{
  HCURSOR hcursor;

  if (strcmp (name, "none") == 0)
    return hcursor_from_type (CDK_BLANK_CURSOR);
...
static CdkCursor*
cursor_new_from_hcursor (CdkDisplay    *display,
                         HCURSOR        hcursor,
			 const gchar   *name,
			 CdkCursorType  cursor_type)
{
  CdkWin32Cursor *private;
  CdkCursor *cursor;

  private = g_object_new (CDK_TYPE_WIN32_CURSOR,
...
static gboolean
_cdk_win32_cursor_update (CdkWin32Display *win32_display,
                          CdkWin32Cursor  *cursor)
{
...
void
_cdk_win32_display_update_cursors (CdkWin32Display *display)
{
  GHashTableIter iter;
  const char *name;
  CdkWin32Cursor *cursor;
  ...
  while (g_hash_table_iter_next (&iter, (gpointer *) &name, (gpointer *) &cursor))
    _cdk_win32_cursor_update (display, cursor);
}

void
_cdk_win32_display_init_cursors (CdkWin32Display *display)
{
...
void
_cdk_win32_display_finalize_cursors (CdkWin32Display *display)
{
...
CdkCursor*
_cdk_win32_display_get_cursor_for_type (CdkDisplay   *display,
					CdkCursorType cursor_type)
{
  ...
  if (hcursor == NULL)
    g_warning ("cdk_cursor_new_for_display: no cursor %d found", cursor_type);
  else
    CDK_NOTE (CURSOR, g_print ("cdk_cursor_new_for_display: %d: %p\n",
			       cursor_type, hcursor));
...
CdkCursor*
_cdk_win32_display_get_cursor_for_name (CdkDisplay  *display,
				        const gchar *name)
{
  HCURSOR hcursor = NULL;
  CdkCursor *result;
  ...
  if (!hcursor)
    hcursor = LoadCursor (_cdk_app_hmodule, name);

  if (hcursor == NULL)
    return NULL;

  result = cursor_new_from_hcursor (display, hcursor, name, CDK_X_CURSOR);
...
_cdk_win32_cursor_get_surface (CdkCursor *cursor,
			       gdouble *x_hot,
			       gdouble *y_hot)
{
  CdkPixbuf *pixbuf;
  ...
  pixbuf = cdk_win32_icon_to_pixbuf_libctk_only (((CdkWin32Cursor *) cursor)->hcursor, x_hot, y_hot);
...
CdkCursor *
_cdk_win32_display_get_cursor_for_surface (CdkDisplay      *display,
					   cairo_surface_t *surface,
					   gdouble          x,
					   gdouble          y)
{
  ...
  if (!hcursor)
    return NULL;
  return cursor_new_from_hcursor (display, hcursor, NULL, CDK_CURSOR_IS_PIXMAP);
}

gboolean
_cdk_win32_display_supports_cursor_alpha (CdkDisplay    *display)
{
  return TRUE;
}

gboolean
_cdk_win32_display_supports_cursor_color (CdkDisplay    *display)
{
  return TRUE;
}

void
_cdk_win32_display_get_default_cursor_size (CdkDisplay *display,
					    guint      *width,
					    guint      *height)
{
...
void
_cdk_win32_display_get_maximal_cursor_size (CdkDisplay *display,
					    guint       *width,
					    guint       *height)
{
  if (width)
 ...
 * cdkwindow-win32.c for creating application icons.
 */
...
static void
cdk_win32_cursor_init (CdkWin32Cursor *cursor)
{
}
static void
cdk_win32_cursor_class_init(CdkWin32CursorClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  CdkCursorClass *cursor_class = CDK_CURSOR_CLASS (klass);

  object_class->finalize = _cdk_win32_cursor_finalize;

  cursor_class->get_surface = _cdk_win32_cursor_get_surface;
}
