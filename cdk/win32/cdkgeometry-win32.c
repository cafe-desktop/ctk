...
/* cdkgeometry-win32.c: emulation of 32 bit coordinates within the
 ...
 * back to the real position determined by cdk_window_compute_position().
 * This is handled in cdk_window_postmove().
 *
...
#include "config.h"
#include "cdk.h"		/* For cdk_rectangle_intersect */
			       ...
			       gint       height)
{
  ...
  _cdk_win32_window_tmp_reset_bg (window, TRUE);
}
				...
				gboolean recurse)
{
  ...
  if (recurse)
	...
	_cdk_win32_window_tmp_unset_bg (l->data, TRUE);
    }
}
...
_cdk_win32_window_tmp_unset_parent_bg (CdkWindow *window)
{
  ...
  _cdk_win32_window_tmp_unset_bg (window, FALSE);
}
  ...
  if (recurse)
    {
      ...
      for (l = window->children; l != NULL; l = l->next)
	_cdk_win32_window_tmp_reset_bg (l->data, TRUE);
    }
}
