...
G_DEFINE_TYPE (GdkWin32DragContext, cdk_win32_drag_context, GDK_TYPE_DRAG_CONTEXT)
...
static void
cdk_win32_drag_context_init (GdkWin32DragContext *context)
{
  if (!use_ole2_dnd)
    {
  ...
  GDK_NOTE (DND, g_print ("cdk_drag_context_init %p\n", context));
}
...
cdk_win32_drag_context_finalize (GObject *object)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_drag_context_finalize %p\n", object));
...
cdk_drag_context_new (GdkDisplay *display)
{
  GdkWin32DragContext *context_win32;
	       ...
	       gboolean        to_dest_window)
{
  ...
  GDK_NOTE (EVENTS, _cdk_win32_print_event (e));
  cdk_event_put (e);
  cdk_event_free (e);
}
			  ...
			  DWORD        dwEffect)
{
  ...
  e = cdk_event_new (GDK_DRAG_STATUS);
		     ...
		     LPSTGMEDIUM  pMedium)
{
   ...
   */
  e.selection.selection = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_OLE2_DND);
  ...
  e.selection.property = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_OLE2_DND);
  ...
  for (p = result->context->targets; p; p = g_list_next (p))
    {
      ...
      if (target == _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_TEXT_URI_LIST))
        context_win32->has_text_uri_list = TRUE;
      else if (target == _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_CFSTR_SHELLIDLIST))
        context_win32->has_shell_id_list = TRUE;
      else if (target == _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_PNG))
        context_win32->has_cf_png = TRUE;
      else if (target == _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_CF_DIB))
        context_win32->has_cf_dib = TRUE;
      else if (target == _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_GIF))
        context_win32->has_gif = TRUE;
      else if (target == _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_JFIF))
        context_win32->has_jfif = TRUE;
      else if (target == _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_CF_UNICODETEXT))
...
cdk_dropfiles_filter (GdkXEvent *xev,
		      GdkEvent  *event,
		      gpointer   data)
{
  GdkDragContext *context;
  ...
  if (msg->message == WM_DROPFILES)
    {
      ...
      context->targets =
	g_list_append (NULL, _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_TEXT_URI_LIST));
		  ...
		  guint32         time)
{
  ...
  if ((current_dest_drag != NULL) &&
    ...
    {
      tmp_event = cdk_event_new (GDK_DRAG_LEAVE);
      ...
      GDK_NOTE (EVENTS, _cdk_win32_print_event (tmp_event));
		  ...
		  guint32         time)
{
  ...
  tmp_event = cdk_event_new (GDK_DRAG_ENTER);
  g_set_object (&tmp_event->dnd.window, context->dest_window);
  tmp_event->dnd.send_event = FALSE;
		   ...
		   guint32         time)
{
  ...
  if ((current_dest_drag != NULL) &&
      ...
      (current_dest_drag->source_window == context->source_window))
    {
      ...
      tmp_event = cdk_event_new (GDK_DRAG_MOTION);
		 ...
		 guint32         time)
{
  ...
  if ((current_dest_drag != NULL) &&
      ...
      (current_dest_drag->source_window == context->source_window))
    {
      ...
      /* Pass ownership of context to the event */
      tmp_event = cdk_event_new (GDK_DROP_START);
      g_set_object (&tmp_event->dnd.window, current_dest_drag->dest_window);
      tmp_event->dnd.send_event = FALSE;
      g_set_object (&tmp_event->dnd.context, current_dest_drag);
...
static void
cdk_drag_do_leave (GdkDragContext *context,
		   guint32         time)
{
  if (context->dest_window)
    {
      GDK_NOTE (DND, g_print ("cdk_drag_do_leave\n"));
...
GdkDragContext *
_cdk_win32_window_drag_begin (GdkWindow *window,
			      GdkDevice *device,
			      GList     *targets,
                              gint       x_root,
                              gint       y_root)
{
  ...
  if (!use_ole2_dnd)
  ...
  else
    {
      ...
      GDK_NOTE (DND, g_print ("cdk_drag_begin\n"));
...
_cdk_win32_dnd_do_dragdrop (void)
{
  ...
  _cdk_win32_begin_modal_call (GDK_WIN32_MODAL_OP_DND);
  hr = DoDragDrop (&dobj->ido, &pending_src_context->ids,
    	       DROPEFFECT_COPY | DROPEFFECT_MOVE,
    	       &dwEffect);
  _cdk_win32_end_modal_call (GDK_WIN32_MODAL_OP_DND);
  ...
  if (hr == DRAGDROP_S_DROP && dwEffect == DROPEFFECT_MOVE)
    {
      ...
      tmp_event.selection.selection = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_OLE2_DND);
      tmp_event.selection.target = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_DELETE);
      win32_sel->property_change_target_atom = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_DELETE);
      tmp_event.selection.property = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_OLE2_DND);
  ...
  {
    GdkEvent *tmp_event;
    tmp_event = cdk_event_new (GDK_DROP_FINISHED);
 ...
 */
GdkDragProtocol
_cdk_win32_window_get_drag_protocol (GdkWindow *window,
                                     GdkWindow **target)
{
...
static GdkWindow *
cdk_win32_drag_context_find_window (GdkDragContext  *context,
				    GdkWindow       *drag_window,
				    GdkScreen       *screen,
				    gint             x_root,
				    gint             y_root,
...
static gboolean
cdk_win32_drag_context_drag_motion (GdkDragContext *context,
		 GdkWindow      *dest_window,
		 GdkDragProtocol protocol,
		 gint            x_root,
		 gint            y_root,
		 ...
		 guint32         time)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_drag_motion: @ %+d:%+d %s suggested=%s, possible=%s\n"
  ...
  if (!use_ole2_dnd)
      ...
      else
	{
	   ...
	   * the drag changed
	   */
	  tmp_event = cdk_event_new (GDK_DRAG_STATUS);
...
static void
cdk_win32_drag_context_drag_drop (GdkDragContext *context,
	       guint32         time)
{
  GdkWin32Selection *sel_win32 = _cdk_win32_selection_get ();
  ...
  GDK_NOTE (DND, g_print ("cdk_drag_drop\n"));
...
static void
cdk_win32_drag_context_drag_abort (GdkDragContext *context,
		guint32         time)
{
  GdkWin32Selection *sel_win32 = _cdk_win32_selection_get ();
  ...
  GDK_NOTE (DND, g_print ("cdk_drag_abort\n"));
...
static void
cdk_win32_drag_context_drag_status (GdkDragContext *context,
		 GdkDragAction   action,
		 guint32         time)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_drag_status: %s\n"
			  " context=%p:{actions=%s,suggested=%s,action=%s}\n",
...
static void
cdk_win32_drag_context_drop_reply (GdkDragContext *context,
		gboolean        ok,
		guint32         time)
{
  g_return_if_fail (context != NULL);

  GDK_NOTE (DND, g_print ("cdk_drop_reply\n"));
...
static void
cdk_win32_drag_context_drop_finish (GdkDragContext *context,
		 gboolean        success,
		 guint32         time)
{
  GdkDragContext *src_context;
  ...
  GDK_NOTE (DND, g_print ("cdk_drop_finish\n"));
  ...
  if (!use_ole2_dnd)
    {
      ...
      if (src_context)
	{
          if (cdk_drag_context_get_selected_action (src_context) == GDK_ACTION_MOVE)
            {
              tmp_event = cdk_event_new (GDK_SELECTION_REQUEST);
              g_set_object (&tmp_event->selection.window, src_context->source_window);
              tmp_event->selection.send_event = FALSE;
              tmp_event->selection.selection = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_LOCAL_DND_SELECTION);
              tmp_event->selection.target = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_DELETE);
              sel_win32->property_change_target_atom = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_DELETE);
              tmp_event->selection.property = _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_LOCAL_DND_SELECTION);
...
static GdkFilterReturn
cdk_destroy_filter (GdkXEvent *xev,
		    GdkEvent  *event,
		    gpointer   data)
{
  MSG *msg = (MSG *) xev;
  ...
  if (msg->message == WM_DESTROY)
    {
      ...
      GDK_NOTE (DND, g_print ("cdk_destroy_filter: WM_DESTROY: %p\n", msg->hwnd));
#if 0
...
void
_cdk_win32_window_register_dnd (GdkWindow *window)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_window_register_dnd: %p\n", GDK_WINDOW_HWND (window)));
...
static gboolean
cdk_win32_drag_context_drop_status (GdkDragContext *context)
{
  GdkWin32DragContext *context_win32 = GDK_WIN32_DRAG_CONTEXT (context);
...
static GdkAtom
cdk_win32_drag_context_get_selection (GdkDragContext *context)
{
  switch (context->protocol)
    {
    case GDK_DRAG_PROTO_LOCAL:
      return _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_LOCAL_DND_SELECTION);
    case GDK_DRAG_PROTO_WIN32_DROPFILES:
      return _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_DROPFILES_DND);
    case GDK_DRAG_PROTO_OLE2:
      return _cdk_win32_selection_atom (GDK_WIN32_ATOM_INDEX_OLE2_DND);
    default:
      return GDK_NONE;
    }
}
...
static void
cdk_win32_drag_context_set_cursor (GdkDragContext *context,
                                   GdkCursor      *cursor)
{
  GdkWin32DragContext *context_win32 = GDK_WIN32_DRAG_CONTEXT (context);

  GDK_NOTE (DND, g_print ("cdk_drag_context_set_cursor: 0x%p 0x%p\n", context, cursor));
...
static void
cdk_win32_drag_context_drop_done (GdkDragContext *context,
                                  gboolean        success)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_drag_context_drop_done: 0x%p %s\n",
                          context,
...
static gboolean
cdk_win32_drag_context_manage_dnd (GdkDragContext *context,
...
static void
cdk_win32_drag_context_cancel (GdkDragContext      *context,
                               GdkDragCancelReason  reason)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_drag_context_cancel: 0x%p %s\n",
                          context,
                          reason_str));
...
static void
cdk_win32_drag_context_drop_performed (GdkDragContext *context,
                                       guint32         time_)
{
  GDK_NOTE (DND, g_print ("cdk_drag_context_drop_performed: 0x%p %u\n",
                          context,
                          time_));
                 ...
                 guint32          evtime)
{
  ...
  cdk_drag_get_current_actions (mods, GDK_BUTTON_PRIMARY, win32_context->actions,
                          ...
                          const GdkEventKey *event)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_dnd_handle_key_event: 0x%p\n",
                          context));
                                  ...
                                  const GdkEventGrabBroken *event)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_dnd_handle_grab_broken_event: 0x%p\n",
                          context));
...
{
  GDK_NOTE (DND, g_print ("cdk_dnd_handle_button_event: 0x%p\n",
                          context));
                            ...
                            const GdkEventDND *event)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_dnd_handle_drag_status: 0x%p\n",
                          context));
                              ...
                              const GdkEventDND *event)
{
  ...
  GDK_NOTE (DND, g_print ("cdk_dnd_handle_drop_finihsed: 0x%p\n",
                          context));
...
gboolean
cdk_win32_drag_context_handle_event (GdkDragContext *context,
...
void
cdk_win32_drag_context_action_changed (GdkDragContext *context,
                                       GdkDragAction   action)
{
  GdkCursor *cursor;
...
static GdkWindow *
cdk_win32_drag_context_get_drag_window (GdkDragContext *context)
{
  return GDK_WIN32_DRAG_CONTEXT (context)->drag_window;
}

static void
cdk_win32_drag_context_set_hotspot (GdkDragContext *context,
                                    gint            hot_x,
                                    gint            hot_y)
{
  GdkWin32DragContext *win32_context = GDK_WIN32_DRAG_CONTEXT (context);

  GDK_NOTE (DND, g_print ("cdk_drag_context_set_hotspot: 0x%p %d:%d\n",
                          context,
                          hot_x, hot_y));
...
static void
cdk_win32_drag_context_class_init (GdkWin32DragContextClass *klass)
{
