...
startup_timeout (void *data)
{
  ...
  if (std->contexts == NULL)
    ...
    g_source_set_name_by_id (std->timeout_id, "[ctk+] startup_timeout");
  }
                     ...
                     const char *startup_id)
{
  ...
  if (data->timeout_id == 0) {
    ...
    g_source_set_name_by_id (data->timeout_id, "[ctk+] startup_timeout");
  }
}
...
G_DEFINE_TYPE (CdkX11AppLaunchContext, cdk_x11_app_launch_context, CDK_TYPE_APP_LAUNCH_CONTEXT)
...
_cdk_x11_display_get_app_launch_context (CdkDisplay *display)
{
  ...
  ctx = g_object_new (CDK_TYPE_X11_APP_LAUNCH_CONTEXT,
                      "display", display,
                      NULL);
