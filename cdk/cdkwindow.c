...
/* Historically a CdkWindow always matches a platform native window,
...
/* This adds a local value to the CdkVisibilityState enum */
#define CDK_VISIBILITY_NOT_VIEWABLE 3
...
static void cdk_window_finalize   (GObject              *object);

static void cdk_window_set_property (GObject      *object,
                                     guint         prop_id,
                                     const GValue *value,
                                     GParamSpec   *pspec);
static void cdk_window_get_property (GObject      *object,
                                     guint         prop_id,
                                     GValue       *value,
                                     GParamSpec   *pspec);

static void cdk_window_clear_backing_region (CdkWindow *window);

static void recompute_visible_regions   (CdkWindow *private,
					 gboolean recalculate_children);
static void cdk_window_invalidate_in_parent (CdkWindow *private);
static void move_native_children        (CdkWindow *private);
static void update_cursor               (CdkDisplay *display,
                                         CdkDevice  *device);
static void impl_window_add_update_area (CdkWindow *impl_window,
					 cairo_region_t *region);
static void cdk_window_invalidate_region_full (CdkWindow       *window,
					       const cairo_region_t *region,
					       gboolean         invalidate_children);
static void cdk_window_invalidate_rect_full (CdkWindow          *window,
					     const CdkRectangle *rect,
					     gboolean            invalidate_children);
static cairo_surface_t *cdk_window_ref_impl_surface (CdkWindow *window);

static void cdk_window_set_frame_clock (CdkWindow      *window,
                                        CdkFrameClock  *clock);

static void draw_ugly_color (CdkWindow       *window,
                             const cairo_region_t *region,
                             int color);
...
G_DEFINE_ABSTRACT_TYPE (CdkWindow, cdk_window, G_TYPE_OBJECT)
...
static void
cdk_window_init (CdkWindow *window)
{
  ...
  /* starts hidden */
  window->effective_visibility = CDK_VISIBILITY_NOT_VIEWABLE;
...
static void
cdk_window_class_init (CdkWindowClass *klass)
{
  ...
  signals[PICK_EMBEDDED_CHILD] =
    g_signal_new (g_intern_static_string ("pick-embedded-child"),
		  ...
		  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (CdkWindowClass, pick_embedded_child),
  ...
  signals[TO_EMBEDDER] =
    g_signal_new (g_intern_static_string ("to-embedder"),
		  G_OBJECT_CLASS_TYPE (object_class),
		  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (CdkWindowClass, to_embedder),
		  NULL, NULL,
  ...
  signals[FROM_EMBEDDER] =
    g_signal_new (g_intern_static_string ("from-embedder"),
		  G_OBJECT_CLASS_TYPE (object_class),
		  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (CdkWindowClass, from_embedder),
		  NULL, NULL,
		  _cdk_marshal_VOID__DOUBLE_DOUBLE_POINTER_POINTER,
		  G_TYPE_NONE,
		  4,
  ...
  signals[CREATE_SURFACE] =
    g_signal_new (g_intern_static_string ("create-surface"),
                  G_OBJECT_CLASS_TYPE (object_class),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (CdkWindowClass, create_surface),
...
static void
cdk_window_finalize (GObject *object)
{
  CdkWindow *window = CDK_WINDOW (object);

  g_signal_handlers_disconnect_by_func (cdk_window_get_display (window),
  ...
  if (!CDK_WINDOW_DESTROYED (window))
      ...
      if (CDK_WINDOW_TYPE (window) != CDK_WINDOW_FOREIGN)
	{
	  ...
	  _cdk_window_destroy (window, FALSE);
	}
      else
  ...
  G_OBJECT_CLASS (cdk_window_parent_class)->finalize (object);
}

static void
cdk_window_set_property (GObject      *object,
                         guint         prop_id,
                         const GValue *value,
                         GParamSpec   *pspec)
{
...
static void
cdk_window_get_property (GObject    *object,
                         guint       prop_id,
                         GValue     *value,
                         GParamSpec *pspec)
{
...
static gboolean
cdk_window_is_offscreen (CdkWindow *window)
{
  return window->window_type == CDK_WINDOW_OFFSCREEN;
}

static gboolean
cdk_window_is_subsurface (CdkWindow *window)
{
   return window->window_type == CDK_WINDOW_SUBSURFACE;
}

static CdkWindow *
cdk_window_get_impl_window (CdkWindow *window)
{
  return window->impl_window;
}

CdkWindow *
_cdk_window_get_impl_window (CdkWindow *window)
{
  return cdk_window_get_impl_window (window);
}

static gboolean
cdk_window_has_impl (CdkWindow *window)
{
  return window->impl_window == window;
}

static gboolean
cdk_window_is_toplevel (CdkWindow *window)
{
  return
    window->parent == NULL ||
...
gboolean
_cdk_window_has_impl (CdkWindow *window)
{
  return cdk_window_has_impl (window);
}

static gboolean
cdk_window_has_no_impl (CdkWindow *window)
{
  return window->impl_window != window;
}

static void
remove_sibling_overlapped_area (CdkWindow *window,
				cairo_region_t *region)
{
  CdkWindow *parent;
  CdkWindow *sibling;
  ...
  for (l = parent->children; l; l = l->next)
	...
	break;
      ...
      if (!CDK_WINDOW_IS_MAPPED (sibling) || sibling->input_only || sibling->composited)
	continue;
	...
	{
	  shape = CDK_WINDOW_IMPL_GET_CLASS (sibling)->get_shape (sibling);
	  if (shape)
	    {
...
static void
remove_child_area (CdkWindow *window,
		   gboolean for_input,
		   cairo_region_t *region)
{
  CdkWindow *child;
  ...
  for (l = window->children; l; l = l->next)
	...
	break;
      ...
      if (!CDK_WINDOW_IS_MAPPED (child) || child->input_only || child->composited)
	continue;
	...
	{
	  shape = CDK_WINDOW_IMPL_GET_CLASS (child)->get_shape (child);
	  if (shape)
	    {
	      cairo_region_intersect (child_region, shape);
	      cairo_region_destroy (shape);
      ...
      if (for_input)
	{
	  if (child->input_shape)
	    ...
	    {
	      shape = CDK_WINDOW_IMPL_GET_CLASS (child)->get_input_shape (child);
	      if (shape)
		{
		  cairo_region_intersect (child_region, shape);
		  cairo_region_destroy (shape);
...
static CdkVisibilityState
effective_visibility (CdkWindow *window)
{
  CdkVisibilityState native;

  if (!cdk_window_is_viewable (window))
    return CDK_VISIBILITY_NOT_VIEWABLE;
...
static void
cdk_window_update_visibility (CdkWindow *window)
{
  ...
  if (new_visibility != window->effective_visibility)
    {
      ...
      if (new_visibility != CDK_VISIBILITY_NOT_VIEWABLE &&
...
static void
cdk_window_update_visibility_recursively (CdkWindow *window,
...
static gboolean
should_apply_clip_as_shape (CdkWindow *window)
{
  return
...
static void
apply_clip_as_shape (CdkWindow *window)
{
  CdkRectangle r;
  cairo_region_t *region;
...
static void
recompute_visible_regions_internal (CdkWindow *private,
 ...
 */
static void
recompute_visible_regions (CdkWindow *private,
			   gboolean recalculate_children)
{
  CdkWindow *toplevel;
...
static void
cdk_window_clear_old_updated_area (CdkWindow *window)
{
  int i;
...
static void
cdk_window_append_old_updated_area (CdkWindow *window,
                                    cairo_region_t *region)
{
...
void
_cdk_window_update_size (CdkWindow *window)
{
 ...
 */
static CdkWindow *
find_native_sibling_above_helper (CdkWindow *parent,
				  CdkWindow *child)
{
  CdkWindow *w;
  GList *l;
...
static CdkWindow *
find_native_sibling_above (CdkWindow *parent,
			   CdkWindow *child)
{
  CdkWindow *w;
...
static CdkEventMask
get_native_device_event_mask (CdkWindow *private,
                              CdkDevice *device)
{
...
static CdkEventMask
get_native_event_mask (CdkWindow *private)
{
 ...
 */
static void
sync_native_window_stack_position (CdkWindow *window)
{
  CdkWindow *above;
  CdkWindowImplClass *impl_class;
  GList listhead = {0};
 ...
 **/
CdkWindow*
cdk_window_new (CdkWindow     *parent,
		...
		gint           attributes_mask)
{
  ...
  g_return_val_if_fail (CDK_IS_WINDOW (parent), NULL);

  if (CDK_WINDOW_DESTROYED (parent))
    {
  ...
  if (attributes_mask & CDK_WA_VISUAL)
    {
      g_return_val_if_fail (cdk_visual_get_screen (attributes->visual) == screen, NULL);
    }
   ...
   */
  real_parent = parent;
  if (CDK_WINDOW_TYPE (parent) == CDK_WINDOW_FOREIGN)
    parent = cdk_screen_get_root_window (screen);
  ...
  if (attributes->wclass == CDK_INPUT_ONLY)
    {
       ...
       * before
       */
      if (CDK_WINDOW_TYPE (parent) == CDK_WINDOW_ROOT)
  ...
  if (window->parent->window_type == CDK_WINDOW_ROOT)
    {
      CdkFrameClock *frame_clock = g_object_new (CDK_TYPE_FRAME_CLOCK_IDLE, NULL);
  ...
  g_signal_connect (cdk_window_get_display (parent), "seat-removed",
  ...
  if ((_cdk_gl_flags & (CDK_GL_ALWAYS | CDK_GL_DISABLE)) == CDK_GL_ALWAYS)
    {
      ...
      if (cdk_window_get_paint_gl_context (window, &error) == NULL)
        {
...
static void
reparent_to_impl (CdkWindow *private)
{
  GList *l;
  CdkWindow *child;
  gboolean show;
  ...
  impl_class = CDK_WINDOW_IMPL_GET_CLASS (private->impl);
 ...
 **/
void
cdk_window_reparent (CdkWindow *window,
		     ...
		     gint       y)
{
  ...
  g_return_if_fail (new_parent == NULL || CDK_IS_WINDOW (new_parent));
  g_return_if_fail (CDK_WINDOW_TYPE (window) != CDK_WINDOW_ROOT);

  if (CDK_WINDOW_DESTROYED (window) ||
      (new_parent && CDK_WINDOW_DESTROYED (new_parent)))
    return;
  ...
  if (cdk_window_has_impl (window))
  ...
  else
    {
      ...
      g_assert (new_parent->window_type != CDK_WINDOW_ROOT &&
		new_parent->window_type != CDK_WINDOW_FOREIGN);
  ...
  switch (CDK_WINDOW_TYPE (new_parent))
	...
	CDK_WINDOW_TYPE (window) = window->toplevel_window_type;
	  ...
	  CDK_WINDOW_TYPE (window) != CDK_WINDOW_FOREIGN)
	{
	   ...
	   */
	  window->toplevel_window_type = CDK_WINDOW_TYPE (window);
   ...
   */
  if (CDK_WINDOW_TYPE (new_parent) == CDK_WINDOW_ROOT &&
      CDK_WINDOW_TYPE (window) != CDK_WINDOW_FOREIGN)
    {
      ...
      if (window->frame_clock != NULL)
        {
          ...
          cdk_window_set_frame_clock (window, NULL);
        }
    }
 ...
 */
gboolean
cdk_window_ensure_native (CdkWindow *window)
{
  CdkWindow *impl_window;
  ...
  if (CDK_WINDOW_TYPE (window) == CDK_WINDOW_ROOT ||
 ...
 */
void
_cdk_event_filter_unref (CdkWindow       *window,
...
static void
window_remove_filters (CdkWindow *window)
{
...
static void
window_remove_from_pointer_info (CdkWindow  *window,
                                 CdkDisplay *display)
{
...
static void
cdk_window_free_current_paint (CdkWindow *window)
{
 ...
 **/
static void
_cdk_window_destroy_hierarchy (CdkWindow *window,
			       gboolean   recursing,
 ...
 **/
void
_cdk_window_destroy (CdkWindow *window,
		     gboolean   foreign_destroy)
{
  _cdk_window_destroy_hierarchy (window, FALSE, FALSE, foreign_destroy);
}
 ...
 **/
void
cdk_window_destroy (CdkWindow *window)
{
  _cdk_window_destroy_hierarchy (window, FALSE, FALSE, FALSE);
 ...
 **/
void
cdk_window_set_user_data (CdkWindow *window,
			  gpointer   user_data)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 **/
void
cdk_window_get_user_data (CdkWindow *window,
			  gpointer  *data)
{
 ...
 **/
CdkWindowType
cdk_window_get_window_type (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), (CdkWindowType) -1);

  return CDK_WINDOW_TYPE (window);
}
 ...
 **/
CdkVisual*
cdk_window_get_visual (CdkWindow *window)
{
 ...
 **/
CdkScreen*
cdk_window_get_screen (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 **/
CdkDisplay *
cdk_window_get_display (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 **/
gboolean
cdk_window_is_destroyed (CdkWindow *window)
{
  return CDK_WINDOW_DESTROYED (window);
}
 ...
 */
gboolean
cdk_window_has_native (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
 ...
 **/
void
cdk_window_get_position (CdkWindow *window,
			 gint      *x,
			 gint      *y)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 **/
CdkWindow*
cdk_window_get_parent (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 **/
CdkWindow *
cdk_window_get_effective_parent (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 **/
CdkWindow *
cdk_window_get_toplevel (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 **/
CdkWindow *
cdk_window_get_effective_toplevel (CdkWindow *window)
{
  CdkWindow *parent;
 ...
 **/
GList*
cdk_window_get_children (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 **/
GList *
cdk_window_peek_children (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 **/
GList *
cdk_window_get_children_with_user_data (CdkWindow *window,
                                        gpointer   user_data)
{
  CdkWindow *child;
  GList *res, *l;
 ...
 **/
void
cdk_window_add_filter (CdkWindow     *window,
 ...
 */
void
cdk_window_remove_filter (CdkWindow     *window,
                          CdkFilterFunc  function,
                          gpointer       data)
{
  GList *tmp_list;
 ...
 **/
gboolean
cdk_window_is_visible (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);

  return CDK_WINDOW_IS_MAPPED (window);
}
 ...
 **/
gboolean
cdk_window_is_viewable (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
 ...
 **/
CdkWindowState
cdk_window_get_state (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
...
cdk_window_get_content (CdkWindow *window)
{
  cairo_surface_t *surface;
  cairo_content_t content;

  g_return_val_if_fail (CDK_IS_WINDOW (window), 0);
...
cdk_window_ref_impl_surface (CdkWindow *window)
{
  return CDK_WINDOW_IMPL_GET_CLASS (window->impl)->ref_cairo_surface (cdk_window_get_impl_window (window));
}

CdkGLContext *
cdk_window_get_paint_gl_context (CdkWindow  *window,
                                 GError    **error)
{
  ...
  if (_cdk_gl_flags & CDK_GL_DISABLE)
    {
      g_set_error_literal (error, CDK_GL_ERROR,
                           ...
                           _("GL support disabled via CDK_DEBUG"));
      return NULL;
    }

  if (window->impl_window->gl_paint_context == NULL)
    {
      CdkWindowImplClass *impl_class = CDK_WINDOW_IMPL_GET_CLASS (window->impl);
 ...
 **/
CdkGLContext *
cdk_window_create_gl_context (CdkWindow    *window,
                              GError      **error)
{
  ...
  return CDK_WINDOW_IMPL_GET_CLASS (window->impl)->create_gl_context (window->impl_window,
								      FALSE,
                                                                      paint_context,
                                                                      error);
}
...
static void
cdk_window_begin_paint_internal (CdkWindow            *window,
			         const cairo_region_t *region)
{
  ...
  if (window->current_paint.use_gl)
    {
      ...
      context = cdk_window_get_paint_gl_context (window, NULL);
      if (context == NULL)
        {
...
static void
cdk_window_end_paint_internal (CdkWindow *window)
{
  ...
  if (window->current_paint.surface == NULL)
    {
      g_warning (G_STRLOC": no preceding call to cdk_window_begin_draw_frame(), see documentation");
      return;
    }
  ...
  for (composited = window;
	...
	{
	  cdk_window_invalidate_rect (CDK_WINDOW (composited->parent),
				      &clip_box, FALSE);
	  break;
	}
    }
 ...
 */
void
cdk_window_begin_paint_rect (CdkWindow          *window,
 ...
 */
void
cdk_window_begin_paint_region (CdkWindow            *window,
			       const cairo_region_t *region)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 */
CdkDrawingContext *
cdk_window_begin_draw_frame (CdkWindow            *window,
                             const cairo_region_t *region)
{
  ...
  if (impl_class->create_draw_context != NULL)
    ...
    {
      context = g_object_new (CDK_TYPE_DRAWING_CONTEXT,
                              "window", window,
                              "clip", region,
                              NULL);
    }
 ...
 */
void
cdk_window_end_draw_frame (CdkWindow         *window,
                           CdkDrawingContext *context)
{
  CdkWindowImplClass *impl_class;

  g_return_if_fail (CDK_IS_WINDOW (window));
  g_return_if_fail (CDK_IS_DRAWING_CONTEXT (context));
 ...
 */
cairo_region_t *
cdk_window_get_current_paint_region (CdkWindow *window)
{
  cairo_region_t *region;
 ...
 */
CdkDrawingContext *
cdk_window_get_drawing_context (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 **/
void
cdk_window_mark_paint_from_clip (CdkWindow *window,
                                 cairo_t   *cr)
{
 ...
 **/
void
cdk_window_end_paint (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 **/
cairo_region_t*
cdk_window_get_clip_region (CdkWindow *window)
{
  cairo_region_t *result;

  g_return_val_if_fail (CDK_WINDOW (window), NULL);
 ...
 **/
cairo_region_t *
cdk_window_get_visible_region (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
...
static void
cdk_window_clear_backing_region (CdkWindow *window)
{
  CdkWindow *bg_window;
...
static cairo_surface_t *
ref_window_surface (CdkWindow *window)
{
...
/* This is used in places like cdk_cairo_set_source_window and
...
gboolean _cdk_debug_updates = FALSE;

static inline gboolean
cdk_window_is_ancestor (CdkWindow *window,
			CdkWindow *ancestor)
{
  while (window)
    {
...
static void
cdk_window_add_update_window (CdkWindow *window)
{
  GSList *tmp;
  GSList *prev = NULL;
  ...
  for (tmp = update_windows; tmp; tmp = tmp->next)
	...
	has_ancestor_in_list = TRUE;
       ...
       */
      if (parent == CDK_WINDOW (tmp->data)->parent)
	{
	  gint index = g_list_index (parent->children, window);
	  for (; tmp && parent == CDK_WINDOW (tmp->data)->parent; tmp = tmp->next)
	    {
...
static void
cdk_window_remove_update_window (CdkWindow *window)
{
  GSList *link;
...
static gboolean
cdk_window_is_toplevel_frozen (CdkWindow *window)
{
  CdkWindow *toplevel;
...
static void
cdk_window_schedule_update (CdkWindow *window)
{
...
static void
cdk_window_add_damage (CdkWindow *toplevel,
                       cairo_region_t *damaged_region)
{
  ...
  g_assert (cdk_window_is_offscreen (toplevel));
...
static void
_cdk_window_process_updates_recurse_helper (CdkWindow *window,
                                            cairo_region_t *expose_region)
{
  CdkWindow *child;
  cairo_region_t *clipped_expose_region;
  CdkWindow **children;
  CdkWindow **free_children = NULL;
  int i, n_children;
  GList *l;
  GList *last_link;
  ...
  children = g_newa (CdkWindow *, n_children);
  if (children == NULL)
    children = free_children = g_new (CdkWindow *, n_children);
  ...
  for (l = last_link; l != NULL; l = l->prev)
    {
      ...
      if (child->destroyed || !CDK_WINDOW_IS_MAPPED (child) || child->input_only || child->composited)
        continue;
...
void
_cdk_window_process_updates_recurse (CdkWindow *window,
                                     cairo_region_t *expose_region)
{
...
static void
cdk_window_update_native_shapes (CdkWindow *window)
{
  CdkWindow *child;
  GList *l;
 ...
 */
static void
cdk_window_process_updates_internal (CdkWindow *window)
{
  CdkWindowImplClass *impl_class;
  CdkWindow *toplevel;
  CdkDisplay *display;
...
before_process_all_updates (void)
{
  ...
  displays = cdk_display_manager_list_displays (cdk_display_manager_get ());
  for (l = displays; l; l = l->next)
    CDK_DISPLAY_GET_CLASS (l->data)->before_process_all_updates (l->data);
...
after_process_all_updates (void)
{
  ...
  displays = cdk_display_manager_list_displays (cdk_display_manager_get ());
  for (l = displays; l; l = l->next)
    CDK_DISPLAY_GET_CLASS (l->data)->after_process_all_updates (l->data);
...
cdk_window_process_all_updates (void)
{
  ...
  if (got_recursive_update)
    cdk_window_schedule_update (NULL);
}
...
static void
cdk_window_process_updates_with_mode (CdkWindow     *window,
                                      int            recurse_mode)
{
  GPtrArray *list;
  int i;
  ...
  for (i = (int)list->len - 1; i >= 0; i --)
    {
      CdkWindow *impl_window = g_ptr_array_index (list, i);
...
static void
cdk_window_invalidate_rect_full (CdkWindow          *window,
				  const CdkRectangle *rect,
				  gboolean            invalidate_children)
{
 ...
 **/
void
cdk_window_invalidate_rect (CdkWindow          *window,
			    const CdkRectangle *rect,
			    gboolean            invalidate_children)
{
 ...
 **/
void
cdk_window_set_invalidate_handler (CdkWindow                      *window,
...
static void
draw_ugly_color (CdkWindow       *window,
		 const cairo_region_t *region,
                 int color)
{
  cairo_t *cr;
...
static void
impl_window_add_update_area (CdkWindow *impl_window,
			     cairo_region_t *region)
{
  if (impl_window->update_area)
...
static void
cdk_window_invalidate_maybe_recurse_full (CdkWindow            *window,
					  const cairo_region_t *region,
 ...
 */
static gboolean
has_visible_ancestor_in_impl (CdkWindow *window,
                              CdkWindow *ancestor,
                              CdkWindowChildFunc child_func,
                              gpointer user_data)
{
...
static void
invalidate_impl_subwindows (CdkWindow            *window,
			    const cairo_region_t *region,
			    CdkWindowChildFunc    child_func,
			    gpointer              user_data)
{
 ...
 **/
void
cdk_window_invalidate_maybe_recurse (CdkWindow            *window,
				     const cairo_region_t *region,
                                     CdkWindowChildFunc    child_func,
				     gpointer              user_data)
{
...
static void
cdk_window_invalidate_region_full (CdkWindow       *window,
				    const cairo_region_t *region,
				    gboolean         invalidate_children)
{
  cdk_window_invalidate_maybe_recurse_full (window, region,
					    invalidate_children ?
					    true_predicate : (gboolean (*) (CdkWindow *, gpointer))NULL,
				       NULL);
}
 ...
 **/
void
cdk_window_invalidate_region (CdkWindow       *window,
			      const cairo_region_t *region,
			      gboolean         invalidate_children)
{
 ...
 **/
void
_cdk_window_invalidate_for_expose (CdkWindow       *window,
				   cairo_region_t       *region)
{
  cdk_window_invalidate_maybe_recurse_full (window, region,
 ...
 **/
cairo_region_t *
cdk_window_get_update_area (CdkWindow *window)
{
  CdkWindow *impl_window;
 ...
 **/
void
_cdk_window_clear_update_area (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 **/
void
cdk_window_freeze_updates (CdkWindow *window)
{
  CdkWindow *impl_window;
 ...
 **/
void
cdk_window_thaw_updates (CdkWindow *window)
{
  CdkWindow *impl_window;
  ...
  if (--impl_window->update_freeze_count == 0)
    cdk_window_schedule_update (CDK_WINDOW (impl_window));
}
 ...
 */
void
cdk_window_freeze_toplevel_updates_libctk_only (CdkWindow *window)
{
  cdk_window_freeze_toplevel_updates (window);
}

void
cdk_window_freeze_toplevel_updates (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
  g_return_if_fail (window->window_type != CDK_WINDOW_CHILD);
 ...
 */
void
cdk_window_thaw_toplevel_updates_libctk_only (CdkWindow *window)
{
  cdk_window_thaw_toplevel_updates (window);
}

void
cdk_window_thaw_toplevel_updates (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
  g_return_if_fail (window->window_type != CDK_WINDOW_CHILD);
 ...
 */
void
cdk_window_constrain_size (CdkGeometry    *geometry,
 ...
 **/
CdkWindow *
cdk_window_get_device_position_double (CdkWindow       *window,
                                       CdkDevice       *device,
                                       double          *x,
                                       double          *y,
                                       CdkModifierType *mask)
{
  ...
  tmp_x = tmp_y = 0;
  tmp_mask = 0;
  normal_child = CDK_WINDOW_IMPL_GET_CLASS (window->impl)->get_device_state (window,
                                                                             device,
                                                                             &tmp_x, &tmp_y,
                                                                             &tmp_mask);
 ...
 **/
CdkWindow *
cdk_window_get_device_position (CdkWindow       *window,
                                CdkDevice       *device,
                                gint            *x,
                                gint            *y,
                                CdkModifierType *mask)
 ...
 **/
CdkWindow *
cdk_get_default_root_window (void)
{
  return cdk_screen_get_root_window (cdk_screen_get_default ());
}

static void
get_all_native_children (CdkWindow *window,
			 GList **native)
{
  CdkWindow *child;
  GList *l;
...
static gboolean
cdk_window_raise_internal (CdkWindow *window)
{
  CdkWindow *parent = window->parent;
  CdkWindow *above;
	      ...
	      gboolean val)
{
  ...
  if (cdk_window_has_impl (w)  &&
      ...
      !cdk_window_is_toplevel (w))
    {
      ...
      impl_class = CDK_WINDOW_IMPL_GET_CLASS (w->impl);
      if (val)
	impl_class->show ((CdkWindow *)w, FALSE);
      else
...
/* Returns TRUE If the native window was mapped or unmapped */
gboolean
_cdk_window_update_viewable (CdkWindow *window)
{
  gboolean viewable;
  ...
  if (window->window_type == CDK_WINDOW_FOREIGN ||
    ...
    viewable = CDK_WINDOW_IS_MAPPED (window);
  else
    viewable = FALSE;
...
static void
cdk_window_show_internal (CdkWindow *window, gboolean raise)
{
  CdkWindowImplClass *impl_class;
  ...
  if (!was_mapped || did_raise)
    {
      ...
      cdk_window_update_visibility_recursively (window, NULL);

      if (cdk_window_is_viewable (window))
	{
	  _cdk_synthesize_crossing_events_for_geometry_change (window);
	  cdk_window_invalidate_rect_full (window, NULL, TRUE);
	}
    }
}
 ...
 */
void
cdk_window_show_unraised (CdkWindow *window)
{
  cdk_window_show_internal (window, FALSE);
}
 ...
 */
void
cdk_window_raise (CdkWindow *window)
{
  gboolean did_raise;
  ...
  if (did_raise &&
      ...
      !window->input_only)
    cdk_window_invalidate_region_full (window, window->clip_region, TRUE);
}

static void
cdk_window_lower_internal (CdkWindow *window)
{
  CdkWindow *parent = window->parent;
  CdkWindowImplClass *impl_class;
  CdkWindow *above;
...
static void
cdk_window_invalidate_in_parent (CdkWindow *private)
{
  ...
  cdk_window_invalidate_rect_full (private->parent, &r, TRUE);
}
 ...
 */
void
cdk_window_lower (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 */
void
cdk_window_restack (CdkWindow     *window,
		    CdkWindow     *sibling,
		    gboolean       above)
{
  CdkWindowImplClass *impl_class;
  ...
  g_return_if_fail (CDK_IS_WINDOW (window));
  g_return_if_fail (sibling == NULL || CDK_IS_WINDOW (sibling));
  ...
  if (cdk_window_is_toplevel (window))
    {
      g_return_if_fail (cdk_window_is_toplevel (sibling));
      impl_class = CDK_WINDOW_IMPL_GET_CLASS (window->impl);
 ...
 */
void
cdk_window_show (CdkWindow *window)
{
  cdk_window_show_internal (window, TRUE);
}
 ...
 */
void
cdk_window_hide (CdkWindow *window)
{
  CdkWindowImplClass *impl_class;
      ...
      for (d = devices; d; d = d->next)
        {
          ...
          if (_cdk_display_end_device_grab (display,
                                            ...
                                            window,
                                            TRUE))
            cdk_device_ungrab (device, CDK_CURRENT_TIME);
        }
  ...
  if (was_mapped && !cdk_window_has_impl (window))
    {
      if (window->event_mask & CDK_STRUCTURE_MASK)
	_cdk_make_event (window, CDK_UNMAP, NULL, FALSE);
 ...
 **/
void
cdk_window_withdraw (CdkWindow *window)
{
  CdkWindowImplClass *impl_class;
  gboolean was_mapped;
  ...
  if (cdk_window_has_impl (window))
      ...
      if (was_mapped)
	{
      ...
      if (current_context != NULL && cdk_gl_context_get_window (current_context) == window)
 ...
 **/
void
cdk_window_set_events (CdkWindow       *window,
		       CdkEventMask     event_mask)
{
  CdkWindowImplClass *impl_class;
  CdkDisplay *display;
 ...
 **/
CdkEventMask
cdk_window_get_events (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), 0);
 ...
 **/
void
cdk_window_set_device_events (CdkWindow    *window,
                              CdkDevice    *device,
                              CdkEventMask  event_mask)
{
  ...
  CDK_DEVICE_GET_CLASS (device)->select_window_events (device, native, device_mask);
}
 ...
 **/
CdkEventMask
cdk_window_get_device_events (CdkWindow *window,
                              CdkDevice *device)
{
  CdkEventMask mask;
...
static void
cdk_window_move_resize_toplevel (CdkWindow *window,
                                 gboolean   with_move,
                                 gint       x,
                                 gint       y,
                                 gint       width,
  ...
  if (expose)
    {
      ...
      cdk_window_invalidate_region_full (window, new_region, TRUE);
...
static void
move_native_children (CdkWindow *private)
{
  GList *l;
  CdkWindow *child;
  CdkWindowImplClass *impl_class;
  ...
  for (l = private->children; l; l = l->next)
    {
      ...
      if (child->impl != private->impl)
	{
	  impl_class = CDK_WINDOW_IMPL_GET_CLASS (child->impl);
...
static void
cdk_window_move_resize_internal (CdkWindow *window,
				 gboolean   with_move,
				 gint       x,
				 gint       y,
				 gint       width,
  ...
  if (expose)
    {
      ...
      cdk_window_invalidate_region_full (window->parent, new_region, TRUE);
 ...
 **/
void
cdk_window_move (CdkWindow *window,
		 gint       x,
		 gint       y)
{
  cdk_window_move_resize_internal (window, TRUE, x, y, -1, -1);
}
 ...
 **/
void
cdk_window_resize (CdkWindow *window,
		   gint       width,
		   gint       height)
{
  cdk_window_move_resize_internal (window, FALSE, 0, 0, width, height);
}
 ...
 **/
void
cdk_window_move_resize (CdkWindow *window,
			gint       x,
			gint       y,
			gint       width,
			gint       height)
{
  cdk_window_move_resize_internal (window, TRUE, x, y, width, height);
}
 ...
 */
void
cdk_window_move_to_rect (CdkWindow          *window,
                         const CdkRectangle *rect,
 ...
 **/
void
cdk_window_scroll (CdkWindow *window,
		   gint       dx,
		   gint       dy)
{
  GList *tmp_list;
  ...
  while (tmp_list)
    {
      CdkWindow *child = CDK_WINDOW (tmp_list->data);
 ...
 */
void
cdk_window_move_region (CdkWindow            *window,
                        const cairo_region_t *region,
                        gint                  dx,
                        gint                  dy)
{
  ...
  cdk_window_invalidate_region_full (window, expose_area, FALSE);
...
static void
cdk_window_set_cursor_internal (CdkWindow *window,
                                CdkDevice *device,
                                CdkCursor *cursor)
{
  if (CDK_WINDOW_DESTROYED (window))
  ...
  g_assert (cdk_window_get_display (window) == cdk_device_get_display (device));
  g_assert (!cursor || cdk_window_get_display (window) == cdk_cursor_get_display (cursor));

  if (window->window_type == CDK_WINDOW_ROOT ||
      window->window_type == CDK_WINDOW_FOREIGN)
    CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_device_cursor (window, device, cursor);
  else
    {
      CdkPointerWindowInfo *pointer_info;
      CdkDisplay *display;
 ...
 */
CdkCursor *
cdk_window_get_cursor (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
 ...
 */
void
cdk_window_set_cursor (CdkWindow *window,
		       CdkCursor *cursor)
{
  CdkDisplay *display;
 ...
 **/
CdkCursor *
cdk_window_get_device_cursor (CdkWindow *window,
                              CdkDevice *device)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), NULL);
  g_return_val_if_fail (CDK_IS_DEVICE (device), NULL);
 ...
 **/
void
cdk_window_set_device_cursor (CdkWindow *window,
                              CdkDevice *device,
                              CdkCursor *cursor)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 */
void
cdk_window_get_geometry (CdkWindow *window,
			 gint      *x,
			 gint      *y,
			 gint      *width,
			 gint      *height)
 ...
 */
int
cdk_window_get_width (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), 0);
 ...
 */
int
cdk_window_get_height (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), 0);
 ...
 */
gint
cdk_window_get_origin (CdkWindow *window,
		       gint      *x,
		       gint      *y)
{
 ...
 */
void
cdk_window_get_root_coords (CdkWindow *window,
			    gint       x,
			    gint       y,
			    gint      *root_x,
			    gint      *root_y)
 ...
 **/
void
cdk_window_coords_to_parent (CdkWindow *window,
                             gdouble    x,
                             gdouble    y,
                             gdouble   *parent_x,
                             gdouble   *parent_y)
 ...
 **/
void
cdk_window_coords_from_parent (CdkWindow *window,
                               gdouble    parent_x,
                               gdouble    parent_y,
                               gdouble   *x,
                               gdouble   *y)
 ...
 */
void
cdk_window_shape_combine_region (CdkWindow       *window,
  ...
  if (old_region)
    {
      ...
      cdk_window_invalidate_region_full (window, diff, TRUE);
      ...
      if (!cdk_window_is_toplevel (window))
	{
	  ...
	  cdk_window_invalidate_region_full (window->parent, diff, TRUE);
...
static void
do_child_shapes (CdkWindow *window,
		 gboolean merge)
{
  CdkRectangle r;
  cairo_region_t *region;
 ...
 **/
void
cdk_window_set_child_shapes (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 */
void
cdk_window_merge_child_shapes (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 */
void
cdk_window_input_shape_combine_region (CdkWindow       *window,
				       const cairo_region_t *shape_region,
				       gint             offset_x,
				       gint             offset_y)
{
...
static void
do_child_input_shapes (CdkWindow *window,
		       gboolean merge)
{
  CdkRectangle r;
  cairo_region_t *region;
 ...
 **/
void
cdk_window_set_child_input_shapes (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 **/
void
cdk_window_set_pass_through (CdkWindow *window,
 ...
 **/
gboolean
cdk_window_get_pass_through (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
 ...
 **/
void
cdk_window_merge_child_input_shapes (CdkWindow *window)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
 ...
 */
gboolean
cdk_window_get_modal_hint (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
 ...
 */
gboolean
cdk_window_get_accept_focus (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
 ...
 */
gboolean
cdk_window_get_focus_on_map (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
 ...
 */
gboolean
cdk_window_is_input_only (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
 ...
 */
gboolean
cdk_window_is_shaped (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
...
static CdkWindow *
get_event_parent (CdkWindow *window)
{
  if (cdk_window_is_offscreen (window))
...
/* Gets the toplevel for a window as used for events,
   ...
   toplevel */
static CdkWindow *
get_event_toplevel (CdkWindow *window)
{
  CdkWindow *parent;
...
gboolean
_cdk_window_event_parent_of (CdkWindow *parent,
 	  	             CdkWindow *child)
{
  CdkWindow *w;
...
static void
update_cursor (CdkDisplay *display,
               CdkDevice  *device)
{
  ...
  impl_class = CDK_WINDOW_IMPL_GET_CLASS (toplevel->impl);
...
static gboolean
point_in_input_window (CdkWindow *window,
		       gdouble    x,
		       gdouble    y,
...
static CdkWindow *
convert_native_coords_to_toplevel (CdkWindow *window,
				   gdouble    child_x,
                                   gdouble    child_y,
				   gdouble   *toplevel_x,
                                   gdouble   *toplevel_y)
...
static CdkWindow *
pick_embedded_child (CdkWindow *window,
		     gdouble    x,
                     gdouble    y)
{
  CdkWindow *res;
...
CdkWindow *
_cdk_window_find_child_at (CdkWindow *window,
			   double     x,
                           double     y)
{
  CdkWindow *sub;
...
CdkWindow *
_cdk_window_find_descendant_at (CdkWindow *window,
				gdouble    x,
                                gdouble    y,
				gdouble   *found_x,
				gdouble   *found_y)
 ...
 **/
void
cdk_window_beep (CdkWindow *window)
{
  CdkDisplay *display;
  CdkWindow *toplevel;
  ...
  if (toplevel)
    {
      if (CDK_WINDOW_IMPL_GET_CLASS (toplevel->impl)->beep (toplevel))
        return;
    }
 ...
 **/
void
cdk_window_set_support_multidevice (CdkWindow *window,
 ...
 **/
gboolean
cdk_window_get_support_multidevice (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
...
G_STATIC_ASSERT (G_N_ELEMENTS (type_masks) == CDK_EVENT_LAST);
...
static gboolean
is_button_type (CdkEventType type)
{
...
static gboolean
is_gesture_type (CdkEventType type)
{
...
static gboolean
is_motion_type (CdkEventType type)
{
...
static gboolean
is_touch_type (CdkEventType type)
{
...
CdkEvent *
_cdk_make_event (CdkWindow    *window,
		 CdkEventType  type,
		     ...
		     gulong                      serial)
{
  ...
  CdkDeviceGrabInfo *grab;
  CdkTouchGrabInfo *touch_grab = NULL;
  CdkPointerWindowInfo *pointer_info;
  ...
  if (window_event_mask & type_event_mask)
    {
      event = _cdk_make_event ((CdkWindow *)window, type, event_in_queue, TRUE);
 ...
 */
void
_cdk_synthesize_crossing_events (CdkDisplay                 *display,
				 CdkWindow                  *src,
				 CdkWindow                  *dest,
                                 CdkDevice                  *device,
                                 CdkDevice                  *source_device,
				 ...
				 gboolean                    non_linear)
{
  ...
  a = (src && CDK_IS_WINDOW (src)) ? src : NULL;
  b = (dest && CDK_IS_WINDOW (dest)) ? dest : NULL;
 ...
 */
static CdkWindow *
get_pointer_window (CdkDisplay *display,
...
void
_cdk_display_set_window_under_pointer (CdkDisplay *display,
                                       CdkDevice  *device,
				       CdkWindow  *window)
{
		  ...
		  guint32	  time)
{
  ...
  g_return_val_if_fail (window != NULL, CDK_GRAB_FAILED);
  g_return_val_if_fail (CDK_IS_WINDOW (window), CDK_GRAB_FAILED);
  g_return_val_if_fail (confine_to == NULL || CDK_IS_WINDOW (confine_to), CDK_GRAB_FAILED);
 ...
 */
void
cdk_window_geometry_changed (CdkWindow *window)
{
  _cdk_synthesize_crossing_events_for_geometry_change (window);
}
 ...
 **/
void
cdk_window_set_source_events (CdkWindow      *window,
                              CdkInputSource  source,
                              CdkEventMask    event_mask)
{
  CdkDeviceManager *device_manager;
 ...
 **/
CdkEventMask
cdk_window_get_source_events (CdkWindow      *window,
                              CdkInputSource  source)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), 0);
...
do_synthesize_crossing_event (gpointer data)
{
  ...
  if (CDK_WINDOW_DESTROYED (changed_toplevel))
    return FALSE;
...
void
_cdk_synthesize_crossing_events_for_geometry_change (CdkWindow *changed_window)
{
  CdkWindow *toplevel;
  ...
  if (toplevel->synthesized_crossing_event_id == 0)
    {
      toplevel->synthesized_crossing_event_id =
        cdk_threads_add_idle_full (CDK_PRIORITY_EVENTS - 1,
...
static CdkWindow *
get_event_window (CdkDisplay                 *display,
                  CdkDevice                  *device,
                  CdkEventSequence           *sequence,
                  CdkWindow                  *pointer_window,
                  CdkEventType                type,
...
static gboolean
proxy_pointer_event (CdkDisplay                 *display,
...
static gboolean
proxy_button_event (CdkEvent *source_event,
		    gulong serial)
{
  CdkWindow *toplevel_window, *event_window;
  CdkWindow *event_win;
  ...
  event = _cdk_make_event (event_win, type, source_event, FALSE);
...
static gboolean
proxy_gesture_event (CdkEvent *source_event,
                     gulong    serial)
{
  ...
  event = _cdk_make_event (event_win, evtype, source_event, FALSE);
  cdk_event_set_device (event, device);
  cdk_event_set_source_device (event, source_device);
  cdk_event_set_seat (event, cdk_device_get_seat (device));
...
static void
cdk_window_print (CdkWindow *window,
		  int indent)
{
  char *s;
...
static void
cdk_window_print_tree (CdkWindow *window,
		       int indent,
...
void
_cdk_windowing_got_event (CdkDisplay *display,
                          GList      *event_link,
                          CdkEvent   *event,
                          gulong      serial)
{
  ...
  CdkPointerWindowInfo *pointer_info = NULL;
  CdkDevice *device, *source_device;
      ...
      if (event->type == CDK_BUTTON_RELEASE ||
          cdk_event_get_pointer_emulated (event))
        {
          ...
          if (button_release_grab &&
              ...
              (event->button.state & CDK_ANY_BUTTON_MASK & ~(CDK_BUTTON1_MASK << (event->button.button - 1))) == 0)
            {
 ...
 **/
void
cdk_window_focus (CdkWindow *window,
                  guint32    timestamp)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->focus (window, timestamp);
}
 ...
 **/
void
cdk_window_set_type_hint (CdkWindow        *window,
			  CdkWindowTypeHint hint)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_type_hint (window, hint);
}
 ...
 **/
CdkWindowTypeHint
cdk_window_get_type_hint (CdkWindow *window)
{
  return CDK_WINDOW_IMPL_GET_CLASS (window->impl)->get_type_hint (window);
}
 ...
 **/
void
cdk_window_set_modal_hint (CdkWindow *window,
			   gboolean   modal)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_modal_hint (window, modal);
}
 ...
 **/
void
cdk_window_set_skip_taskbar_hint (CdkWindow *window,
                                  gboolean   skips_taskbar)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_skip_taskbar_hint (window, skips_taskbar);
}
 ...
 **/
void
cdk_window_set_skip_pager_hint (CdkWindow *window,
                                gboolean   skips_pager)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_skip_pager_hint (window, skips_pager);
}
 ...
 **/
void
cdk_window_set_urgency_hint (CdkWindow *window,
			     gboolean   urgent)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_urgency_hint (window, urgent);
}
 ...
 **/
void
cdk_window_set_geometry_hints (CdkWindow         *window,
			       const CdkGeometry *geometry,
			       CdkWindowHints     geom_mask)
{
  g_return_if_fail (geometry != NULL || geom_mask == 0);

  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_geometry_hints (window, geometry, geom_mask);
}
 ...
 **/
void
cdk_window_set_title (CdkWindow   *window,
		      const gchar *title)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_title (window, title);
}
 ...
 **/
void
cdk_window_set_role (CdkWindow   *window,
		     const gchar *role)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_role (window, role);
}
 ...
 **/
void
cdk_window_set_startup_id (CdkWindow   *window,
			   const gchar *startup_id)
{
  CdkWindowImplClass *klass = CDK_WINDOW_IMPL_GET_CLASS (window->impl);
 ...
 **/
void
cdk_window_set_transient_for (CdkWindow *window,
			      CdkWindow *parent)
{
  window->transient_for = parent;

  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_transient_for (window, parent);
}
 ...
 **/
void
cdk_window_get_root_origin (CdkWindow *window,
			    gint      *x,
			    gint      *y)
{
  CdkRectangle rect;
 ...
 **/
void
cdk_window_get_frame_extents (CdkWindow    *window,
                              CdkRectangle *rect)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->get_frame_extents (window, rect);
}
 ...
 **/
void
cdk_window_set_override_redirect (CdkWindow *window,
				  gboolean override_redirect)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_override_redirect (window, override_redirect);
}
 ...
 **/
void
cdk_window_set_accept_focus (CdkWindow *window,
			     gboolean accept_focus)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_accept_focus (window, accept_focus);
}
 ...
 **/
void
cdk_window_set_focus_on_map (CdkWindow *window,
			     gboolean focus_on_map)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_focus_on_map (window, focus_on_map);
}
 ...
 **/
void
cdk_window_set_event_compression (CdkWindow *window,
 ...
 **/
gboolean
cdk_window_get_event_compression (CdkWindow *window)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), TRUE);
 ...
 */
void
cdk_window_set_icon_list (CdkWindow *window,
			  GList     *pixbufs)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_icon_list (window, pixbufs);
}
 ...
 **/
void
cdk_window_set_icon_name (CdkWindow   *window,
			  const gchar *name)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_icon_name (window, name);
}
 ...
 **/
void
cdk_window_iconify (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->iconify (window);
}
 ...
 **/
void
cdk_window_deiconify (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->deiconify (window);
}
 ...
 **/
void
cdk_window_stick (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->stick (window);
}
 ...
 **/
void
cdk_window_unstick (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->unstick (window);
}
 ...
 **/
void
cdk_window_maximize (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->maximize (window);
}
 ...
 **/
void
cdk_window_unmaximize (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->unmaximize (window);
}
 ...
 **/
void
cdk_window_fullscreen (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->fullscreen (window);
}
 ...
 **/
void
cdk_window_fullscreen_on_monitor (CdkWindow      *window,
                                  gint            monitor)
{
  CdkDisplay *display = cdk_window_get_display (window);

  g_return_if_fail (monitor >= 0);
  g_return_if_fail (monitor < cdk_display_get_n_monitors (display));

  if (CDK_WINDOW_IMPL_GET_CLASS (window->impl)->fullscreen_on_monitor != NULL)
    CDK_WINDOW_IMPL_GET_CLASS (window->impl)->fullscreen_on_monitor (window, monitor);
  else
    CDK_WINDOW_IMPL_GET_CLASS (window->impl)->fullscreen (window);
}
 ...
 **/
void
cdk_window_set_fullscreen_mode (CdkWindow        *window,
...
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), CDK_FULLSCREEN_ON_CURRENT_MONITOR);
 ...
 **/
void
cdk_window_unfullscreen (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->unfullscreen (window);
}
 ...
 **/
void
cdk_window_set_keep_above (CdkWindow *window,
                           gboolean   setting)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_keep_above (window, setting);
}
 ...
 **/
void
cdk_window_set_keep_below (CdkWindow *window, gboolean setting)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_keep_below (window, setting);
}
 ...
 **/
CdkWindow *
cdk_window_get_group (CdkWindow *window)
{
  return CDK_WINDOW_IMPL_GET_CLASS (window->impl)->get_group (window);
}
 ...
 **/
void
cdk_window_set_group (CdkWindow *window,
		      CdkWindow *leader)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_group (window, leader);
}
 ...
 **/
void
cdk_window_set_decorations (CdkWindow      *window,
			    CdkWMDecoration decorations)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_decorations (window, decorations);
}
 ...
 **/
gboolean
cdk_window_get_decorations(CdkWindow       *window,
			   CdkWMDecoration *decorations)
{
  return CDK_WINDOW_IMPL_GET_CLASS (window->impl)->get_decorations (window, decorations);
}
 ...
 **/
void
cdk_window_set_functions (CdkWindow    *window,
			  CdkWMFunction functions)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_functions (window, functions);
}
 ...
 */
void
cdk_window_begin_resize_drag_for_device (CdkWindow     *window,
                                         CdkWindowEdge  edge,
                                         CdkDevice     *device,
                                         gint           button,
                                         gint           root_x,
                                         ...
                                         guint32        timestamp)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->begin_resize_drag (window, edge, device, button, root_x, root_y, timestamp);
}
 ...
 */
void
cdk_window_begin_resize_drag (CdkWindow     *window,
                              CdkWindowEdge  edge,
                              gint           button,
                              gint           root_x,
                              gint           root_y,
 ...
 */
void
cdk_window_begin_move_drag_for_device (CdkWindow *window,
                                       CdkDevice *device,
                                       gint       button,
                                       gint       root_x,
                                       gint       root_y,
                                       guint32    timestamp)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->begin_move_drag (window,
                                                             device, button, root_x, root_y, timestamp);
}
 ...
 */
void
cdk_window_begin_move_drag (CdkWindow *window,
                            gint       button,
                            gint       root_x,
                            gint       root_y,
                            guint32    timestamp)
 ...
 */
void
cdk_window_set_opacity (CdkWindow *window,
			gdouble    opacity)
{
  if (opacity < 0)
    opacity = 0;
  ...
  if (cdk_window_has_impl (window))
    CDK_WINDOW_IMPL_GET_CLASS (window->impl)->set_opacity (window, opacity);
  else
    {
      recompute_visible_regions (window, FALSE);
      cdk_window_invalidate_rect_full (window, NULL, TRUE);
 ...
 */
void
cdk_window_destroy_notify (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->destroy_notify (window);
}
 ...
 */
void
cdk_window_register_dnd (CdkWindow *window)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->register_dnd (window);
}
...
cdk_window_get_drag_protocol (CdkWindow  *window,
                              CdkWindow **target)
{
  g_return_val_if_fail (CDK_IS_WINDOW (window), CDK_DRAG_PROTO_NONE);

  return CDK_WINDOW_IMPL_GET_CLASS (window->impl)->get_drag_protocol (window, target);
}
...
cdk_drag_begin_from_point (CdkWindow *window,
                           ...
                           gint       y_root)
{
  return CDK_WINDOW_IMPL_GET_CLASS (window->impl)->drag_begin (window, device, targets, x_root, y_root);
}
...
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->sync_rendering (window);
}
...
{
  return CDK_WINDOW_IMPL_GET_CLASS (window->impl)
 ...
 * recommended.
 *
* Also, cdk_test_simulate_button() is a fairly low level function,
 ...
 */
gboolean
cdk_property_get (CdkWindow  *window,
                  CdkAtom     property,
                  CdkAtom     type,
                  gulong      offset,
                  gulong      length,
 ...
 */
void
cdk_property_change (CdkWindow    *window,
                     CdkAtom       property,
                     CdkAtom       type,
                     gint          format,
                     CdkPropMode   mode,
                     ...
                     gint          nelements)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)
 ...
 */
void
cdk_property_delete (CdkWindow *window,
                     CdkAtom    property)
{
  CDK_WINDOW_IMPL_GET_CLASS (window->impl)->delete_property (window, property);
}

static void
cdk_window_flush_events (CdkFrameClock *clock,
                         void          *data)
{
  CdkWindow *window;
  CdkDisplay *display;

  window = CDK_WINDOW (data);
...
static void
cdk_window_set_frame_clock (CdkWindow     *window,
                            CdkFrameClock *clock)
{
  g_return_if_fail (CDK_IS_WINDOW (window));
  g_return_if_fail (clock == NULL || CDK_IS_FRAME_CLOCK (clock));
  g_return_if_fail (clock == NULL || cdk_window_is_toplevel (window));
  ...
  if (clock)
    {
      ...
      g_signal_connect (G_OBJECT (clock),
                        "flush-events",
                        G_CALLBACK (cdk_window_flush_events),
                        window);
      g_signal_connect (G_OBJECT (clock),
                        "paint",
                        G_CALLBACK (cdk_window_paint_on_clock),
                        window);
      g_signal_connect (G_OBJECT (clock),
                        "resume-events",
                        G_CALLBACK (cdk_window_resume_events),
                        window);
    }
  ...
  if (window->frame_clock)
    {
      if (window->frame_clock_events_paused)
        cdk_window_resume_events (window->frame_clock, G_OBJECT (window));
 ...
 */
CdkFrameClock*
cdk_window_get_frame_clock (CdkWindow *window)
{
  CdkWindow *toplevel;
 ...
 */
gint
cdk_window_get_scale_factor (CdkWindow *window)
{
  CdkWindowImplClass *impl_class;
...
void
cdk_window_get_unscaled_size (CdkWindow *window,
                              int *unscaled_width,
                              int *unscaled_height)
{
  CdkWindowImplClass *impl_class;
 ...
 */
void
cdk_window_set_opaque_region (CdkWindow      *window,
                              cairo_region_t *region)
{
  CdkWindowImplClass *impl_class;

  g_return_if_fail (CDK_IS_WINDOW (window));
  g_return_if_fail (!CDK_WINDOW_DESTROYED (window));
 ...
 */
void
cdk_window_set_shadow_width (CdkWindow *window,
                             gint       left,
                             gint       right,
                             gint       top,
                             gint       bottom)
 ...
 */
gboolean
cdk_window_show_window_menu (CdkWindow *window,
                             CdkEvent  *event)
{
  CdkWindowImplClass *impl_class;

  g_return_val_if_fail (CDK_IS_WINDOW (window), FALSE);
  g_return_val_if_fail (!CDK_WINDOW_DESTROYED (window), FALSE);
