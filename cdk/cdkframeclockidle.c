...
struct _CdkFrameClockIdlePrivate
{
  ...
  SmoothDeltaState smooth_phase_state; /* The state of smoothed_frame_time_phase - is it valid, awaiting vsync etc. Thanks to zero-init, the initial value
                                          ...
                                          the initial value of smooth_phase_state is SMOOTH_PHASE_STATE_VALID. See the comment in cdk_frame_clock_paint_idle()
                                          for details. */
...
cdk_frame_clock_idle_dispose (GObject *object)
{
  ...
  G_OBJECT_CLASS (cdk_frame_clock_idle_parent_class)->dispose (object);
}
      ...
      if (priv->flush_idle_id == 0 && RUN_FLUSH_IDLE (priv))
        {
          ...
          g_source_set_name_by_id (priv->flush_idle_id, "[ctk+] cdk_frame_clock_flush_idle");
        }
	  ...
	  priv->paint_idle_id == 0 && RUN_PAINT_IDLE (priv))
        {
          ...
          priv->paint_idle_id = cdk_threads_add_timeout_full (GDK_PRIORITY_REDRAW,
                                                              ...
                                                              g_object_ref (clock_idle),
                                                              (GDestroyNotify) g_object_unref);
          g_source_set_name_by_id (priv->paint_idle_id, "[ctk+] cdk_frame_clock_paint_idle");
        }
    }
}
          ...
          if (priv->freeze_count == 0)
            {
              ...
              _cdk_frame_clock_begin_frame (clock);
          ...
          if (priv->freeze_time != 0)
            {
              ...
              cdk_profiler_add_mark (priv->freeze_time * 1000,
...
cdk_frame_clock_idle_class_init (CdkFrameClockIdleClass *klass)
{
  ...
  frame_clock_class->get_frame_time = cdk_frame_clock_idle_get_frame_time;
  frame_clock_class->request_phase = cdk_frame_clock_idle_request_phase;
  frame_clock_class->begin_updating = cdk_frame_clock_idle_begin_updating;
  frame_clock_class->end_updating = cdk_frame_clock_idle_end_updating;
  frame_clock_class->freeze = cdk_frame_clock_idle_freeze;
