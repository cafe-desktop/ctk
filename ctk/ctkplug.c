...
static void            xembed_set_info                (CdkWindow        *window,
				                       unsigned long     flags);
 ...
 * @plug: a #CtkPlug.
 * @display: the #CdkDisplay associated with @socket_id’s 
 *	     #CtkSocket.
 * @socket_id: the XID of the socket’s window.
 *
 * Finish the initialization of @plug for a given #CtkSocket identified by
 ...
 * @display: the #CdkDisplay on which @socket_id is displayed
 * @socket_id: the XID of the socket’s window.
 * 
...
static void
xembed_set_info (CdkWindow     *window,
		 unsigned long  flags)
{
...
#ifdef CTK_HAVE_ATK_PLUG_SET_CHILD
static void
_ctk_plug_accessible_embed_set_info (CtkWidget *widget, CdkWindow *window)
{
  CdkDisplay *display = cdk_window_get_display (window);
...
handle_xembed_message (CtkPlug           *plug,
		       ...
		       glong              detail,
		       glong              data1 G_GNUC_UNUSED,
		       glong              data2 G_GNUC_UNUSED,
...
ctk_plug_filter_func (CdkXEvent *cdk_xevent,
		      CdkEvent  *event,
		      gpointer   data)
{
  CdkScreen *screen = cdk_window_get_screen (event->any.window);
  ...
  switch (xevent->type)
    ...
    case KeyRelease:
      {
        CdkModifierType state, consumed;
        CdkDevice *keyboard;
        CdkKeymap *keymap;
        CdkSeat *seat;
...
static gboolean
ctk_plug_focus_event (CtkWidget      *widget G_GNUC_UNUSED,
...
static void
add_grabbed_key (gpointer key,
		 gpointer val G_GNUC_UNUSED,
		 gpointer data)
{
  GrabbedKey *grabbed_key = key;
  CtkPlug *plug = data;
...
static void
remove_grabbed_key (gpointer key,
		    gpointer val G_GNUC_UNUSED,
		    gpointer data)
{
  GrabbedKey *grabbed_key = key;
...
static void
keys_foreach (CtkWindow      *window G_GNUC_UNUSED,
	      guint           keyval,
	      CdkModifierType modifiers,
	      gboolean        is_mnemonic G_GNUC_UNUSED,
	      gpointer        data)
{
