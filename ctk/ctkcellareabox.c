 ...
 * @Short_Description: A cell area that renders CtkCellRenderers
 *     into a row or a column
 * @Title: CtkCellAreaBox
 *
 * The #CtkCellAreaBox renders cell renderers into a row or a column
 * depending on its #CtkOrientation.
 *
 * CtkCellAreaBox uses a notion of packing. Packing
 * refers to adding cell renderers with reference to a particular position
 * in a #CtkCellAreaBox. There are two reference positions: the
 * start and the end of the box.
 * When the #CtkCellAreaBox is oriented in the %CTK_ORIENTATION_VERTICAL
 * orientation, the start is defined as the top of the box and the end is
 * defined as the bottom. In the %CTK_ORIENTATION_HORIZONTAL orientation
 * start is defined as the left side and the end is defined as the right
 * side.
 *
 * Alignments of #CtkCellRenderers rendered in adjacent rows can be
 * configured by configuring the #CtkCellAreaBox align child cell property
 * with ctk_cell_area_cell_set_property() or by specifying the "align"
 * argument to ctk_cell_area_box_pack_start() and ctk_cell_area_box_pack_end().
 */
...
/* GObjectClass */
static void      ctk_cell_area_box_finalize                       (GObject              *object);
static void      ctk_cell_area_box_dispose                        (GObject              *object);
static void      ctk_cell_area_box_set_property                   (GObject              *object,
                                                                   guint                 prop_id,
                                                                   const GValue         *value,
                                                                   GParamSpec           *pspec);
static void      ctk_cell_area_box_get_property                   (GObject              *object,
                                                                   guint                 prop_id,
                                                                   GValue               *value,
                                                                   GParamSpec           *pspec);

/* CtkCellAreaClass */
static void      ctk_cell_area_box_add                            (CtkCellArea          *area,
                                                                   CtkCellRenderer      *renderer);
static void      ctk_cell_area_box_remove                         (CtkCellArea          *area,
                                                                   CtkCellRenderer      *renderer);
static void      ctk_cell_area_box_foreach                        (CtkCellArea          *area,
...
static void      ctk_cell_area_box_set_cell_property              (CtkCellArea          *area,
                                                                   CtkCellRenderer      *renderer,
                                                                   guint                 prop_id,
                                                                   const GValue         *value,
                                                                   GParamSpec           *pspec);
static void      ctk_cell_area_box_get_cell_property              (CtkCellArea          *area,
                                                                   CtkCellRenderer      *renderer,
                                                                   guint                 prop_id,
                                                                   GValue               *value,
                                                                   GParamSpec           *pspec);
static CtkCellAreaContext *ctk_cell_area_box_create_context       (CtkCellArea          *area);
static CtkCellAreaContext *ctk_cell_area_box_copy_context         (CtkCellArea          *area,
                                                                   CtkCellAreaContext   *context);
static CtkSizeRequestMode  ctk_cell_area_box_get_request_mode     (CtkCellArea          *area);
static void      ctk_cell_area_box_get_preferred_width            (CtkCellArea          *area,
                                                                   CtkCellAreaContext   *context,
                                                                   CtkWidget            *widget,
                                                                   gint                 *minimum_width,
                                                                   gint                 *natural_width);
static void      ctk_cell_area_box_get_preferred_height           (CtkCellArea          *area,
                                                                   CtkCellAreaContext   *context,
                                                                   CtkWidget            *widget,
                                                                   gint                 *minimum_height,
                                                                   gint                 *natural_height);
static void      ctk_cell_area_box_get_preferred_height_for_width (CtkCellArea          *area,
                                                                   CtkCellAreaContext   *context,
                                                                   CtkWidget            *widget,
                                                                   gint                  width,
                                                                   gint                 *minimum_height,
                                                                   gint                 *natural_height);
static void      ctk_cell_area_box_get_preferred_width_for_height (CtkCellArea          *area,
                                                                   CtkCellAreaContext   *context,
                                                                   CtkWidget            *widget,
                                                                   gint                  height,
                                                                   gint                 *minimum_width,
                                                                   gint                 *natural_width);
static gboolean  ctk_cell_area_box_focus                          (CtkCellArea          *area,
                                                                   CtkDirectionType      direction);

/* CtkCellLayoutIface */
static void      ctk_cell_area_box_cell_layout_init               (CtkCellLayoutIface *iface);
static void      ctk_cell_area_box_layout_pack_start              (CtkCellLayout      *cell_layout,
                                                                   CtkCellRenderer    *renderer,
                                                                   gboolean            expand);
static void      ctk_cell_area_box_layout_pack_end                (CtkCellLayout      *cell_layout,
                                                                   CtkCellRenderer    *renderer,
                                                                   gboolean            expand);
static void      ctk_cell_area_box_layout_reorder                 (CtkCellLayout      *cell_layout,
                                                                   CtkCellRenderer    *renderer,
                                                                   gint                position);
static void      ctk_cell_area_box_focus_changed                  (CtkCellArea        *area,
                                                                   GParamSpec         *pspec,
                                                                   CtkCellAreaBox     *box);
...
static CellInfo      *cell_info_new          (CtkCellRenderer       *renderer,
                                              CtkPackType            pack,
                                              gboolean               expand,
                                              gboolean               align,
					      gboolean               fixed);
...
static AllocatedCell *allocated_cell_new     (CtkCellRenderer       *renderer,
                                              gint                   position,
                                              gint                   size);
static void           allocated_cell_free    (AllocatedCell         *cell);
static GList         *list_consecutive_cells (CtkCellAreaBox        *box);
static gint           count_expand_groups    (CtkCellAreaBox        *box);
static void           context_weak_notify    (CtkCellAreaBox        *box,
                                              CtkCellAreaBoxContext *dead_context);
static void           reset_contexts         (CtkCellAreaBox        *box);
static void           init_context_groups    (CtkCellAreaBox        *box);
static void           init_context_group     (CtkCellAreaBox        *box,
                                              CtkCellAreaBoxContext *context);
static GSList        *get_allocated_cells    (CtkCellAreaBox        *box,
                                              CtkCellAreaBoxContext *context,
                                              CtkWidget             *widget,
                                              gint                   width,
                                              gint                   height);
...
struct _CtkCellAreaBoxPrivate
{
   ...
   */
  CtkCellRenderer *last_focus_cell;
...
G_DEFINE_TYPE_WITH_CODE (CtkCellAreaBox, ctk_cell_area_box, CTK_TYPE_CELL_AREA,
                         G_ADD_PRIVATE (CtkCellAreaBox)
                         G_IMPLEMENT_INTERFACE (CTK_TYPE_CELL_LAYOUT,
                                                ctk_cell_area_box_cell_layout_init)
                         G_IMPLEMENT_INTERFACE (CTK_TYPE_ORIENTABLE, NULL))

static void
ctk_cell_area_box_init (CtkCellAreaBox *box)
{
  CtkCellAreaBoxPrivate *priv;

  box->priv = ctk_cell_area_box_get_instance_private (box);
  priv = box->priv;
  ...
  priv->focus_cell_id = g_signal_connect (box, "notify::focus-cell",
                                          G_CALLBACK (ctk_cell_area_box_focus_changed), box);
}

static void
ctk_cell_area_box_class_init (CtkCellAreaBoxClass *class)
{
  GObjectClass     *object_class = G_OBJECT_CLASS (class);
  CtkCellAreaClass *area_class   = CTK_CELL_AREA_CLASS (class);

  /* GObjectClass */
  object_class->finalize     = ctk_cell_area_box_finalize;
  object_class->dispose      = ctk_cell_area_box_dispose;
  object_class->set_property = ctk_cell_area_box_set_property;
  object_class->get_property = ctk_cell_area_box_get_property;

  /* CtkCellAreaClass */
  area_class->add                 = ctk_cell_area_box_add;
  area_class->remove              = ctk_cell_area_box_remove;
  area_class->foreach             = ctk_cell_area_box_foreach;
  area_class->foreach_alloc       = ctk_cell_area_box_foreach_alloc;
  area_class->apply_attributes    = ctk_cell_area_box_apply_attributes;
  area_class->set_cell_property   = ctk_cell_area_box_set_cell_property;
  area_class->get_cell_property   = ctk_cell_area_box_get_cell_property;

  area_class->create_context                 = ctk_cell_area_box_create_context;
  area_class->copy_context                   = ctk_cell_area_box_copy_context;
  area_class->get_request_mode               = ctk_cell_area_box_get_request_mode;
  area_class->get_preferred_width            = ctk_cell_area_box_get_preferred_width;
  area_class->get_preferred_height           = ctk_cell_area_box_get_preferred_height;
  area_class->get_preferred_height_for_width = ctk_cell_area_box_get_preferred_height_for_width;
  area_class->get_preferred_width_for_height = ctk_cell_area_box_get_preferred_width_for_height;

  area_class->focus = ctk_cell_area_box_focus;

  /* Properties */
  g_object_class_override_property (object_class, PROP_ORIENTATION, "orientation");

  /**
   * CtkCellAreaBox:spacing:
   *
   ...
   * Since: 3.0
   */
  ctk_cell_area_class_install_cell_property (area_class,
                                             CELL_PROP_EXPAND,
  ...
  /**
   * CtkCellAreaBox:pack-type:
   *
   * A CtkPackType indicating whether the cell renderer is packed
  ...
  ctk_cell_area_class_install_cell_property (area_class,
                                             ...
                                             ("pack-type",
                                              P_("Pack Type"),
                                              P_("A CtkPackType indicating whether the cell is packed with "
 ...
 *************************************************************/
static CellInfo *
cell_info_new  (CtkCellRenderer *renderer,
                CtkPackType      pack,
                gboolean         expand,
                gboolean         align,
		gboolean         fixed)
...
allocated_cell_new (CtkCellRenderer *renderer,
                    gint             position,
                    gint             size)
{
...
static GList *
list_consecutive_cells (CtkCellAreaBox *box)
{
...
static void
cell_groups_clear (CtkCellAreaBox *box)
{
  CtkCellAreaBoxPrivate *priv = box->priv;
  gint                   i;
...
static void
cell_groups_rebuild (CtkCellAreaBox *box)
{
  CtkCellAreaBoxPrivate *priv = box->priv;
  CellGroup              group = { 0, };
  CellGroup             *group_ptr;
  ...
  for (l = group->cells; l; l = l->next)
    {
      ...
      if (ctk_cell_renderer_get_visible (info->renderer))
        {
          visible_cells++;
...
static gint
count_expand_groups (CtkCellAreaBox  *box)
{
  CtkCellAreaBoxPrivate *priv = box->priv;
  gint                   i;
...
static void
context_weak_notify (CtkCellAreaBox        *box,
                     CtkCellAreaBoxContext *dead_context)
{
  CtkCellAreaBoxPrivate *priv = box->priv;
...
static void
init_context_group (CtkCellAreaBox        *box,
                    CtkCellAreaBoxContext *context)
{
  CtkCellAreaBoxPrivate *priv = box->priv;
  ...
  _ctk_cell_area_box_init_groups (context, priv->groups->len, expand_groups, align_groups);
  g_free (expand_groups);
  g_free (align_groups);
}

static void
init_context_groups (CtkCellAreaBox *box)
{
  CtkCellAreaBoxPrivate *priv = box->priv;
  GSList                *l;
  ...
  for (l = priv->contexts; l; l = l->next)
    {
      CtkCellAreaBoxContext *context = l->data;
...
static void
reset_contexts (CtkCellAreaBox *box)
{
  CtkCellAreaBoxPrivate *priv = box->priv;
  GSList                *l;
  ...
  for (l = priv->contexts; l; l = l->next)
    {
      CtkCellAreaContext *context = l->data;

      ctk_cell_area_context_reset (context);
    }
}
 ...
 */
static GSList *
allocate_cells_manually (CtkCellAreaBox        *box,
                         CtkWidget             *widget,
                         gint                   width,
                         gint                   height)
{
  ...
  sizes = g_new0 (CtkRequestedSize, nvisible);
  for (l = cells, i = 0; l; l = l->next)
    {
      ...
      ctk_cell_area_request_renderer (CTK_CELL_AREA (box), info->renderer,
                                      priv->orientation,
                                      widget, for_size,
  ...
  if (avail_size > 0)
    avail_size = ctk_distribute_natural_allocation (avail_size, nvisible, sizes);
  else
    avail_size = 0;
 ...
 */
static GSList *
get_allocated_cells (CtkCellAreaBox        *box,
                     CtkCellAreaBoxContext *context,
                     CtkWidget             *widget,
                     gint                   width,
                     gint                   height)
{
  CtkCellAreaBoxAllocation *group_allocs;
  CtkCellArea              *area = CTK_CELL_AREA (box);
  CtkCellAreaBoxPrivate    *priv = box->priv;
  GList                    *cell_list;
  GSList                   *allocated_cells = NULL;
  ...
  group_allocs = _ctk_cell_area_box_context_get_orientation_allocs (context, &n_allocs);
  if (!group_allocs)
  ...
  for (position = 0, i = 0; i < n_allocs; i++)
    {
	  ...
	  if (info->fixed)
	    ...
	    {
	      gint dummy;
              ctk_cell_area_request_renderer (area, info->renderer,
                                              priv->orientation,
                                              widget, for_size,
                                              &dummy,
                                              &cell_size);
	  ...
	  if (group->align)
	  ...
	  else
	    {
          ...
          sizes = g_new (CtkRequestedSize, visible_cells);
          ...
          if (avail_size > 0)
            avail_size = ctk_distribute_natural_allocation (avail_size, visible_cells, sizes);
          else
            avail_size = 0;
 ...
 *************************************************************/
static void
ctk_cell_area_box_finalize (GObject *object)
{
  CtkCellAreaBox        *box = CTK_CELL_AREA_BOX (object);
  CtkCellAreaBoxPrivate *priv = box->priv;
  GSList                *l;
  ...
  G_OBJECT_CLASS (ctk_cell_area_box_parent_class)->finalize (object);
}

static void
ctk_cell_area_box_dispose (GObject *object)
{
  G_OBJECT_CLASS (ctk_cell_area_box_parent_class)->dispose (object);
}

static void
ctk_cell_area_box_set_property (GObject       *object,
                                guint          prop_id,
                                const GValue  *value,
                                GParamSpec    *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_ORIENTATION:
      ...
      break;
    case PROP_SPACING:
      ctk_cell_area_box_set_spacing (box, g_value_get_int (value));
      break;
    default:
...
static void
ctk_cell_area_box_get_property (GObject     *object,
                                guint        prop_id,
                                GValue      *value,
                                GParamSpec  *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_ORIENTATION:
      ...
      break;
    case PROP_SPACING:
      g_value_set_int (value, ctk_cell_area_box_get_spacing (box));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
 ...
 *************************************************************/
static void
ctk_cell_area_box_add (CtkCellArea        *area,
                       CtkCellRenderer    *renderer)
{
  ctk_cell_area_box_pack_start (CTK_CELL_AREA_BOX (area),
                                renderer, FALSE, FALSE, TRUE);
}

static void
ctk_cell_area_box_remove (CtkCellArea        *area,
                          CtkCellRenderer    *renderer)
{
  CtkCellAreaBox        *box  = CTK_CELL_AREA_BOX (area);
  CtkCellAreaBoxPrivate *priv = box->priv;
  GList                 *node;
  ...
  if (node)
    ...
    g_warning ("Trying to remove a cell renderer that is not present CtkCellAreaBox");
}

static void
ctk_cell_area_box_foreach (CtkCellArea        *area,
                           CtkCellCallback     callback,
                           gpointer            callback_data)
{
  CtkCellAreaBox        *box  = CTK_CELL_AREA_BOX (area);
  ...
  for (l = allocated_cells; l; l = l->next)
    {
       ...
       * this can happen in the expander CtkTreeViewColumn where only the
...
static void
ctk_cell_area_box_apply_attributes (CtkCellArea  *area,
				    CtkTreeModel *tree_model,
				    CtkTreeIter  *iter,
				    gboolean      is_expander,
				    gboolean      is_expanded)
{
  ...
  CTK_CELL_AREA_CLASS
    (ctk_cell_area_box_parent_class)->apply_attributes (area, tree_model, iter,
...
static void
ctk_cell_area_box_set_cell_property (CtkCellArea        *area,
                                     CtkCellRenderer    *renderer,
                                     guint               prop_id,
                                     const GValue       *value,
                                     GParamSpec         *pspec)
{
  ...
  switch (prop_id)
    ...
    case CELL_PROP_PACK_TYPE:
      ...
      break;
    default:
      CTK_CELL_AREA_WARN_INVALID_CELL_PROPERTY_ID (area, prop_id, pspec);
      break;
    }
...
static void
ctk_cell_area_box_get_cell_property (CtkCellArea        *area,
                                     CtkCellRenderer    *renderer,
                                     guint               prop_id,
                                     GValue             *value,
                                     GParamSpec         *pspec)
...
ctk_cell_area_box_create_context (CtkCellArea *area)
{
  CtkCellAreaBox        *box  = CTK_CELL_AREA_BOX (area);
  CtkCellAreaBoxPrivate *priv = box->priv;
  CtkCellAreaContext    *context =
    (CtkCellAreaContext *)g_object_new (CTK_TYPE_CELL_AREA_BOX_CONTEXT,
                                     "area", area, NULL);
  ...
  init_context_group (box, CTK_CELL_AREA_BOX_CONTEXT (context));

  return context;
}

static CtkCellAreaContext *
ctk_cell_area_box_copy_context (CtkCellArea        *area,
                                CtkCellAreaContext *context)
{
  CtkCellAreaBox        *box  = CTK_CELL_AREA_BOX (area);
  CtkCellAreaBoxPrivate *priv = box->priv;
  CtkCellAreaContext    *copy =
    (CtkCellAreaContext *)_ctk_cell_area_box_context_copy (CTK_CELL_AREA_BOX (area),
                                                          CTK_CELL_AREA_BOX_CONTEXT (context));
...
ctk_cell_area_box_get_request_mode (CtkCellArea *area)
{
  CtkCellAreaBox        *box  = CTK_CELL_AREA_BOX (area);
  CtkCellAreaBoxPrivate *priv = box->priv;

  return (priv->orientation) == CTK_ORIENTATION_HORIZONTAL ?
    CTK_SIZE_REQUEST_HEIGHT_FOR_WIDTH :
    CTK_SIZE_REQUEST_WIDTH_FOR_HEIGHT;
}

static void
compute_size (CtkCellAreaBox        *box,
              CtkOrientation         orientation,
              CtkCellAreaBoxContext *context,
              CtkWidget             *widget,
              gint                   for_size,
              ...
              gint                  *natural_size)
{
  ...
  for (i = 0; i < priv->groups->len; i++)
    {
      ...
      for (list = group->cells; list; list = list->next)
        {
          ...
          ctk_cell_area_request_renderer (area, info->renderer, orientation, widget, for_size,
      ...
      if (orientation == CTK_ORIENTATION_HORIZONTAL)
        {
          if (for_size < 0)
            _ctk_cell_area_box_context_push_group_width (context, group->id, group_min_size, group_nat_size);
          else
            _ctk_cell_area_box_context_push_group_width_for_height (context, group->id, for_size,
                                                                   group_min_size, group_nat_size);
        }
      else
        {
          if (for_size < 0)
            _ctk_cell_area_box_context_push_group_height (context, group->id, group_min_size, group_nat_size);
          else
            _ctk_cell_area_box_context_push_group_height_for_width (context, group->id, for_size,
                                                                   group_min_size, group_nat_size);
        }
    }
  ...
  *natural_size = nat_size;
  ...
  priv->rtl = (priv->orientation == CTK_ORIENTATION_HORIZONTAL &&
               ctk_widget_get_direction (widget) == CTK_TEXT_DIR_RTL);
}

static CtkRequestedSize *
get_group_sizes (CtkCellArea    *area,
                 CellGroup      *group,
                 CtkOrientation  orientation,
                 CtkWidget      *widget,
                 gint           *n_sizes)
{
  ...
  *n_sizes = count_visible_cells (group, NULL);
  sizes    = g_new (CtkRequestedSize, *n_sizes);
...
static void
compute_group_size_for_opposing_orientation (CtkCellAreaBox     *box,
                                             CellGroup          *group,
                                             CtkWidget          *widget,
                                             gint                for_size,
                                             gint               *minimum_size,
                                             gint               *natural_size)
{
  ...
  if (group->n_cells == 1)
  ...
  else
    {
      ...
      if (avail_size > 0)
        avail_size = ctk_distribute_natural_allocation (avail_size, n_sizes, orientation_sizes);
      else
        avail_size = 0;
...
static void
compute_size_for_opposing_orientation (CtkCellAreaBox        *box,
                                       CtkCellAreaBoxContext *context,
                                       CtkWidget             *widget,
                                       gint                   for_size,
                                       gint                  *minimum_size,
                                       gint                  *natural_size)
{
  ...
  if (priv->orientation == CTK_ORIENTATION_HORIZONTAL)
    orientation_sizes = _ctk_cell_area_box_context_get_widths (context, &n_groups);
  else
    orientation_sizes = _ctk_cell_area_box_context_get_heights (context, &n_groups);
  ...
  if (avail_size > 0)
    avail_size = ctk_distribute_natural_allocation (avail_size, n_groups, orientation_sizes);
  else
    avail_size = 0;
  ...
  for (i = 0; i < n_groups; i++)
    {
      ...
      if (priv->orientation == CTK_ORIENTATION_HORIZONTAL)
        {
          _ctk_cell_area_box_context_push_group_height_for_width (context, group_idx, for_size,
                                                                 group_min, group_nat);
        }
      else
        {
          _ctk_cell_area_box_context_push_group_width_for_height (context, group_idx, for_size,
                                                                 group_min, group_nat);
        }
    }
...
static void
ctk_cell_area_box_get_preferred_width (CtkCellArea        *area,
                                       CtkCellAreaContext *context,
                                       CtkWidget          *widget,
                                       gint               *minimum_width,
                                       gint               *natural_width)
{
  ...
  g_return_if_fail (CTK_IS_CELL_AREA_BOX_CONTEXT (context));

  box_context = CTK_CELL_AREA_BOX_CONTEXT (context);

  /* Compute the size of all renderers for current row data,
   * bumping cell alignments in the context along the way
   */
  compute_size (box, CTK_ORIENTATION_HORIZONTAL,
...
static void
ctk_cell_area_box_get_preferred_height (CtkCellArea        *area,
                                        CtkCellAreaContext *context,
                                        CtkWidget          *widget,
                                        gint               *minimum_height,
                                        gint               *natural_height)
{
   ...
   * bumping cell alignments in the context along the way
   */
  compute_size (box, CTK_ORIENTATION_VERTICAL,
...
static void
ctk_cell_area_box_get_preferred_height_for_width (CtkCellArea        *area,
                                                  CtkCellAreaContext *context,
                                                  CtkWidget          *widget,
                                                  gint                width,
                                                  gint               *minimum_height,
...
static void
ctk_cell_area_box_get_preferred_width_for_height (CtkCellArea        *area,
                                                  CtkCellAreaContext *context,
                                                  CtkWidget          *widget,
                                                  gint                height,
                                                  gint               *minimum_width,
...
static gboolean
ctk_cell_area_box_focus (CtkCellArea      *area,
                         CtkDirectionType  direction)
{
  CtkCellAreaBox        *box   = CTK_CELL_AREA_BOX (area);
  CtkCellAreaBoxPrivate *priv  = box->priv;
  ...
  focus_cell = ctk_cell_area_get_focus_cell (area);
   ...
   */
  if (focus_cell && !ctk_cell_area_is_activatable (area))
    {
      ctk_cell_area_set_focus_cell (area, NULL);
      return FALSE;
    }
  ...
  switch (direction)
    ...
    case CTK_DIR_TAB_BACKWARD:
      ...
      break;
    case CTK_DIR_UP:
      if (priv->orientation == CTK_ORIENTATION_VERTICAL || !priv->last_focus_cell)
        cycle = FOCUS_PREV;
      else if (!focus_cell)
    ...
    case CTK_DIR_DOWN:
      ...
      break;
    case CTK_DIR_LEFT:
      if (priv->orientation == CTK_ORIENTATION_HORIZONTAL || !priv->last_focus_cell)
        cycle = priv->rtl ? FOCUS_NEXT : FOCUS_PREV;
      else if (!focus_cell)
        cycle = FOCUS_LAST_CELL;
      break;
  ...
  if (cycle == FOCUS_LAST_CELL)
    {
      ctk_cell_area_set_focus_cell (area, priv->last_focus_cell);
      cycled_focus = TRUE;
    }
               ...
               cycled_focus == FALSE && list; list = (cycle == FOCUS_NEXT) ? list->next : list->prev)
            {
              ...
              if (info->renderer == focus_cell)
                found_cell = TRUE;
                       ...
                       ctk_cell_area_get_focus_from_sibling (area, info->renderer) == NULL)
                {
                  ctk_cell_area_set_focus_cell (area, info->renderer);
                  cycled_focus = TRUE;
                }
            }
        }
 ...
 *************************************************************/
static void
ctk_cell_area_box_cell_layout_init (CtkCellLayoutIface *iface)
{
  iface->pack_start = ctk_cell_area_box_layout_pack_start;
  iface->pack_end   = ctk_cell_area_box_layout_pack_end;
  iface->reorder    = ctk_cell_area_box_layout_reorder;
}

static void
ctk_cell_area_box_layout_pack_start (CtkCellLayout      *cell_layout,
                                     CtkCellRenderer    *renderer,
                                     gboolean            expand)
{
  ctk_cell_area_box_pack_start (CTK_CELL_AREA_BOX (cell_layout), renderer, expand, FALSE, TRUE);
}

static void
ctk_cell_area_box_layout_pack_end (CtkCellLayout      *cell_layout,
                                   CtkCellRenderer    *renderer,
                                   gboolean            expand)
{
  ctk_cell_area_box_pack_end (CTK_CELL_AREA_BOX (cell_layout), renderer, expand, FALSE, TRUE);
}

static void
ctk_cell_area_box_layout_reorder (CtkCellLayout      *cell_layout,
                                  CtkCellRenderer    *renderer,
                                  gint                position)
{
 ...
 *************************************************************/
gboolean
_ctk_cell_area_box_group_visible (CtkCellAreaBox  *box,
				  gint             group_idx)
{
  CtkCellAreaBoxPrivate *priv  = box->priv;
  CellGroup *group;
 ...
 *
 * Returns: a newly created #CtkCellAreaBox
 *
 * Since: 3.0
 */
CtkCellArea *
...
{
  return (CtkCellArea *)g_object_new (CTK_TYPE_CELL_AREA_BOX, NULL);
}

/**
 * ctk_cell_area_box_pack_start:
 * @box: a #CtkCellAreaBox
 * @renderer: the #CtkCellRenderer to add
 ...
 *
 * The @renderer is packed after any other #CtkCellRenderer packed
			       ...
			       gboolean         fixed)
{
  ...
  g_return_if_fail (CTK_IS_CELL_AREA_BOX (box));
  ...
  if (g_list_find_custom (priv->cells, renderer,
                          (GCompareFunc)cell_info_find))
    {
      g_warning ("Refusing to add the same cell renderer to a CtkCellAreaBox twice");
      return;
    }

  info = cell_info_new (renderer, CTK_PACK_START, expand, align, fixed);
 ...
 * #CtkCellRenderer packed with reference to the end of @box.
 *
 * Since: 3.0
 */
void
			    ...
			    gboolean         fixed)
{
  ...
  if (g_list_find_custom (priv->cells, renderer,
                          (GCompareFunc)cell_info_find))
    {
      g_warning ("Refusing to add the same cell renderer to a CtkCellArea twice");
      return;
    }

  info = cell_info_new (renderer, CTK_PACK_END, expand, align, fixed);
...
{
  g_return_val_if_fail (CTK_IS_CELL_AREA_BOX (box), 0);
 ...
 * @box: a #CtkCellAreaBox
 * @spacing: the space to add between #CtkCellRenderers
 *
