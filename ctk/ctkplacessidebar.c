 ...
 * @Title: CtkPlacesSidebar
 * @See_also: #CtkFileChooser
 *
 * #CtkPlacesSidebar is a widget that displays a list of frequently-used places in the
 * file system:  the user’s home directory, the user’s bookmarks, and volumes and drives.
 * This widget is used as a sidebar in #CtkFileChooser and may be used by file managers
 ...
 * for a Clipart folder.  You can do this with ctk_places_sidebar_add_shortcut().
 *
 * To make use of the places sidebar, an application at least needs to connect
 * to the #CtkPlacesSidebar::open-location signal.  This is emitted when the
 * user selects in the sidebar a location to open.  The application should also
 * call ctk_places_sidebar_set_location() when it changes the currently-viewed
 * location.
 *
 * # CSS nodes
 *
 * CtkPlacesSidebar uses a single CSS node with name placessidebar and style
 * class .sidebar.
 *
 ...
 * DROP_STATE_NEW_BOOKMARK_ARMED, however, if the client of CtkPlacesSidebar
...
struct _CtkPlacesSidebar {
  CtkScrolledWindow parent;

  CtkWidget *list_box;
  CtkWidget *new_bookmark_row;
  ...
  CtkTrashMonitor   *trash_monitor;
  CtkSettings       *ctk_settings;
  GFile             *current_location;

  CtkWidget *rename_popover;
  CtkWidget *rename_entry;
  CtkWidget *rename_button;
  CtkWidget *rename_error;
  gchar *rename_uri;
  ...
  gint drag_root_x;
  gint drag_root_y;
  CtkWidget *row_placeholder;
  DropState drop_state;
  CtkGesture *long_press_gesture;

  /* volume mounting - delayed open process */
  CtkPlacesOpenFlags go_to_after_mount_open_flags;
  GCancellable *cancellable;

  CtkWidget *popover;
  CtkSidebarRow *context_row;
  GSList *shortcuts;
...
struct _CtkPlacesSidebarClass {
  CtkScrolledWindowClass parent;

  void    (* open_location)          (CtkPlacesSidebar   *sidebar,
                                      GFile              *location,
                                      CtkPlacesOpenFlags  open_flags);
  void    (* populate_popup)         (CtkPlacesSidebar   *sidebar,
                                      CtkMenu            *menu,
                                      GFile              *selected_item,
  ...
  void    (* show_other_locations_with_flags)   (CtkPlacesSidebar   *sidebar,
                                                 CtkPlacesOpenFlags  open_flags);

  void    (* show_starred_location)    (CtkPlacesSidebar   *sidebar);

  void    (* mount)                  (CtkPlacesSidebar   *sidebar,
                                      GMountOperation    *mount_operation);
  void    (* unmount)                (CtkPlacesSidebar   *sidebar,
...
static gboolean eject_or_unmount_bookmark  (CtkSidebarRow *row);
static gboolean eject_or_unmount_selection (CtkPlacesSidebar *sidebar);
...
static void stop_drop_feedback (CtkPlacesSidebar *sidebar);
static GMountOperation * get_mount_operation (CtkPlacesSidebar *sidebar);
static GMountOperation * get_unmount_operation (CtkPlacesSidebar *sidebar);
...
static const CtkTargetEntry dnd_source_targets[] = {
  { "DND_CTK_SIDEBAR_ROW", CTK_TARGET_SAME_WIDGET, DND_CTK_SIDEBAR_ROW }
};

/* Target types for dropping into the shortcuts list */
static const CtkTargetEntry dnd_drop_targets [] = {
  { "DND_CTK_SIDEBAR_ROW", CTK_TARGET_SAME_WIDGET, DND_CTK_SIDEBAR_ROW }
};

G_DEFINE_TYPE (CtkPlacesSidebar, ctk_places_sidebar, CTK_TYPE_SCROLLED_WINDOW);

static void
emit_open_location (CtkPlacesSidebar   *sidebar,
                    GFile              *location,
                    CtkPlacesOpenFlags  open_flags)
{
...
static void
emit_show_error_message (CtkPlacesSidebar *sidebar,
                         const gchar      *primary,
                         const gchar      *secondary)
{
...
static void
emit_show_connect_to_server (CtkPlacesSidebar *sidebar)
{
  g_signal_emit (sidebar, places_sidebar_signals[SHOW_CONNECT_TO_SERVER], 0);
}

static void
emit_show_enter_location (CtkPlacesSidebar *sidebar)
{
  g_signal_emit (sidebar, places_sidebar_signals[SHOW_ENTER_LOCATION], 0);
}

static void
emit_show_other_locations (CtkPlacesSidebar *sidebar)
{
  g_signal_emit (sidebar, places_sidebar_signals[SHOW_OTHER_LOCATIONS], 0);
}

static void
emit_show_other_locations_with_flags (CtkPlacesSidebar   *sidebar,
                                      CtkPlacesOpenFlags  open_flags)
{
...
static void
emit_show_starred_location (CtkPlacesSidebar  *sidebar,
                            CtkPlacesOpenFlags open_flags)
{
...
static void
emit_mount_operation (CtkPlacesSidebar *sidebar,
...
static void
emit_unmount_operation (CtkPlacesSidebar *sidebar,
                        GMountOperation  *mount_op)
{
                      ...
                      gpointer       user_data)
{
  ...
  if (before)
  ...
  else
    {
      ...
      ctk_widget_set_margin_top (CTK_WIDGET (row), 4);
    }

  if (before && before_section_type != row_section_type)
    {
      separator = ctk_separator_new (CTK_ORIENTATION_HORIZONTAL);
      ctk_widget_set_margin_top (separator, 4);
      ctk_widget_set_margin_bottom (separator, 4);
      ctk_list_box_row_set_header (row, separator);
    }
}

static CtkWidget*
add_place (CtkPlacesSidebar            *sidebar,
           ...
           const gchar                 *tooltip)
{
  ...
  row = g_object_new (CTK_TYPE_SIDEBAR_ROW,
                      "sidebar", sidebar,
  ...
  eject_button = ctk_sidebar_row_get_eject_button (CTK_SIDEBAR_ROW (row));
  event_box = ctk_sidebar_row_get_event_box (CTK_SIDEBAR_ROW (row));
  ...
  ctk_container_add (CTK_CONTAINER (sidebar->list_box), CTK_WIDGET (row));
...
static gboolean
recent_files_setting_is_enabled (CtkPlacesSidebar *sidebar)
{
...
static gboolean
should_show_recent (CtkPlacesSidebar *sidebar)
{
...
static void
open_home (CtkPlacesSidebar *sidebar)
{
  const gchar *home_path;
  GFile       *home_dir;
...
static void
add_special_dirs (CtkPlacesSidebar *sidebar)
{
  GList *dirs;
  gint index;
  ...
  for (index = 0; index < G_USER_N_DIRECTORIES; index++)
    {
      ...
      if (!_ctk_bookmarks_manager_get_is_xdg_dir_builtin (index))
        continue;
      ...
      name = _ctk_bookmarks_manager_get_bookmark_label (sidebar->bookmarks_manager, root);
      if (!name)
...
static gboolean
should_show_file (CtkPlacesSidebar *sidebar,
                  GFile            *file)
{
  gchar *path;
...
static gboolean
file_is_shown (CtkPlacesSidebar *sidebar,
               GFile            *file)
{
  gchar *uri;
  GList *rows;
  ...
  rows = ctk_container_get_children (CTK_CONTAINER (sidebar->list_box));
  l = rows;
  while (l != NULL && !found)
    {
...
static void
add_application_shortcuts (CtkPlacesSidebar *sidebar)
{
  GSList *l;
                                 ...
                                 gpointer      data)
{
  ...
  CtkPlacesSidebar *sidebar = clos->sidebar;
  ...
  bookmark_name = _ctk_bookmarks_manager_get_bookmark_label (sidebar->bookmarks_manager, root);
...
static void
update_trash_icon (CtkPlacesSidebar *sidebar)
{
  if (sidebar->trash_row)
    {
      GIcon *icon;

      icon = _ctk_trash_monitor_get_icon (sidebar->trash_monitor);
      ctk_sidebar_row_set_start_icon (CTK_SIDEBAR_ROW (sidebar->trash_row), icon);
      g_object_unref (icon);
    }
}
...
static gboolean
create_cloud_provider_account_row (CtkPlacesSidebar      *sidebar,
...
static void
update_places (CtkPlacesSidebar *sidebar)
{
  GList *mounts, *l, *ll;
  GMount *mount;
  GList *drives;
  ...
  selected = ctk_list_box_get_selected_row (CTK_LIST_BOX (sidebar->list_box));
  if (selected)
  ...
  ctk_container_foreach (CTK_CONTAINER (sidebar->list_box),
  ...
  if (!sidebar->local_only && sidebar->show_trash)
    {
      start_icon = _ctk_trash_monitor_get_icon (sidebar->trash_monitor);
  ...
  /* add bookmarks */
  bookmarks = _ctk_bookmarks_manager_list_bookmarks (sidebar->bookmarks_manager);
  ...
  for (sl = bookmarks, index = 0; sl; sl = sl->next, index++)
    {
      ...
      if (_ctk_bookmarks_manager_get_is_builtin (sidebar->bookmarks_manager, root))
        continue;
  ...
  context = ctk_widget_get_style_context (sidebar->new_bookmark_row);
  ctk_style_context_add_class (context, "sidebar-new-bookmark-row");
  ...
  ctk_sidebar_row_hide (CTK_SIDEBAR_ROW (sidebar->new_bookmark_row), TRUE);
  ...
  if (original_uri)
    {
      ...
      ctk_places_sidebar_set_location (sidebar, restore);
                              ...
                              CdkDragContext   *context)
{
  ...
  for (l = rows; l != NULL; l = l->next)
    {
      sensitive = !dragging || check_valid_drop_target (sidebar, CTK_SIDEBAR_ROW (l->data), context);
      ctk_widget_set_sensitive (CTK_WIDGET (l->data), sensitive);
    }
...
static void
free_drag_data (CtkPlacesSidebar *sidebar)
{
                     ...
                     CdkDragContext   *context)
{
  if (sidebar->drag_data_info != DND_CTK_SIDEBAR_ROW)
    {
      ctk_sidebar_row_reveal (CTK_SIDEBAR_ROW (sidebar->new_bookmark_row));
...
static void
stop_drop_feedback (CtkPlacesSidebar *sidebar)
{
  ...
  if (sidebar->drop_state != DROP_STATE_NEW_BOOKMARK_ARMED_PERMANENT &&
    ...
    {
      ctk_sidebar_row_hide (CTK_SIDEBAR_ROW (sidebar->new_bookmark_row), FALSE);
      sidebar->drop_state = DROP_STATE_NORMAL;
    }

  if (sidebar->drag_row != NULL)
    {
      ctk_widget_show (sidebar->drag_row);
      sidebar->drag_row = NULL;
    }

  if (sidebar->row_placeholder != NULL)
    {
      ctk_widget_destroy (sidebar->row_placeholder);
                     ...
                     gpointer        user_data)
{
  ...
  CtkWidget *drag_widget;
  CtkWidget *window;

  ctk_widget_get_allocation (sidebar->drag_row, &allocation);
  ctk_widget_hide (sidebar->drag_row);

  drag_widget = CTK_WIDGET (ctk_sidebar_row_clone (CTK_SIDEBAR_ROW (sidebar->drag_row)));
  window = ctk_window_new (CTK_WINDOW_POPUP);
  sidebar->drag_row_height = allocation.height;
  ctk_widget_set_size_request (window, allocation.width, allocation.height);

  ctk_container_add (CTK_CONTAINER (window), drag_widget);
  ctk_widget_show_all (window);
  ctk_widget_set_opacity (window, 0.8);

  ctk_drag_set_icon_widget (context,
                            window,
                            sidebar->drag_row_x,
                            sidebar->drag_row_y);
}
                      ...
                      gpointer        user_data)
{
  ...
  sidebar->dragging_over = TRUE;
  action = 0;
  row = ctk_list_box_get_row_at_y (CTK_LIST_BOX (sidebar->list_box), y);

  ctk_list_box_drag_unhighlight_row (CTK_LIST_BOX (sidebar->list_box));
  ...
  if (!check_valid_drop_target (sidebar, CTK_SIDEBAR_ROW (row), context))
    goto out;
  ...
  if (sidebar->drag_data_received &&
      sidebar->drag_data_info == DND_CTK_SIDEBAR_ROW)
    {
      ...
      if (sidebar->row_placeholder == NULL)
        {
          ...
          ctk_widget_show (sidebar->row_placeholder);
          g_object_ref_sink (sidebar->row_placeholder);
        }
      else if (CTK_WIDGET (row) == sidebar->row_placeholder)
        {
          goto out;
        }

      if (ctk_widget_get_parent (sidebar->row_placeholder) != NULL)
        {
          ctk_container_remove (CTK_CONTAINER (sidebar->list_box),
      ...
      if (row != NULL)
        {
          ...
          ctk_widget_translate_coordinates (widget, CTK_WIDGET (row),
		                            x, y,
		                            &dest_x, &dest_y);
      ...
      ctk_list_box_prepend (CTK_LIST_BOX (sidebar->list_box),
                            sidebar->row_placeholder);
    }
  else
    {
      ctk_list_box_drag_highlight_row (CTK_LIST_BOX (sidebar->list_box), row);
 ...
 out:
  start_drop_feedback (sidebar, CTK_SIDEBAR_ROW (row), context);
...
static void
reorder_bookmarks (CtkPlacesSidebar *sidebar,
                   CtkSidebarRow    *row,
                   gint              new_position)
{
  gchar *uri;
  ...
  _ctk_bookmarks_manager_reorder_bookmark (sidebar->bookmarks_manager, file, new_position, NULL);
...
static void
drop_files_as_bookmarks (CtkPlacesSidebar *sidebar,
                         GList            *files,
                         gint              position)
{
  GList *l;
      ...
      if (info)
        {
          if (_ctk_file_info_consider_as_directory (info))
            _ctk_bookmarks_manager_insert_bookmark (sidebar->bookmarks_manager, f, position++, NULL);
                             ...
                             gpointer          user_data)
{
  ...
  CtkPlacesSidebarPlaceType target_place_type;
  CtkPlacesSidebarSectionType target_section_type;
  gchar *target_uri;
  gboolean success;
  CtkPlacesSidebar *sidebar = CTK_PLACES_SIDEBAR (user_data);
  CtkListBoxRow *target_row;
  ...
  target_row = ctk_list_box_get_row_at_y (CTK_LIST_BOX (sidebar->list_box), y);
  ...
  if (!check_valid_drop_target (sidebar, CTK_SIDEBAR_ROW (target_row), context))
    goto out;

  if (sidebar->drag_data_info == DND_CTK_SIDEBAR_ROW)
    {
      CtkWidget **source_row;
      /* A bookmark got reordered */
      if (target_section_type != SECTION_BOOKMARKS)
        goto out;

      source_row = (void*) ctk_selection_data_get_data (selection_data);

      if (sidebar->row_placeholder != NULL)
        g_object_get (sidebar->row_placeholder, "order-index", &target_order_index, NULL);

      reorder_bookmarks (sidebar, CTK_SIDEBAR_ROW (*source_row), target_order_index);
      success = TRUE;
    }
  else
    {
 ...
 * ctk_places_sidebar_show_drop_hints) and we will free the drag data next time
...
typedef struct {
  ...
  CtkWidget *remove_item;
  CtkWidget *rename_item;
  CtkWidget *separator_item;
  CtkWidget *mount_item;
  CtkWidget *unmount_item;
  CtkWidget *eject_item;
  CtkWidget *rescan_item;
  CtkWidget *start_item;
  CtkWidget *stop_item;
} PopoverData;

static void
check_popover_sensitivity (CtkSidebarRow *row,
                           PopoverData   *data)
{
  gboolean show_mount;
  gboolean show_unmount;
  ...
  ctk_widget_set_visible (data->add_shortcut_item, (type == PLACES_MOUNTED_VOLUME));

  actions = ctk_widget_get_action_group (sidebar, "row");
  ...
  g_simple_action_set_enabled (G_SIMPLE_ACTION (action), !ctk_list_box_row_is_selected (CTK_LIST_BOX_ROW (row)));

  check_visibility (mount, volume, drive,
                    &show_mount, &show_unmount, &show_eject, &show_rescan, &show_start, &show_stop);

  ctk_widget_set_visible (data->separator_item, show_mount || show_unmount || show_eject);
  ctk_widget_set_visible (data->mount_item, show_mount);
  ctk_widget_set_visible (data->unmount_item, show_unmount);
  ctk_widget_set_visible (data->eject_item, show_eject);
  ctk_widget_set_visible (data->rescan_item, show_rescan);
  ctk_widget_set_visible (data->start_item, show_start);
  ctk_widget_set_visible (data->stop_item, show_stop);
                              ...
                              gpointer      user_data)
{
  ...
  sidebar = CTK_PLACES_SIDEBAR (user_data);
...
volume_mount_cb (GObject      *source_object,
                 ...
                 gpointer      user_data)
{
  CtkSidebarRow *row = CTK_SIDEBAR_ROW (user_data);
  CtkPlacesSidebar *sidebar;
  GVolume *volume;
  GError *error;
  gchar *primary;
  ...
  ctk_sidebar_row_set_busy (row, FALSE);
...
static void
mount_volume (CtkSidebarRow *row,
              GVolume       *volume)
{
  CtkPlacesSidebar *sidebar;
  GMountOperation *mount_op;
            ...
            CtkPlacesOpenFlags  open_flags)
{
  ...
  if (drive != NULL &&
      (g_drive_can_start (drive) || g_drive_can_start_degraded (drive)))
    {
      ...
      ctk_sidebar_row_set_busy (row, TRUE);
      mount_op = get_mount_operation (sidebar);
...
static void
open_uri (CtkPlacesSidebar   *sidebar,
          const gchar        *uri,
          CtkPlacesOpenFlags  open_flags)
{
  GFile *location;
                  ...
                  gpointer       data)
{
  ...
  flags = (CtkPlacesOpenFlags)g_variant_get_int32 (parameter);
                 ...
                 gpointer       data)
{
  ...
  if (uri != NULL)
    {
      location = g_file_new_for_uri (uri);
      if (_ctk_bookmarks_manager_insert_bookmark (sidebar->bookmarks_manager, location, -1, NULL))
        _ctk_bookmarks_manager_set_bookmark_label (sidebar->bookmarks_manager, location, name, NULL);
      g_object_unref (location);
    }
                      ...
                      CtkPlacesSidebar *sidebar)
{
  ...
  new_name = ctk_entry_get_text (CTK_ENTRY (sidebar->rename_entry));

  if (strcmp (new_name, "") == 0)
    {
      ctk_widget_set_sensitive (sidebar->rename_button, FALSE);
      ctk_label_set_label (CTK_LABEL (sidebar->rename_error), "");
      return;
    }
  ...
  ctk_widget_set_sensitive (sidebar->rename_button, !found);
  ctk_label_set_label (CTK_LABEL (sidebar->rename_error),
           ...
           CtkPlacesSidebar *sidebar)
{
  ...
  new_text = g_strdup (ctk_entry_get_text (CTK_ENTRY (sidebar->rename_entry)));

  file = g_file_new_for_uri (sidebar->rename_uri);
  if (!_ctk_bookmarks_manager_has_bookmark (sidebar->bookmarks_manager, file))
    _ctk_bookmarks_manager_insert_bookmark (sidebar->bookmarks_manager, file, -1, NULL);

  _ctk_bookmarks_manager_set_bookmark_label (sidebar->bookmarks_manager, file, new_text, NULL);
  ...
  if (sidebar->rename_popover)
    ctk_popover_popdown (CTK_POPOVER (sidebar->rename_popover));
}
...
static void
create_rename_popover (CtkPlacesSidebar *sidebar)
{
  CtkWidget *popover;
  CtkWidget *grid;
  CtkWidget *label;
  ...
  popover = ctk_popover_new (CTK_WIDGET (sidebar));
  ...
  ctk_popover_set_position (CTK_POPOVER (popover), CTK_POS_RIGHT);
  grid = ctk_grid_new ();
  ctk_container_add (CTK_CONTAINER (popover), grid);
  g_object_set (grid,
                "margin", 10,
                "row-spacing", 6,
                "column-spacing", 6,
  ...
  g_free (str);
  button = ctk_button_new_with_mnemonic (_("_Rename"));
  ctk_widget_set_can_default (button, TRUE);
  ctk_style_context_add_class (ctk_widget_get_style_context (button), "suggested-action");
  g_signal_connect (button, "clicked", G_CALLBACK (do_rename), sidebar);
  error = ctk_label_new ("");
  ctk_widget_set_halign (error, CTK_ALIGN_START);
  ctk_grid_attach (CTK_GRID (grid), label, 0, 0, 2, 1);
  ctk_grid_attach (CTK_GRID (grid), entry, 0, 1, 1, 1);
  ctk_grid_attach (CTK_GRID (grid), button,1, 1, 1, 1);
  ctk_grid_attach (CTK_GRID (grid), error, 0, 2, 2, 1);
  ctk_widget_show_all (grid);
  ctk_popover_set_default_widget (CTK_POPOVER (popover), button);
 ...
 */
static void
update_popover_shadowing (CtkWidget *row,
                          gboolean   shown)
{
  CtkStyleContext *context;
  gint count;
  ...
  if (count > 0)
    ctk_style_context_add_class (context, "has-open-popup");
  else
    ctk_style_context_remove_class (context, "has-open-popup");
}

static void
set_prelight (CtkPopover *popover)
{
  update_popover_shadowing (ctk_popover_get_relative_to (popover), TRUE);
}

static void
unset_prelight (CtkPopover *popover)
{
  update_popover_shadowing (ctk_popover_get_relative_to (popover), FALSE);
}

static void
setup_popover_shadowing (CtkWidget *popover)
{
...
static void
show_rename_popover (CtkSidebarRow *row)
{
  gchar *name;
  gchar *uri;
  CtkPlacesSidebar *sidebar;
  ...
  ctk_entry_set_text (CTK_ENTRY (sidebar->rename_entry), name);
  ctk_popover_set_relative_to (CTK_POPOVER (sidebar->rename_popover), CTK_WIDGET (row));
  setup_popover_shadowing (sidebar->rename_popover);

  ctk_popover_popup (CTK_POPOVER (sidebar->rename_popover));
  ctk_widget_grab_focus (sidebar->rename_entry);
...
static void
rename_bookmark (CtkSidebarRow *row)
{
  CtkPlacesSidebarPlaceType type;
...
static void
remove_bookmark (CtkSidebarRow *row)
{
  CtkPlacesSidebarPlaceType type;
  gchar *uri;
  GFile *file;
  ...
  if (type == PLACES_BOOKMARK)
    {
      file = g_file_new_for_uri (uri);
      _ctk_bookmarks_manager_remove_bookmark (sidebar->bookmarks_manager, file, NULL);
      g_object_unref (file);
    }
...
get_mount_operation (CtkPlacesSidebar *sidebar)
{
  GMountOperation *mount_op;
...
static GMountOperation *
get_unmount_operation (CtkPlacesSidebar *sidebar)
{
  GMountOperation *mount_op;
...
static gboolean
is_current_location_on_volume (CtkPlacesSidebar *sidebar,
                               GMount           *mount,
                               GVolume          *volume,
                               GDrive           *drive)
{
...
static gboolean
eject_or_unmount_bookmark (CtkSidebarRow *row)
{
...
static gboolean
eject_or_unmount_selection (CtkPlacesSidebar *sidebar)
{
  gboolean ret;
  CtkListBoxRow *row;

  row = ctk_list_box_get_selected_row (CTK_LIST_BOX (sidebar->list_box));
  ret = eject_or_unmount_bookmark (CTK_SIDEBAR_ROW (row));
...
static void
add_actions (CtkPlacesSidebar *sidebar)
{
  GActionGroup *actions;
  ...
  g_action_map_add_action_entries (G_ACTION_MAP (actions),
                                   ...
                                   sidebar);
  ctk_widget_insert_action_group (CTK_WIDGET (sidebar), "row", actions);
                 ...
                 CtkPlacesOpenFlags  flags)
{
  ...
  item = g_object_new (CTK_TYPE_MODEL_BUTTON,
                       "visible", TRUE,
                       "action-name", flags == CTK_PLACES_OPEN_NORMAL ? "row.open" : "row.open-other",
...
#ifdef HAVE_CLOUDPROVIDERS
static void
build_popup_menu_using_gmenu (CtkSidebarRow *row)
{
  CloudProvidersAccount *cloud_provider_account;
  CtkPlacesSidebar *sidebar;
  ...
  if (cloud_provider_account)
    {
      ...
      g_menu_item_set_action_and_target_value (item, "row.open", g_variant_new_int32(CTK_PLACES_OPEN_NORMAL));
      g_menu_append_item (menu, item);
      if (sidebar->open_flags & CTK_PLACES_OPEN_NEW_TAB)
        {
          item = g_menu_item_new (_("Open in New _Tab"), "row.open-other");
          g_menu_item_set_action_and_target_value (item, "row.open-other", g_variant_new_int32(CTK_PLACES_OPEN_NEW_TAB));
          g_menu_append_item (menu, item);
        }
      if (sidebar->open_flags & CTK_PLACES_OPEN_NEW_WINDOW)
        {
          item = g_menu_item_new (_("Open in New _Window"), "row.open-other");
          g_menu_item_set_action_and_target_value (item, "row.open-other", g_variant_new_int32(CTK_PLACES_OPEN_NEW_WINDOW));
          g_menu_append_item (menu, item);
        }
      ...
      if (cloud_provider_menu != NULL && cloud_provider_action_group != NULL)
        {
          ...
          ctk_widget_insert_action_group (CTK_WIDGET (sidebar),
                                          "cloudprovider",
                                          G_ACTION_GROUP (cloud_provider_action_group));
        }
      add_actions (sidebar);
      if (sidebar->popover)
        ctk_widget_destroy (sidebar->popover);

      sidebar->popover = ctk_popover_new_from_model (CTK_WIDGET (sidebar),
                                                     G_MENU_MODEL (menu));
...
static void
create_row_popover (CtkPlacesSidebar *sidebar,
                    CtkSidebarRow    *row)
{
  PopoverData data;
  CtkWidget *box;
  ...
  sidebar->popover = ctk_popover_new (CTK_WIDGET (sidebar));
  /* Clean sidebar pointer when its destroyed, most of the times due to its
   * relative_to associated row being destroyed */
  ...
  ctk_widget_show (box);
  ctk_container_add (CTK_CONTAINER (sidebar->popover), box);

  add_open_button (box, _("_Open"), CTK_PLACES_OPEN_NORMAL);

  if (sidebar->open_flags & CTK_PLACES_OPEN_NEW_TAB)
    add_open_button (box, _("Open in New _Tab"), CTK_PLACES_OPEN_NEW_TAB);

  if (sidebar->open_flags & CTK_PLACES_OPEN_NEW_WINDOW)
    add_open_button (box, _("Open in New _Window"), CTK_PLACES_OPEN_NEW_WINDOW);
...
static void
show_row_popover (CtkSidebarRow *row)
{
  CtkPlacesSidebar *sidebar;
  ...
  ctk_popover_set_relative_to (CTK_POPOVER (sidebar->popover), CTK_WIDGET (row));

  sidebar->context_row = row;
  ctk_popover_popup (CTK_POPOVER (sidebar->popover));
                  ...
                  gpointer       user_data)
{
   ...
   */
  if (CTK_PLACES_SIDEBAR (user_data)->dragging_over)
    return;

  selected_row = CTK_SIDEBAR_ROW (ctk_list_box_get_selected_row (list_box));
                       ...
                       CtkSidebarRow  *row)
{
  ...
  g_object_get (CTK_SIDEBAR_ROW (row),
                "sidebar", &sidebar,
                "section_type", &section_type,
                NULL);
  ...
  if (section_type == SECTION_BOOKMARKS)
    {
      sidebar->drag_row = CTK_WIDGET (row);
...
{
  gboolean ret = FALSE;
  CtkPlacesSidebarPlaceType row_type;
      ...
      if (event->button == 1)
      ...
      else if (event->button == 3)
        {
          if (row_type != PLACES_CONNECT_TO_SERVER)
            show_row_popover (CTK_SIDEBAR_ROW (row));
        }
    }
...
static void
popup_menu_cb (CtkSidebarRow *row)
{
  CtkPlacesSidebarPlaceType row_type;
               ...
               CtkPlacesSidebar *sidebar)
{
  ...
  row = CTK_WIDGET (ctk_list_box_get_row_at_y (CTK_LIST_BOX (sidebar->list_box), y));
  if (CTK_IS_SIDEBAR_ROW (row))
    popup_menu_cb (CTK_SIDEBAR_ROW (row));
}
...
static void
update_hostname (CtkPlacesSidebar *sidebar)
{
  GVariant *variant;
  gsize len;
...
static void
create_volume_monitor (CtkPlacesSidebar *sidebar)
{
                             ...
                             gpointer     user_data)
{
  ...
  g_assert (settings == sidebar->ctk_settings);
...
static void
ctk_places_sidebar_init (CtkPlacesSidebar *sidebar)
{
  ...
  sidebar->open_flags = CTK_PLACES_OPEN_NORMAL;

  sidebar->bookmarks_manager = _ctk_bookmarks_manager_new ((CtkBookmarksChangedFunc)update_places, sidebar);

  sidebar->trash_monitor = _ctk_trash_monitor_get ();
  ...
  ctk_widget_set_size_request (CTK_WIDGET (sidebar), 140, 280);
  ...
  context = ctk_widget_get_style_context (CTK_WIDGET (sidebar));
  ctk_style_context_add_class (context, CTK_STYLE_CLASS_SIDEBAR);
  ctk_style_context_set_junction_sides (context, CTK_JUNCTION_RIGHT | CTK_JUNCTION_LEFT);

  /* list box */
  sidebar->list_box = ctk_list_box_new ();

  ctk_list_box_set_header_func (CTK_LIST_BOX (sidebar->list_box),
                                list_box_header_func, sidebar, NULL);
  ctk_list_box_set_sort_func (CTK_LIST_BOX (sidebar->list_box),
                              list_box_sort_func, NULL, NULL);
  ctk_list_box_set_selection_mode (CTK_LIST_BOX (sidebar->list_box), CTK_SELECTION_SINGLE);
  ctk_list_box_set_activate_on_single_click (CTK_LIST_BOX (sidebar->list_box), TRUE);
  ...
  sidebar->long_press_gesture = ctk_gesture_long_press_new (CTK_WIDGET (sidebar));
  ctk_gesture_single_set_touch_only (CTK_GESTURE_SINGLE (sidebar->long_press_gesture), TRUE);
  ...
  sidebar->source_targets = ctk_target_list_new (dnd_source_targets, G_N_ELEMENTS (dnd_source_targets));
  ctk_target_list_add_text_targets (sidebar->source_targets, 0);
  ...
  ctk_container_add (CTK_CONTAINER (sidebar), sidebar->list_box);
...
static void
ctk_places_sidebar_set_property (GObject      *obj,
                                 guint         property_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  CtkPlacesSidebar *sidebar = CTK_PLACES_SIDEBAR (obj);

  switch (property_id)
    {
    case PROP_LOCATION:
      ctk_places_sidebar_set_location (sidebar, g_value_get_object (value));
      break;

    case PROP_OPEN_FLAGS:
      ctk_places_sidebar_set_open_flags (sidebar, g_value_get_flags (value));
      break;

    case PROP_SHOW_RECENT:
      ctk_places_sidebar_set_show_recent (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_DESKTOP:
      ctk_places_sidebar_set_show_desktop (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_CONNECT_TO_SERVER:
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
      ctk_places_sidebar_set_show_connect_to_server (sidebar, g_value_get_boolean (value));
G_GNUC_END_IGNORE_DEPRECATIONS
      break;

    case PROP_SHOW_ENTER_LOCATION:
      ctk_places_sidebar_set_show_enter_location (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_OTHER_LOCATIONS:
      ctk_places_sidebar_set_show_other_locations (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_TRASH:
      ctk_places_sidebar_set_show_trash (sidebar, g_value_get_boolean (value));
      break;

    case PROP_SHOW_STARRED_LOCATION:
      ctk_places_sidebar_set_show_starred_location (sidebar, g_value_get_boolean (value));
      break;

    case PROP_LOCAL_ONLY:
      ctk_places_sidebar_set_local_only (sidebar, g_value_get_boolean (value));
      break;
...
static void
ctk_places_sidebar_get_property (GObject    *obj,
                                 guint       property_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_LOCATION:
      g_value_take_object (value, ctk_places_sidebar_get_location (sidebar));
      break;

    case PROP_OPEN_FLAGS:
      g_value_set_flags (value, ctk_places_sidebar_get_open_flags (sidebar));
      break;

    case PROP_SHOW_RECENT:
      g_value_set_boolean (value, ctk_places_sidebar_get_show_recent (sidebar));
      break;

    case PROP_SHOW_DESKTOP:
      g_value_set_boolean (value, ctk_places_sidebar_get_show_desktop (sidebar));
      break;

    case PROP_SHOW_CONNECT_TO_SERVER:
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
      g_value_set_boolean (value, ctk_places_sidebar_get_show_connect_to_server (sidebar));
G_GNUC_END_IGNORE_DEPRECATIONS
      break;

    case PROP_SHOW_ENTER_LOCATION:
      g_value_set_boolean (value, ctk_places_sidebar_get_show_enter_location (sidebar));
      break;

    case PROP_SHOW_OTHER_LOCATIONS:
      g_value_set_boolean (value, ctk_places_sidebar_get_show_other_locations (sidebar));
      break;

    case PROP_SHOW_TRASH:
      g_value_set_boolean (value, ctk_places_sidebar_get_show_trash (sidebar));
      break;

    case PROP_SHOW_STARRED_LOCATION:
      g_value_set_boolean (value, ctk_places_sidebar_get_show_starred_location (sidebar));
      break;

    case PROP_LOCAL_ONLY:
      g_value_set_boolean (value, ctk_places_sidebar_get_local_only (sidebar));
      break;
...
static void
ctk_places_sidebar_dispose (GObject *object)
{
  CtkPlacesSidebar *sidebar;
#ifdef HAVE_CLOUDPROVIDERS
  GList *l;
  ...
  sidebar = CTK_PLACES_SIDEBAR (object);
  ...
  if (sidebar->bookmarks_manager != NULL)
    {
      _ctk_bookmarks_manager_free (sidebar->bookmarks_manager);
  ...
  if (sidebar->rename_popover)
    {
      ctk_widget_destroy (sidebar->rename_popover);
      sidebar->rename_popover = NULL;
      sidebar->rename_entry = NULL;
      sidebar->rename_button = NULL;
      sidebar->rename_error = NULL;
  ...
  if (sidebar->ctk_settings)
    {
      g_signal_handlers_disconnect_by_func (sidebar->ctk_settings, shell_shows_desktop_changed, sidebar);
      sidebar->ctk_settings = NULL;
    }
  ...
  if (sidebar->source_targets)
    {
      ctk_target_list_unref (sidebar->source_targets);
  ...
  G_OBJECT_CLASS (ctk_places_sidebar_parent_class)->dispose (object);
}

static void
ctk_places_sidebar_finalize (GObject *object)
{
  G_OBJECT_CLASS (ctk_places_sidebar_parent_class)->finalize (object);
}

static void
ctk_places_sidebar_class_init (CtkPlacesSidebarClass *class)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (class);
  CtkWidgetClass *widget_class = CTK_WIDGET_CLASS (class);


  gobject_class->dispose = ctk_places_sidebar_dispose;
  gobject_class->finalize = ctk_places_sidebar_finalize;
  gobject_class->set_property = ctk_places_sidebar_set_property;
  gobject_class->get_property = ctk_places_sidebar_get_property;

  /**
   * CtkPlacesSidebar::open-location:
   * @sidebar: the object which received the signal.
   * @location: (type Gio.File): #GFile to which the caller should switch.
   * @open_flags: a single value from #CtkPlacesOpenFlags specifying how the @location should be opened.
   *
  ...
  places_sidebar_signals [OPEN_LOCATION] =
          g_signal_new (I_("open-location"),
                        ...
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, open_location),
                        NULL, NULL,
  ...
  /**
   * CtkPlacesSidebar::populate-popup:
   * @sidebar: the object which received the signal.
   * @container: (type Ctk.Widget): a #CtkMenu or another #CtkContainer
   ...
   * during their callbacks, the application can use ctk_places_sidebar_get_location()
   ...
   *
   * Before 3.18, the @container always was a #CtkMenu, and you were expected
   * to add your items as #CtkMenuItems. Since 3.18, the popup may be implemented
   * as a #CtkPopover, in which case @container will be something else, e.g. a
   * #CtkBox, to which you may add #CtkModelButtons or other widgets, such as
   * #CtkEntries, #CtkSpinButtons, etc. If your application can deal with this
   * situation, you can set #CtkPlacesSidebar::populate-all to %TRUE to request
  ...
  places_sidebar_signals [POPULATE_POPUP] =
          g_signal_new (I_("populate-popup"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, populate_popup),
                        NULL, NULL,
  ...
  /**
   * CtkPlacesSidebar::show-error-message:
   * @sidebar: the object which received the signal.
  ...
  places_sidebar_signals [SHOW_ERROR_MESSAGE] =
          g_signal_new (I_("show-error-message"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, show_error_message),
                        NULL, NULL,
  ...
  /**
   * CtkPlacesSidebar::show-connect-to-server:
   * @sidebar: the object which received the signal.
   *
   * The places sidebar emits this signal when it needs the calling
   ...
   *
   * Deprecated: 3.18: use the #CtkPlacesSidebar::show-other-locations signal
  ...
  places_sidebar_signals [SHOW_CONNECT_TO_SERVER] =
          g_signal_new (I_("show-connect-to-server"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, show_connect_to_server),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE, 0);

  /**
   * CtkPlacesSidebar::show-enter-location:
   * @sidebar: the object which received the signal.
   *
   * The places sidebar emits this signal when it needs the calling
  ...
  places_sidebar_signals [SHOW_ENTER_LOCATION] =
          g_signal_new (I_("show-enter-location"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, show_enter_location),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE, 0);
  ...
  places_sidebar_signals [DRAG_ACTION_REQUESTED] =
          g_signal_new (I_("drag-action-requested"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, drag_action_requested),
                        NULL, NULL,
  ...
  /**
   * CtkPlacesSidebar::drag-action-ask:
   * @sidebar: the object which received the signal.
  ...
  places_sidebar_signals [DRAG_ACTION_ASK] =
          g_signal_new (I_("drag-action-ask"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, drag_action_ask),
                        NULL, NULL,
                        _ctk_marshal_INT__INT,
                        G_TYPE_INT, 1,
                        G_TYPE_INT);

  /**
   * CtkPlacesSidebar::drag-perform-drop:
   * @sidebar: the object which received the signal.
  ...
  places_sidebar_signals [DRAG_PERFORM_DROP] =
          g_signal_new (I_("drag-perform-drop"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, drag_perform_drop),
                        NULL, NULL,
                        _ctk_marshal_VOID__OBJECT_POINTER_INT,
                        G_TYPE_NONE, 3,
                        G_TYPE_OBJECT,
                        G_TYPE_POINTER, /* GList of GFile */
                        G_TYPE_INT);
  ...
  /**
   * CtkPlacesSidebar::show-other-locations:
   * @sidebar: the object which received the signal.
   *
   * The places sidebar emits this signal when it needs the calling
   ...
   *
   * Deprecated: 3.20: use the #CtkPlacesSidebar::show-other-locations-with-flags
   * which includes the open flags in order to allow the user to specify to open
   * in a new tab or window, in a similar way than #CtkPlacesSidebar::open-location
   *
   * Since: 3.18
   */
  places_sidebar_signals [SHOW_OTHER_LOCATIONS] =
          g_signal_new (I_("show-other-locations"),
                        ...
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, show_other_locations),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE, 0);

  /**
   * CtkPlacesSidebar::show-other-locations-with-flags:
   * @sidebar: the object which received the signal.
   * @open_flags: a single value from #CtkPlacesOpenFlags specifying how it should be opened.
   *
   * The places sidebar emits this signal when it needs the calling
   * application to present a way to show other locations e.g. drives
   * and network access points.
  ...
  places_sidebar_signals [SHOW_OTHER_LOCATIONS_WITH_FLAGS] =
          g_signal_new (I_("show-other-locations-with-flags"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, show_other_locations_with_flags),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE, 1,
                        CTK_TYPE_PLACES_OPEN_FLAGS);
   ...
   * In this way the application using the #CtkPlacesSidebar can track the
  ...
  places_sidebar_signals [MOUNT] =
          g_signal_new (I_("mount"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, mount),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE,
                        1,
  ...
  places_sidebar_signals [UNMOUNT] =
          g_signal_new (I_("unmount"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, unmount),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE,
                        1,
  ...
  /**
   * CtkPlacesSidebar::show-starred-location:
   * @sidebar: the object which received the signal.
   * @open_flags: a single value from #CtkPlacesOpenFlags specifying how the
  ...
  places_sidebar_signals [SHOW_STARRED_LOCATION] =
          g_signal_new (I_("show-starred-location"),
                        G_OBJECT_CLASS_TYPE (gobject_class),
                        G_SIGNAL_RUN_FIRST,
                        G_STRUCT_OFFSET (CtkPlacesSidebarClass, show_starred_location),
                        NULL, NULL,
                        NULL,
                        G_TYPE_NONE, 1,
                        CTK_TYPE_PLACES_OPEN_FLAGS);
  ...
  /**
   * CtkPlacesSidebar:populate-all:
   *
   * If :populate-all is %TRUE, the #CtkPlacesSidebar::populate-popup signal
  ...
  ctk_widget_class_set_css_name (widget_class, "placessidebar");
}

/**
 * ctk_places_sidebar_new:
 *
 * Creates a new #CtkPlacesSidebar widget.
 *
 * The application should connect to at least the
 * #CtkPlacesSidebar::open-location signal to be notified
 * when the user makes a selection in the sidebar.
 *
 * Returns: a newly created #CtkPlacesSidebar
 *
 * Since: 3.10
 */
CtkWidget *
ctk_places_sidebar_new (void)
{
  return CTK_WIDGET (g_object_new (ctk_places_sidebar_get_type (), NULL));
}

/* Public methods for CtkPlacesSidebar */

/**
 * ctk_places_sidebar_set_open_flags:
 ...
 *
 * When the #CtkPlacesSidebar::open-location signal is emitted, its flags
 ...
 */
void
ctk_places_sidebar_set_open_flags (CtkPlacesSidebar   *sidebar,
                                   CtkPlacesOpenFlags  flags)
{
  g_return_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar));
...
/**
 * ctk_places_sidebar_get_open_flags:
 * @sidebar: a #CtkPlacesSidebar
 *
 * Gets the open flags.
 *
 * Returns: the #CtkPlacesOpenFlags of @sidebar
 *
 * Since: 3.10
 */
CtkPlacesOpenFlags
ctk_places_sidebar_get_open_flags (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), 0);

  return sidebar->open_flags;
}

/**
 * ctk_places_sidebar_set_location:
 * @sidebar: a places sidebar
 ...
 */
void
ctk_places_sidebar_set_location (CtkPlacesSidebar *sidebar,
                                 GFile            *location)
{
  GList *children;
  GList *child;
  ...
  ctk_list_box_unselect_all (CTK_LIST_BOX (sidebar->list_box));
  ...
  children = ctk_container_get_children (CTK_CONTAINER (sidebar->list_box));
  for (child = children; child != NULL && !found; child = child->next)
    {
      ...
      if (row_uri != NULL && g_strcmp0 (row_uri, uri) == 0)
        {
          ctk_list_box_select_row (CTK_LIST_BOX (sidebar->list_box),
                                   CTK_LIST_BOX_ROW (child->data));
          found = TRUE;
        }
...
/**
 * ctk_places_sidebar_get_location:
 * @sidebar: a places sidebar
 *
 * Gets the currently selected location in the @sidebar. This can be %NULL when
 * nothing is selected, for example, when ctk_places_sidebar_set_location() has
 * been called with a location that is not among the sidebar’s list of places to
 * show.
 *
 * You can use this function to get the selection in the @sidebar.  Also, if you
 * connect to the #CtkPlacesSidebar::populate-popup signal, you can use this
 ...
 */
GFile *
ctk_places_sidebar_get_location (CtkPlacesSidebar *sidebar)
{
  CtkListBoxRow *selected;
  GFile *file;
...
gchar *
ctk_places_sidebar_get_location_title (CtkPlacesSidebar *sidebar)
{
  CtkListBoxRow *selected;
  gchar *title;
...
/**
 * ctk_places_sidebar_set_show_recent:
 * @sidebar: a places sidebar
 ...
 */
void
ctk_places_sidebar_set_show_recent (CtkPlacesSidebar *sidebar,
...
/**
 * ctk_places_sidebar_get_show_recent:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with ctk_places_sidebar_set_show_recent()
 *
 ...
 */
gboolean
ctk_places_sidebar_get_show_recent (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), FALSE);

  return sidebar->show_recent;
}

/**
 * ctk_places_sidebar_set_show_desktop:
 * @sidebar: a places sidebar
 ...
 */
void
ctk_places_sidebar_set_show_desktop (CtkPlacesSidebar *sidebar,
                                     gboolean          show_desktop)
{
  g_return_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar));

  /* Don't bother disconnecting from the CtkSettings -- it will just
...
/**
 * ctk_places_sidebar_get_show_desktop:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with ctk_places_sidebar_set_show_desktop()
 *
 ...
 */
gboolean
ctk_places_sidebar_get_show_desktop (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), FALSE);
...
/**
 * ctk_places_sidebar_set_show_connect_to_server:
 * @sidebar: a places sidebar
 ...
 * #CtkPlacesSidebar::show-connect-to-server signal.
 *
 * Since: 3.10
 *
 ...
 */
void
ctk_places_sidebar_set_show_connect_to_server (CtkPlacesSidebar *sidebar,
...
/**
 * ctk_places_sidebar_get_show_connect_to_server:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with ctk_places_sidebar_set_show_connect_to_server()
 *
 ...
 */
gboolean
ctk_places_sidebar_get_show_connect_to_server (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), FALSE);
...
/**
 * ctk_places_sidebar_set_show_enter_location:
 * @sidebar: a places sidebar
 ...
 *
 * If you enable this, you should connect to the
 * #CtkPlacesSidebar::show-enter-location signal.
 *
 * Since: 3.14
 */
void
ctk_places_sidebar_set_show_enter_location (CtkPlacesSidebar *sidebar,
...
/**
 * ctk_places_sidebar_get_show_enter_location:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with ctk_places_sidebar_set_show_enter_location()
 *
 ...
 */
gboolean
ctk_places_sidebar_get_show_enter_location (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), FALSE);
...
/**
 * ctk_places_sidebar_set_show_other_locations:
 * @sidebar: a places sidebar
 ...
 *
 * If you enable this, you should connect to the
 * #CtkPlacesSidebar::show-other-locations signal.
 *
 * Since: 3.18
 */
void
ctk_places_sidebar_set_show_other_locations (CtkPlacesSidebar *sidebar,
...
/**
 * ctk_places_sidebar_get_show_other_locations:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with ctk_places_sidebar_set_show_other_locations()
 *
 ...
 */
gboolean
ctk_places_sidebar_get_show_other_locations (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), FALSE);
...
/**
 * ctk_places_sidebar_set_show_trash:
 * @sidebar: a places sidebar
 ...
 */
void
ctk_places_sidebar_set_show_trash (CtkPlacesSidebar *sidebar,
                                   gboolean          show_trash)
{
  g_return_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar));
...
/**
 * ctk_places_sidebar_get_show_trash:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with ctk_places_sidebar_set_show_trash()
 *
 ...
 */
gboolean
ctk_places_sidebar_get_show_trash (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), TRUE);

  return sidebar->show_trash;
}

/**
 * ctk_places_sidebar_set_local_only:
 * @sidebar: a places sidebar
 ...
 */
void
ctk_places_sidebar_set_local_only (CtkPlacesSidebar *sidebar,
                                   gboolean          local_only)
{
  g_return_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar));
...
/**
 * ctk_places_sidebar_get_local_only:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with ctk_places_sidebar_set_local_only().
 *
 ...
 */
gboolean
ctk_places_sidebar_get_local_only (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), FALSE);
...
static GSList *
find_shortcut_link (CtkPlacesSidebar *sidebar,
                    GFile            *location)
{
  GSList *l;
...
/**
 * ctk_places_sidebar_add_shortcut:
 * @sidebar: a places sidebar
 ...
 */
void
ctk_places_sidebar_add_shortcut (CtkPlacesSidebar *sidebar,
                                 GFile            *location)
{
  g_return_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar));
...
/**
 * ctk_places_sidebar_remove_shortcut:
 * @sidebar: a places sidebar
 * @location: location to remove
 *
 * Removes an application-specific shortcut that has been previously been
 * inserted with ctk_places_sidebar_add_shortcut().  If the @location is not a
 * shortcut in the sidebar, then nothing is done.
 *
 * Since: 3.10
 */
void
ctk_places_sidebar_remove_shortcut (CtkPlacesSidebar *sidebar,
                                    GFile            *location)
{
  GSList *link;
  GFile *shortcut;
...
/**
 * ctk_places_sidebar_list_shortcuts:
 * @sidebar: a places sidebar
 *
 ...
 *     application-specific shortcuts with ctk_places_sidebar_add_shortcut().
 ...
 */
GSList *
ctk_places_sidebar_list_shortcuts (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), NULL);

  return g_slist_copy_deep (sidebar->shortcuts, (GCopyFunc) g_object_ref, NULL);
}

/**
 * ctk_places_sidebar_get_nth_bookmark:
 * @sidebar: a places sidebar
 * @n: index of the bookmark to query
 *
 * This function queries the bookmarks added by the user to the places sidebar,
 * and returns one of them.  This function is used by #CtkFileChooser to implement
 ...
 */
GFile *
ctk_places_sidebar_get_nth_bookmark (CtkPlacesSidebar *sidebar,
                                     gint              n)
{
  GList *rows;
  GList *l;
...
/**
 * ctk_places_sidebar_set_drop_targets_visible:
 * @sidebar: a places sidebar.
 ...
 *
 * Make the CtkPlacesSidebar show drop targets, so it can show the available
 ...
 * automatically if the drag finishes in the CtkPlacesSidebar. You only need
...
/**
 * ctk_places_sidebar_set_show_starred_location:
 * @sidebar: a places sidebar
 * @show_starred_location: whether to show an item for Starred files
 *
 * If you enable this, you should connect to the
 * #CtkPlacesSidebar::show-starred-location signal.
 *
 * Since: 3.22.26
 */
void
ctk_places_sidebar_set_show_starred_location (CtkPlacesSidebar *sidebar,
...
/**
 * ctk_places_sidebar_get_show_starred_location:
 * @sidebar: a places sidebar
 *
 * Returns the value previously set with ctk_places_sidebar_set_show_starred_location()
 *
 ...
 */
gboolean
ctk_places_sidebar_get_show_starred_location (CtkPlacesSidebar *sidebar)
{
  g_return_val_if_fail (CTK_IS_PLACES_SIDEBAR (sidebar), FALSE);
