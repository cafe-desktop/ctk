                   ...
                   GError     **error)
{
  ...
  if (object_info->object == NULL)
    {
      object = _ctk_builder_construct (data->builder, object_info, error);
      if (!object)
        return NULL;
    }
  else
    {
      ...
      _ctk_builder_apply_properties (data->builder, object_info, error);
    }
                ...
                GError      **error)
{
  ...
  if (!g_markup_collect_attributes (element_name, names, values, error,
    ...
    {
      _ctk_builder_prefix_error (data->builder, data->ctx, error);
      return;
    }
              ...
              GError              **error)
{
  ...
  if (type_func)
    ...
    {
      object_type = ctk_builder_get_type_from_name (data->builder, object_class);
      if (object_type == G_TYPE_INVALID)
        {
          g_set_error (error,
                       CTK_BUILDER_ERROR,
                ...
                GError              **error)
{
  ...
  template_type = _ctk_builder_get_template_type (data->builder);
  ...
  if (parent_class)
    {
      ...
      if (parent_type == G_TYPE_INVALID)
        {
          g_set_error (error, CTK_BUILDER_ERROR,
                       CTK_BUILDER_ERROR_INVALID_VALUE,
  ...
  object_info->object = ctk_builder_get_object (data->builder, object_class);
  state_push (data, object_info);
                ...
                GError      **error)
{
  ...
  if (bind_flags_str)
    {
      if (!_ctk_builder_flags_from_string (G_TYPE_BINDING_FLAGS, NULL, bind_flags_str, &bind_flags, error))
        {
          _ctk_builder_prefix_error (data->builder, data->ctx, error);
          return;
        }
  ...
  if (domain)
    {
      ...
      ctk_builder_set_translation_domain (data->builder, data->domain);
    }
}
               ...
               GError              **error)
{
  ...
  ctk_buildable_custom_tag_end (CTK_BUILDABLE (data->subparser->object),
                                data->builder,
  ...
  if (_ctk_builder_lookup_failed (data->builder, error))
    return;

  if (CTK_BUILDABLE_GET_IFACE (data->subparser->object)->custom_finished)
              ...
              GError              **error)
{
  ...
  if (strcmp (parent_info->tag.name, "object") == 0 ||
      strcmp (parent_info->tag.name, "template") == 0)
    {
      ...
      if (!object_info->object)
        {
          object_info->properties = g_slist_reverse (object_info->properties);
          object_info->object = _ctk_builder_construct (data->builder,
                                                        object_info,
                                                        error);
          if (!object_info->object)
  ...
  else if (strcmp (parent_info->tag.name, "child") == 0)
    {
      ...
      _ctk_builder_add (data->builder, child_info);
  ...
  if (!ctk_buildable_custom_tag_start (CTK_BUILDABLE (object),
                                       data->builder,
                                       child,
                                       element_name,
                                       &parser,
               ...
               GError              **error)
{
  ...
  if (strcmp (element_name, "requires") == 0)
    ...
    _ctk_builder_menu_start (data, element_name, names, values, error);
...
const gchar *
_ctk_builder_parser_translate (const gchar *domain,
                               const gchar *context,
                               const gchar *text)
{
  const gchar *s;
             ...
             GError              **error)
{
  ...
  CTK_NOTE (BUILDER, g_message ("</%s>", element_name));
  ...
  if (strcmp (element_name, "requires") == 0)
          ...
          if (!CTK_CHECK_VERSION (req_info->major, req_info->minor, 0))
            {
              g_set_error (error,
                           ...
                           CTK_MAJOR_VERSION, CTK_MINOR_VERSION);
              _ctk_builder_prefix_error (data->builder, context, error);
           }
        }
           ...
           strcmp (element_name, "template") == 0)
    {
      ...
      if (CTK_IS_BUILDABLE (object_info->object) &&
          CTK_BUILDABLE_GET_IFACE (object_info->object)->parser_finished)
        data->finalizers = g_slist_prepend (data->finalizers, object_info->object);
      _ctk_builder_add_signals (data->builder, object_info->signals);
                                  ...
                                  GError      **error)
{
   ...
   * ctk_builder_get_translation_domain() without breaking the ABI or API
   */
  ...
  for (l = data->custom_finalizers; l; l = l->next)
    {
      ...
      ctk_buildable_custom_finished (CTK_BUILDABLE (sub->object),
                                     builder,
                                     sub->child,
                                     sub->tagname,
                                     sub->data);
