 ...
 * @Title: CtkIconTheme
 *
 * #CtkIconTheme provides a facility for looking up icons by name
 ...
 *   by the #CtkIconSize enumeration, or by custom sizes defined
 *   by ctk_icon_size_register(), while named icons are more flexible
 ...
 *
 * In many cases, named themes are used indirectly, via #CtkImage
...
static void         ctk_icon_theme_finalize   (GObject          *object);
static void         theme_dir_destroy         (IconThemeDir     *dir);
static void         theme_destroy              (IconTheme       *theme);
static CtkIconInfo *theme_lookup_icon         (IconTheme        *theme,
...
static void         theme_list_contexts       (IconTheme        *theme,
                                               GHashTable       *contexts);
static void         theme_subdir_load         (CtkIconTheme     *icon_theme,
                                               IconTheme        *theme,
                                               GKeyFile         *theme_file,
                                               gchar            *subdir);
static void         do_theme_change           (CtkIconTheme     *icon_theme);
static void         blow_themes               (CtkIconTheme     *icon_themes);
static gboolean     rescan_themes             (CtkIconTheme     *icon_themes);
static IconSuffix   theme_dir_get_icon_suffix (IconThemeDir     *dir,
                                               const gchar      *icon_name,
                                               gboolean         *has_icon_file);
static CtkIconInfo *icon_info_new             (IconThemeDirType  type,
                                               gint              dir_size,
                                               gint              dir_scale);
static CtkIconInfo *icon_info_new_builtin     (BuiltinIcon      *icon);
...
static void         remove_from_lru_cache     (CtkIconTheme     *icon_theme,
                                               CtkIconInfo      *icon_info);
static gboolean     icon_info_ensure_scale_and_pixbuf (CtkIconInfo* icon_info);
...
G_DEFINE_TYPE_WITH_PRIVATE (CtkIconTheme, ctk_icon_theme, G_TYPE_OBJECT)
 ...
 * Usually, you’ll want to use ctk_icon_theme_get_default()
 * or ctk_icon_theme_get_for_screen() rather than creating
 * a new icon theme object for scratch.
 * 
 * Returns: the newly created #CtkIconTheme object.
 *
 * Since: 2.4
 */
CtkIconTheme *
ctk_icon_theme_new (void)
{
  return g_object_new (CTK_TYPE_ICON_THEME, NULL);
}
 ...
 *
 * Returns: (transfer none): A unique #CtkIconTheme associated with
 ...
 */
CtkIconTheme *
ctk_icon_theme_get_default (void)
{
  return ctk_icon_theme_get_for_screen (cdk_screen_get_default ());
}
 ...
 * is usually a better choice than calling than ctk_icon_theme_new()
  ...
  if (!icon_theme)
    {
      ...
      ctk_icon_theme_set_screen (icon_theme, screen);
...
static void
ctk_icon_theme_class_init (CtkIconThemeClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize = ctk_icon_theme_finalize;
  ...
  signal_changed = g_signal_new (I_("changed"),
                                 ...
                                 G_SIGNAL_RUN_LAST,
                                 G_STRUCT_OFFSET (CtkIconThemeClass, changed),
                                 NULL, NULL,
                                 NULL,
                                 G_TYPE_NONE, 0);
}
                ...
                CtkIconTheme *icon_theme)
{
  ...
  ctk_icon_theme_set_screen (icon_theme, NULL);
...
static void
update_current_theme (CtkIconTheme *icon_theme)
{
...
static void
unset_screen (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv = icon_theme->priv;
  ...
  if (priv->screen)
    {
      ...
      display = cdk_screen_get_display (priv->screen);
...
static void
icon_info_uncached (CtkIconInfo *icon_info)
{
...
static void
ctk_icon_theme_init (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv;
  const gchar * const *xdg_data_dirs;
  int i, j;

  priv = ctk_icon_theme_get_instance_private (icon_theme);
...
{
  if (dir_mtime->cache)
    _ctk_icon_cache_unref (dir_mtime->cache);
...
theme_changed_idle (gpointer user_data)
{
  ...
  icon_theme = CTK_ICON_THEME (user_data);
  priv = icon_theme->priv;
...
static void
queue_theme_changed (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv = icon_theme->priv;
  ...
  if (!priv->theme_changed_idle)
    {
      priv->theme_changed_idle =
        cdk_threads_add_idle_full (CTK_PRIORITY_RESIZE - 2,
...
static void
do_theme_change (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv = icon_theme->priv;
...
static void
blow_themes (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv = icon_theme->priv;
...
static void
ctk_icon_theme_finalize (GObject *object)
{
  CtkIconTheme *icon_theme;
  CtkIconThemePrivate *priv;
  int i;

  icon_theme = CTK_ICON_THEME (object);
  priv = icon_theme->priv;
  ...
  G_OBJECT_CLASS (ctk_icon_theme_parent_class)->finalize (object);  
}

/**
 * ctk_icon_theme_set_search_path:
 * @icon_theme: a #CtkIconTheme
 ...
 */
void
ctk_icon_theme_set_search_path (CtkIconTheme *icon_theme,
                                const gchar  *path[],
                                gint          n_elements)
{
  CtkIconThemePrivate *priv;
  ...
  g_return_if_fail (CTK_IS_ICON_THEME (icon_theme));
...
/**
 * ctk_icon_theme_get_search_path:
 * @icon_theme: a #CtkIconTheme
 ...
 *
 * Gets the current search path. See ctk_icon_theme_set_search_path().
 *
 * Since: 2.4
 */
void
ctk_icon_theme_get_search_path (CtkIconTheme  *icon_theme,
                                gchar        **path[],
                                gint          *n_elements)
{
  CtkIconThemePrivate *priv;
...
/**
 * ctk_icon_theme_append_search_path:
 * @icon_theme: a #CtkIconTheme
 * @path: (type filename): directory name to append to the icon path
 * 
 * Appends a directory to the search path. 
 * See ctk_icon_theme_set_search_path(). 
 *
 * Since: 2.4
 */
void
ctk_icon_theme_append_search_path (CtkIconTheme *icon_theme,
                                   const gchar  *path)
{
  CtkIconThemePrivate *priv;
...
/**
 * ctk_icon_theme_prepend_search_path:
 * @icon_theme: a #CtkIconTheme
 ...
 */
void
ctk_icon_theme_prepend_search_path (CtkIconTheme *icon_theme,
                                    const gchar  *path)
{
  CtkIconThemePrivate *priv;
  gint i;
...
/**
 * ctk_icon_theme_add_resource_path:
 * @icon_theme: a #CtkIconTheme
 ...
 */
void
ctk_icon_theme_add_resource_path (CtkIconTheme *icon_theme,
                                  const gchar  *path)
{
  CtkIconThemePrivate *priv = NULL;
 ...
 * 
 * Sets the name of the icon theme that the #CtkIconTheme object uses
 * overriding system configuration. This function cannot be called
 * on the icon theme objects returned from ctk_icon_theme_get_default()
 * and ctk_icon_theme_get_for_screen().
 *
 * Since: 2.4
 */
void
ctk_icon_theme_set_custom_theme (CtkIconTheme *icon_theme,
...
static void
insert_theme (CtkIconTheme *icon_theme,
              const gchar  *theme_name)
{
  gint i;
  GList *l;
...
static void
add_unthemed_icon (CtkIconTheme *icon_theme,
                   const gchar  *dir,
                   const gchar  *file,
                   gboolean      is_resource)
{
...
static void
load_themes (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv;
  GDir *gdir;
  gint base;
  ...
  for (base = 0; base < icon_theme->priv->search_path_len; base++)
    {
      ...
      dir_mtime->cache = _ctk_icon_cache_new_for_path (dir);
 ...
 */
static void
ensure_lru_cache_space (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv = icon_theme->priv;
  GList *l;
...
static void
add_to_lru_cache (CtkIconTheme *icon_theme,
                  CtkIconInfo  *icon_info)
{
  CtkIconThemePrivate *priv = icon_theme->priv;
...
static void
ensure_in_lru_cache (CtkIconTheme *icon_theme,
                     CtkIconInfo  *icon_info)
{
  CtkIconThemePrivate *priv = icon_theme->priv;
  GList *l;
...
static void
remove_from_lru_cache (CtkIconTheme *icon_theme,
                       CtkIconInfo  *icon_info)
{
  CtkIconThemePrivate *priv = icon_theme->priv;
...
static CtkIconInfo *
real_choose_icon (CtkIconTheme       *icon_theme,
                  ...
                  CtkIconLookupFlags  flags)
{
  ...
  CtkIconInfo *unscaled_icon_info;
  ...
  if (flags & CTK_ICON_LOOKUP_NO_SVG)
    allow_svg = FALSE;
  else if (flags & CTK_ICON_LOOKUP_FORCE_SVG)
    allow_svg = TRUE;
  else
    allow_svg = priv->pixbuf_supports_svg;

  use_builtin = flags & CTK_ICON_LOOKUP_USE_BUILTIN;
      ...
      if (hIcon)
        {
          ...
          icon_info->cache_pixbuf = cdk_win32_icon_to_pixbuf_libctk_only (hIcon, NULL, NULL);
          DestroyIcon (hIcon);
        }
  ...
  if (icon_info)
    {
      ...
      icon_info->forced_size = (flags & CTK_ICON_LOOKUP_FORCE_SIZE) != 0;
...
static CtkIconInfo *
choose_icon (CtkIconTheme       *icon_theme,
             const gchar        *icon_names[],
             gint                size,
             gint                scale,
             CtkIconLookupFlags  flags)
{
  ...
  if (flags & CTK_ICON_LOOKUP_DIR_LTR)
    dir_suffix = "-ltr";
  else if (flags & CTK_ICON_LOOKUP_DIR_RTL)
    dir_suffix = "-rtl";
  else
    dir_suffix = NULL;
  ...
  if ((flags & CTK_ICON_LOOKUP_FORCE_REGULAR) && has_symbolic)
    {
      ...
      icon_info = real_choose_icon (icon_theme,
                                    ...
                                    size,
                                    scale,
                                    flags & ~(CTK_ICON_LOOKUP_FORCE_REGULAR | CTK_ICON_LOOKUP_FORCE_SYMBOLIC));

      g_ptr_array_free (new_names, TRUE);
    }
  else if ((flags & CTK_ICON_LOOKUP_FORCE_SYMBOLIC) && has_regular)
    {
      new_names = g_ptr_array_new_with_free_func (g_free);
      for (i = 0; icon_names[i]; i++)
        {
...
/**
 * ctk_icon_theme_lookup_icon:
 * @icon_theme: a #CtkIconTheme
 * @icon_name: the name of the icon to lookup
 * @size: desired icon size
 * @flags: flags modifying the behavior of the icon lookup
 * 
 * Looks up a named icon and returns a #CtkIconInfo containing
 * information such as the filename of the icon. The icon
 * can then be rendered into a pixbuf using
 * ctk_icon_info_load_icon(). (ctk_icon_theme_load_icon()
 * combines these two steps if all you need is the pixbuf.)
 *
 * When rendering on displays with high pixel densities you should not
 * use a @size multiplied by the scaling factor returned by functions
 * like cdk_window_get_scale_factor(). Instead, you should use
 * ctk_icon_theme_lookup_icon_for_scale(), as the assets loaded
 * for a given scaling factor may be different.
 * 
 * Returns: (nullable) (transfer full): a #CtkIconInfo object
 ...
 */
CtkIconInfo *
ctk_icon_theme_lookup_icon (CtkIconTheme       *icon_theme,
                            const gchar        *icon_name,
                            gint                size,
                            CtkIconLookupFlags  flags)
{
  ...
  g_return_val_if_fail ((flags & CTK_ICON_LOOKUP_NO_SVG) == 0 ||
                        (flags & CTK_ICON_LOOKUP_FORCE_SVG) == 0, NULL);

  CTK_NOTE (ICONTHEME, g_message ("looking up icon %s", icon_name));

  return ctk_icon_theme_lookup_icon_for_scale (icon_theme, icon_name,
                                               size, 1, flags);
}

/**
 * ctk_icon_theme_lookup_icon_for_scale:
 * @icon_theme: a #CtkIconTheme
 * @icon_name: the name of the icon to lookup
 * @size: desired icon size
 ...
 * #CtkIconInfo containing information such as the filename of the
 * icon. The icon can then be rendered into a pixbuf using
 * ctk_icon_info_load_icon(). (ctk_icon_theme_load_icon() combines
 ...
 */
CtkIconInfo *
ctk_icon_theme_lookup_icon_for_scale (CtkIconTheme       *icon_theme,
                                      const gchar        *icon_name,
                                      gint                size,
                                      gint                scale,
                                      CtkIconLookupFlags  flags)
{
  ...
  CTK_NOTE (ICONTHEME, g_message ("looking up icon %s for scale %d", icon_name, scale));

  if (flags & CTK_ICON_LOOKUP_GENERIC_FALLBACK)
    {
 ...
 */
CtkIconInfo *
ctk_icon_theme_choose_icon (CtkIconTheme       *icon_theme,
                            const gchar        *icon_names[],
                            gint                size,
                            CtkIconLookupFlags  flags)
{
...
/**
 * ctk_icon_theme_choose_icon_for_scale:
 * @icon_theme: a #CtkIconTheme
 ...
 * a #CtkIconInfo containing information such as the filename of the
 * icon. The icon can then be rendered into a pixbuf using
 * ctk_icon_info_load_icon(). (ctk_icon_theme_load_icon()
 * combines these two steps if all you need is the pixbuf.)
 *
 ...
 */
CtkIconInfo *
ctk_icon_theme_choose_icon_for_scale (CtkIconTheme       *icon_theme,
                                      const gchar        *icon_names[],
                                      gint                size,
                                      gint                scale,
                                      CtkIconLookupFlags  flags)
 ...
 * ctk_icon_theme_lookup_icon() followed by ctk_icon_info_load_icon().
 *
                          ...
                          GError              **error)
{
  ...
  return ctk_icon_theme_load_icon_for_scale (icon_theme, icon_name,
                                             size, 1, flags, error);
}

/**
 * ctk_icon_theme_load_icon_for_scale:
 * @icon_theme: a #CtkIconTheme
 * @icon_name: the name of the icon to lookup
 * @size: the desired icon size. The resulting icon may not be
 *     exactly this size; see ctk_icon_info_load_icon().
 ...
 * use ctk_icon_theme_lookup_icon() followed by
 * ctk_icon_info_load_icon().
 *
 * Note that you probably want to listen for icon theme changes and
 * update the icon. This is usually done by connecting to the
 * CtkWidget::style-set signal. If for some reason you do not want to
                                    ...
                                    GError             **error)
{
  ...
  icon_info = ctk_icon_theme_lookup_icon_for_scale (icon_theme, icon_name, size, scale,
                                                    flags | CTK_ICON_LOOKUP_USE_BUILTIN);
  if (!icon_info)
    {
      g_set_error (error, CTK_ICON_THEME_ERROR,  CTK_ICON_THEME_NOT_FOUND,
 ...
 */
cairo_surface_t *
ctk_icon_theme_load_surface (CtkIconTheme        *icon_theme,
                             const gchar         *icon_name,
                             gint                 size,
                             gint                 scale,
                             ...
                             GError             **error)
{
  ...
  surface = ctk_icon_info_load_surface (icon_info, for_window, error);
  g_object_unref (icon_info);
 ...
 */
gboolean 
ctk_icon_theme_has_icon (CtkIconTheme *icon_theme,
                         const gchar  *icon_name)
{
  CtkIconThemePrivate *priv;
  GList *l;

  g_return_val_if_fail (CTK_IS_ICON_THEME (icon_theme), FALSE);
  ...
  for (l = priv->dir_mtimes; l; l = l->next)
    {
      ...
      CtkIconCache *cache = dir_mtime->cache;
      
      if (cache && _ctk_icon_cache_has_icon (cache, icon_name))
        return TRUE;
    }
 ...
 */
gint *
ctk_icon_theme_get_icon_sizes (CtkIconTheme *icon_theme,
                               const gchar  *icon_name)
{
  GList *l, *d, *icons;
  GHashTable *sizes;
 ...
 * Also see ctk_icon_theme_list_contexts().
 *
 ...
 */
GList *
ctk_icon_theme_list_icons (CtkIconTheme *icon_theme,
                           const gchar  *context)
{
  CtkIconThemePrivate *priv;
  GHashTable *icons;
...
/**
 * ctk_icon_theme_list_contexts:
 * @icon_theme: a #CtkIconTheme
 *
 * Gets the list of contexts available within the current
 * hierarchy of icon themes.
 * See ctk_icon_theme_list_icons() for details about contexts.
 *
 * Returns: (element-type utf8) (transfer full): a #GList list
 ...
 */
GList *
ctk_icon_theme_list_contexts (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv;
  GHashTable *contexts;
  GList *list, *l;
...
/**
 * ctk_icon_theme_get_example_icon_name:
 * @icon_theme: a #CtkIconTheme
 * 
 ...
 */
gchar *
ctk_icon_theme_get_example_icon_name (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv;
  GList *l;
  IconTheme *theme;
...
static gboolean
rescan_themes (CtkIconTheme *icon_theme)
{
  CtkIconThemePrivate *priv;
  IconThemeDirMtime *dir_mtime;
  GList *d;
...
/**
 * ctk_icon_theme_rescan_if_needed:
 * @icon_theme: a #CtkIconTheme
 * 
 ...
 */
gboolean
ctk_icon_theme_rescan_if_needed (CtkIconTheme *icon_theme)
{
  gboolean retval;
...
{
  if (dir->cache)
    _ctk_icon_cache_unref (dir->cache);
  if (dir->icons)
                           ...
                           gboolean     *has_icon_file)
{
  ...
  if (dir->cache)
    {
      suffix = (IconSuffix)_ctk_icon_cache_get_icon_flags (dir->cache,
                                                           icon_name,
                                                           dir->subdir_index);
      ...
      if (icon_name_is_symbolic (icon_name))
        {
          ...
          symbolic_suffix = (IconSuffix)_ctk_icon_cache_get_icon_flags (dir->cache,
  ...
  CTK_NOTE (ICONTHEME, g_message ("get icon suffix%s: %u", dir->cache ? " (cached)" : "", suffix));
  ...
  while (l != NULL)
    {
      ...
      CTK_NOTE (ICONTHEME, g_message ("look up icon dir %s", dir->dir));
      suffix = theme_dir_get_icon_suffix (dir, icon_name, NULL);
  ...
  if (min_dir)
    {
      ...
      if (min_dir->cache)
        {
          icon_info->cache_pixbuf = _ctk_icon_cache_get_icon (min_dir->cache, icon_name,
  ...
  while (l != NULL)
    {
      ...
      if (context == dir->context ||
        ...
        {
          if (dir->cache)
            _ctk_icon_cache_add_icons (dir->cache, dir->subdir, icons);
          else
  ...
  for (l = theme->dirs; l; l = l->next)
    {
      ...
      if (dir->cache)
        {
          if (_ctk_icon_cache_has_icon (dir->cache, icon_name))
            return TRUE;
        }
      else
        {
                ...
                gchar               *full_dir)
{
  ...
  CTK_NOTE (ICONTHEME, g_message ("scanning directory %s", full_dir));
                ...
                gchar                *full_dir)
{
  ...
  CTK_NOTE (ICONTHEME, g_message ("scanning resources %s", full_dir));
...
static void
theme_subdir_load (CtkIconTheme *icon_theme,
                   IconTheme    *theme,
                   GKeyFile     *theme_file,
                   gchar        *subdir)
{
      ...
      if (dir_mtime->cache != NULL || g_file_test (full_dir, G_FILE_TEST_IS_DIR))
        {
          if (dir_mtime->cache == NULL)
            {
              ...
              dir_mtime->cache = _ctk_icon_cache_new_for_path (dir_mtime->dir);
            }
          ...
          if (dir_mtime->cache != NULL)
            {
              dir->cache = _ctk_icon_cache_ref (dir_mtime->cache);
              dir->subdir_index = _ctk_icon_cache_get_directory_index (dir->cache, dir->subdir);
              has_icons = _ctk_icon_cache_has_icons (dir->cache, dir->subdir);
            }
          else
            {
              dir->cache = NULL;
...
static void ctk_icon_info_class_init (CtkIconInfoClass *klass);

G_DEFINE_TYPE (CtkIconInfo, ctk_icon_info, G_TYPE_OBJECT)

static void
ctk_icon_info_init (CtkIconInfo *icon_info)
{
  icon_info->scale = -1.;
}
               ...
               gint             dir_scale)
{
  ...
  icon_info = g_object_new (CTK_TYPE_ICON_INFO, NULL);
 ...
 */
static CtkIconInfo *
icon_info_dup (CtkIconInfo *icon_info)
{
  CtkIconInfo *dup;
  GSList *l;
...
{
  CtkIconInfo *icon_info = icon_info_new (ICON_THEME_DIR_THRESHOLD, icon->size, 1);
...
/**
 * ctk_icon_info_copy: (skip)
 * @icon_info: a #CtkIconInfo
 * 
 * Make a copy of a #CtkIconInfo.
 * 
 * Returns: (transfer full): the new CtkIconInfo
 *
 * Since: 2.4
 *
 ...
 */
CtkIconInfo *
ctk_icon_info_copy (CtkIconInfo *icon_info)
{
...
/**
 * ctk_icon_info_free: (skip)
 * @icon_info: a #CtkIconInfo
 * 
 * Free a #CtkIconInfo and associated information
 *
 * Since: 2.4
 *
 ...
 */
void
ctk_icon_info_free (CtkIconInfo *icon_info)
{
...
static void
ctk_icon_info_finalize (GObject *object)
{
  CtkIconInfo *icon_info = (CtkIconInfo *) object;
  ...
  G_OBJECT_CLASS (ctk_icon_info_parent_class)->finalize (object);
}

static void
ctk_icon_info_class_init (CtkIconInfoClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize = ctk_icon_info_finalize;
}

/**
 * ctk_icon_info_get_base_size:
 * @icon_info: a #CtkIconInfo
 * 
 ...
 */
gint
ctk_icon_info_get_base_size (CtkIconInfo *icon_info)
{
 ...
 */
gint
ctk_icon_info_get_base_scale (CtkIconInfo *icon_info)
{
  g_return_val_if_fail (icon_info != NULL, 0);
...
/**
 * ctk_icon_info_get_filename:
 * @icon_info: a #CtkIconInfo
 * 
 * Gets the filename for the icon. If the %CTK_ICON_LOOKUP_USE_BUILTIN
 * flag was passed to ctk_icon_theme_lookup_icon(), there may be no
 * filename if a builtin icon is returned; in this case, you should
 * use ctk_icon_info_get_builtin_pixbuf().
 * 
 ...
 */
const gchar *
ctk_icon_info_get_filename (CtkIconInfo *icon_info)
{
  g_return_val_if_fail (icon_info != NULL, NULL);
 ...
 */
gboolean
ctk_icon_info_is_symbolic (CtkIconInfo *icon_info)
{
  gchar *icon_uri;
  gboolean is_symbolic;

  g_return_val_if_fail (CTK_IS_ICON_INFO (icon_info), FALSE);
                         ...
                         CtkIconInfo *info)
{
  ...
  for (l = info->emblem_infos, pos = 0; l; l = l->next, pos++)
    {
      CtkIconInfo *emblem_info = l->data;
 ...
 */
static gboolean
icon_info_get_pixbuf_ready (CtkIconInfo *icon_info)
{
  if (icon_info->pixbuf &&
 ...
 */
static gboolean
icon_info_ensure_scale_and_pixbuf (CtkIconInfo *icon_info)
{
 ...
 * by passing the %CTK_ICON_LOOKUP_FORCE_SIZE flag when obtaining
 * the #CtkIconInfo. If this flag has been specified, the pixbuf
                            ...
                            GError      **error)
{
  ...
  surface = cdk_cairo_surface_create_from_pixbuf (pixbuf, icon_info->desired_scale, for_window);
  g_object_unref (pixbuf);
 ...
 * from the icon theme using ctk_icon_theme_lookup_icon().
 *
 * For more details, see ctk_icon_info_load_icon() which is the synchronous
 * version of this call.
 *
 * Since: 3.8
 */
void
ctk_icon_info_load_icon_async (CtkIconInfo         *icon_info,
                               GCancellable        *cancellable,
...
/**
 * ctk_icon_info_load_icon_finish:
 * @icon_info: a #CtkIconInfo from ctk_icon_theme_lookup_icon()
 * @res: a #GAsyncResult
 * @error: (allow-none): location to store error information on failure,
 *     or %NULL.
 *
 * Finishes an async icon load, see ctk_icon_info_load_icon_async().
 *
 * Returns: (transfer full): the rendered icon; this may be a newly
 *     created icon or a new reference to an internal icon, so you must
 *     not modify the icon. Use g_object_unref() to release your reference
                                ...
                                GError       **error)
{
  ...
  return ctk_icon_info_load_icon (icon_info, error);
}
...
rgba_to_string_noalpha (const CdkRGBA *rgba)
{
  ...
  return cdk_rgba_to_string (&color);
}
                                      ...
                                      const CdkRGBA *error_color)
{
  ...
  colored = cdk_pixbuf_new (GDK_COLORSPACE_RGB, TRUE, 8, width, height);
                                 ...
                                 GError        **error)
{
  ...
  return ctk_icon_theme_color_symbolic_pixbuf (icon_info->pixbuf,
                                               fg ? fg : &fg_default,
				      ...
				      GError        **error)
{
  ...
  if (g_str_has_suffix (icon_uri, ".symbolic.png"))
    pixbuf = ctk_icon_info_load_symbolic_png (icon_info, fg, success_color, warning_color, error_color, error);
  else
    pixbuf = ctk_icon_info_load_symbolic_svg (icon_info, fg, success_color, warning_color, error_color, error);
 ...
 * one, the function will return the result from ctk_icon_info_load_icon().
 *
                             ...
                             GError        **error)
{
  ...
  is_symbolic = ctk_icon_info_is_symbolic (icon_info);
  ...
  return ctk_icon_info_load_symbolic_internal (icon_info,
                                               fg, success_color,
...
/**
 * ctk_icon_info_load_symbolic_for_context:
 * @icon_info: a #CtkIconInfo
                                         ...
                                         GError          **error)
{
  ...
  ctk_icon_theme_lookup_symbolic_colors (ctk_style_context_lookup_style (context),
                                         &fg, &success_color,
                                         &warning_color, &error_color);
 ...
 * from the icon theme using ctk_icon_theme_lookup_icon().
 *
 * For more details, see ctk_icon_info_load_symbolic() which is the synchronous
 * version of this call.
 *
 * Since: 3.8
 */
                                   ...
                                   gpointer              user_data)
{
  ...
  data->is_symbolic = ctk_icon_info_is_symbolic (icon_info);

  if (!data->is_symbolic)
    {
      ctk_icon_info_load_icon_async (icon_info, cancellable, async_load_no_symbolic_cb, g_object_ref (task));
    }
  else
    {
      symbolic_cache = symbolic_pixbuf_cache_matches (icon_info->symbolic_pixbuf_cache,
...
/**
 * ctk_icon_info_load_symbolic_finish:
 * @icon_info: a #CtkIconInfo from ctk_icon_theme_lookup_icon()
 * @res: a #GAsyncResult
 * @was_symbolic: (out) (allow-none): a #gboolean, returns whether the
 *     loaded icon was a symbolic one and whether the @fg color was
 ...
 *     or %NULL.
 *
 * Finishes an async icon load, see ctk_icon_info_load_symbolic_async().
 *
 * Returns: (transfer full): the rendered icon; this may be a newly
 *     created icon or a new reference to an internal icon, so you must
 *     not modify the icon. Use g_object_unref() to release your reference
...
/**
 * ctk_icon_info_load_symbolic_for_context_async:
 * @icon_info: a #CtkIconInfo from ctk_icon_theme_lookup_icon()
 * @context: a #CtkStyleContext
 * @cancellable: (allow-none): optional #GCancellable object,
 *     %NULL to ignore
 ...
 * looked up from the icon theme using ctk_icon_theme_lookup_icon().
 *
 * For more details, see ctk_icon_info_load_symbolic_for_context()
 * which is the synchronous version of this call.
 *
 * Since: 3.8
 */
void
ctk_icon_info_load_symbolic_for_context_async (CtkIconInfo         *icon_info,
                                               CtkStyleContext     *context,
                                               GCancellable        *cancellable,
                                               GAsyncReadyCallback  callback,
                                               gpointer             user_data)
{
  ...
  ctk_icon_info_load_symbolic_async (icon_info,
                                     &fg, &success_color,
                                     &warning_color, &error_color,
...
/**
 * ctk_icon_info_load_symbolic_for_context_finish:
 * @icon_info: a #CtkIconInfo from ctk_icon_theme_lookup_icon()
 * @res: a #GAsyncResult
 * @was_symbolic: (out) (allow-none): a #gboolean, returns whether the
 *     loaded icon was a symbolic one and whether the @fg color was
 ...
 *     or %NULL.
 *
 * Finishes an async icon load, see ctk_icon_info_load_symbolic_for_context_async().
 *
 * Returns: (transfer full): the rendered icon; this may be a newly
 *     created icon or a new reference to an internal icon, so you must
 *     not modify the icon. Use g_object_unref() to release your reference
...
/**
 * ctk_icon_info_load_symbolic_for_style:
 * @icon_info: a #CtkIconInfo
 * @style: a #CtkStyle to take the colors from
                                       ...
                                       GError       **error)
{
  ...
  if (ctk_style_lookup_color (style, "success_color", &color))
    success_colorp = color_to_rgba (&color, &success_color);

  if (ctk_style_lookup_color (style, "warning_color", &color))
    warning_colorp = color_to_rgba (&color, &warning_color);

  if (ctk_style_lookup_color (style, "error_color", &color))
...
/**
 * ctk_icon_info_set_raw_coordinates:
 * @icon_info: a #CtkIconInfo
 * @raw_coordinates: whether the coordinates of embedded rectangles
 *     and attached points should be returned in their original
 *     (unscaled) form.
 * 
 * Sets whether the coordinates returned by ctk_icon_info_get_embedded_rect()
 * and ctk_icon_info_get_attach_points() should be returned in their
 * original form as specified in the icon theme, instead of scaled
 * appropriately for the pixbuf returned by ctk_icon_info_load_icon().
 *
 * Raw coordinates are somewhat strange; they are specified to be with
 * respect to the unscaled pixmap for PNG and XPM icons, but for SVG
 * icons, they are in a 1000x1000 coordinate space that is scaled
 * to the final size of the icon.  You can determine if the icon is an SVG
 * icon by using ctk_icon_info_get_filename(), and seeing if it is non-%NULL
 * and ends in “.svg”.
 *
...
/**
 * ctk_icon_info_get_attach_points:
 * @icon_info: a #CtkIconInfo
...
/**
 * ctk_icon_info_get_display_name:
 * @icon_info: a #CtkIconInfo
 * 
...
/**
 * ctk_icon_theme_add_builtin_icon:
 ...
 * Since: 2.4
 *
 * Deprecated: 3.14: Use ctk_icon_theme_add_resource_path()
...
/**
 * ctk_icon_theme_lookup_by_gicon:
 * @icon_theme: a #CtkIconTheme
 * @icon: the #GIcon to look up
 * @size: desired icon size
 * @flags: flags modifying the behavior of the icon lookup
 * 
 * Looks up an icon and returns a #CtkIconInfo containing information
 * such as the filename of the icon. The icon can then be rendered
 * into a pixbuf using ctk_icon_info_load_icon().
 *
 * When rendering on displays with high pixel densities you should not
 * use a @size multiplied by the scaling factor returned by functions
 * like cdk_window_get_scale_factor(). Instead, you should use
 * ctk_icon_theme_lookup_by_gicon_for_scale(), as the assets loaded
 * for a given scaling factor may be different.
 *
 * Returns: (nullable) (transfer full): a #CtkIconInfo containing
 ...
 */
CtkIconInfo *
ctk_icon_theme_lookup_by_gicon (CtkIconTheme       *icon_theme,
                                GIcon              *icon,
                                gint                size,
                                CtkIconLookupFlags  flags)
{
  return ctk_icon_theme_lookup_by_gicon_for_scale (icon_theme, icon,
                                                   size, 1, flags);
}


/**
 * ctk_icon_theme_lookup_by_gicon_for_scale:
 * @icon_theme: a #CtkIconTheme
 * @icon: the #GIcon to look up
 * @size: desired icon size
 * @scale: the desired scale
 ...
 * a pixbuf using ctk_icon_info_load_icon().
 *
 * Returns: (nullable) (transfer full): a #CtkIconInfo containing
 *     information about the icon, or %NULL if the icon wasn’t
 *     found. Unref with g_object_unref()
 ...
 */
CtkIconInfo *
ctk_icon_theme_lookup_by_gicon_for_scale (CtkIconTheme       *icon_theme,
                                          GIcon              *icon,
                                          gint                size,
                                          gint                scale,
                                          CtkIconLookupFlags  flags)
  ...
  if (GDK_IS_PIXBUF (icon))
    {
      ...
      if ((flags & CTK_ICON_LOOKUP_FORCE_SIZE) != 0)
        ...
        {
          info = ctk_icon_info_new_for_pixbuf (icon_theme, pixbuf);
        }
  ...
  else if (G_IS_FILE_ICON (icon))
    {
      ...
      info = ctk_icon_info_new_for_file (file, size, scale);
      info->forced_size = (flags & CTK_ICON_LOOKUP_FORCE_SIZE) != 0;
  ...
  else if (G_IS_THEMED_ICON (icon))
    {
      ...
      info = ctk_icon_theme_choose_icon_for_scale (icon_theme, names, size, scale, flags);
  ...
  else if (G_IS_EMBLEMED_ICON (icon))
    {
      GIcon *base, *emblem;
      GList *list, *l;
      CtkIconInfo *base_info, *emblem_info;

G_GNUC_BEGIN_IGNORE_DEPRECATIONS
      if (CTK_IS_NUMERABLE_ICON (icon))
        _ctk_numerable_icon_set_background_icon_size (CTK_NUMERABLE_ICON (icon), size / 2);
G_GNUC_END_IGNORE_DEPRECATIONS

      base = g_emblemed_icon_get_icon (G_EMBLEMED_ICON (icon));
      base_info = ctk_icon_theme_lookup_by_gicon_for_scale (icon_theme, base, size, scale, flags);
      if (base_info)
        {
          ...
          for (l = list; l; l = l->next)
            {
              ...
              emblem_info = ctk_icon_theme_lookup_by_gicon_for_scale (icon_theme, emblem, size / 2, scale, flags | CTK_ICON_LOOKUP_FORCE_SIZE);
              if (emblem_info)
...
CtkIconInfo *
ctk_icon_info_new_for_file (GFile *file,
                            gint   size,
                            gint   scale)
{
  CtkIconInfo *info;
