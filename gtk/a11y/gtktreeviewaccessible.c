...
static void ctk_cell_accessible_parent_interface_init (GtkCellAccessibleParentIface *iface);

G_DEFINE_TYPE_WITH_CODE (GtkTreeViewAccessible, ctk_tree_view_accessible, GTK_TYPE_CONTAINER_ACCESSIBLE,
                         ...
                         G_IMPLEMENT_INTERFACE (GTK_TYPE_CELL_ACCESSIBLE_PARENT, ctk_cell_accessible_parent_interface_init))
...
{
  ctk_accessible_set_widget (GTK_ACCESSIBLE (cell_info->cell), NULL);
                                     ...
                                     gpointer   data)
{
  ...
  if (tree_model)
    {
      if (ctk_tree_model_get_flags (tree_model) & GTK_TREE_MODEL_LIST_ONLY)
                                     ...
                                     GParamSpec *pspec)
{
  ...
  accessible = GTK_TREE_VIEW_ACCESSIBLE (ctk_widget_get_accessible (widget));
  tree_view = GTK_TREE_VIEW (widget);
  ...
  if (g_strcmp0 (pspec->name, "model") == 0)
    ...
    GTK_WIDGET_ACCESSIBLE_CLASS (ctk_tree_view_accessible_parent_class)->notify_gtk (obj, pspec);
}
...
ctk_tree_view_accessible_widget_unset (GtkAccessible *gtkaccessible)
{
  ...
  GTK_ACCESSIBLE_CLASS (ctk_tree_view_accessible_parent_class)->widget_unset (gtkaccessible);
}
...
get_n_columns (GtkTreeView *tree_view)
{
  ...
  for (i = 0; i < ctk_tree_view_get_n_columns (tree_view); i++)
    {
      GtkTreeViewColumn *column = ctk_tree_view_get_column (tree_view, i);
                                     ...
                                     AtkObject       *parent)
{
  ...
  cell = GTK_CELL_ACCESSIBLE (ctk_renderer_cell_accessible_new (renderer));
                        ...
                        GtkTreeViewColumn     *column)
{
  ...
  renderer_list = ctk_cell_layout_get_cells (GTK_CELL_LAYOUT (column));
  ...
  if (g_list_length (renderer_list) == 1)
  ...
  else
    {
      ...
      _ctk_cell_accessible_initialize (GTK_CELL_ACCESSIBLE (container), GTK_WIDGET (treeview), ATK_OBJECT (accessible));
...
static void
ctk_tree_view_accessible_class_init (GtkTreeViewAccessibleClass *klass)
{
...
static void
ctk_tree_view_accessible_init (GtkTreeViewAccessible *view)
{
                                       ...
                                       gint      column)
{
  ...
  widget = ctk_accessible_get_widget (GTK_ACCESSIBLE (table));
  if (widget == NULL)
    return -1;
                                          ...
                                          gint      row)
{
  ...
  if (!_ctk_rbtree_find_index (_ctk_tree_view_get_rbtree (GTK_TREE_VIEW (widget)),
                               row,
                               &tree,
                               &node))
    return FALSE;
                                            ...
                                            gint      row)
{
  ...
  treeview = GTK_TREE_VIEW (ctk_accessible_get_widget (GTK_ACCESSIBLE (table)));
  if (treeview == NULL)
    return FALSE;
...
static void
ctk_tree_view_accessible_get_cell_area (GtkCellAccessibleParent *parent,
                                        GtkCellAccessible       *cell,
                                        GdkRectangle            *cell_rect)
{
  GtkWidget *widget;
      ...
      if (top_cell != cell)
        {
          ...
          renderers = ctk_cell_layout_get_cells (GTK_CELL_LAYOUT (tv_col));
...
static void
ctk_tree_view_accessible_get_cell_extents (GtkCellAccessibleParent *parent,
                                           GtkCellAccessible       *cell,
                                           gint                    *x,
                                           gint                    *y,
                                           gint                    *width,
...
static gboolean
ctk_tree_view_accessible_grab_cell_focus (GtkCellAccessibleParent *parent,
                                          GtkCellAccessible       *cell)
{
  GtkWidget *widget;
  GtkTreeView *tree_view;
...
static int
ctk_tree_view_accessible_get_child_index (GtkCellAccessibleParent *parent,
                                          GtkCellAccessible       *cell)
{
  GtkTreeViewAccessibleCellInfo *cell_info;
  GtkTreeView *tree_view;
  ...
  tree_view = GTK_TREE_VIEW (ctk_accessible_get_widget (GTK_ACCESSIBLE (parent)));

  return cell_info_get_index (tree_view, cell_info);
}

static GtkCellRendererState
ctk_tree_view_accessible_get_renderer_state (GtkCellAccessibleParent *parent,
                                             GtkCellAccessible       *cell)
{
  GtkTreeViewAccessibleCellInfo *cell_info;
  GtkTreeView *treeview;
  ...
  treeview = GTK_TREE_VIEW (ctk_accessible_get_widget (GTK_ACCESSIBLE (parent)));
  ...
  if (ctk_widget_has_focus (GTK_WIDGET (treeview)))
    {
      GtkTreeViewColumn *column;
      GtkTreePath *path;
      GtkRBTree *tree;
...
static void
ctk_tree_view_accessible_expand_collapse (GtkCellAccessibleParent *parent,
                                          GtkCellAccessible       *cell)
{
  GtkTreeViewAccessibleCellInfo *cell_info;
  GtkTreeView *treeview;
...
static void
ctk_tree_view_accessible_activate (GtkCellAccessibleParent *parent,
                                   GtkCellAccessible       *cell)
{
  GtkTreeViewAccessibleCellInfo *cell_info;
  GtkTreeView *treeview;
...
static void
ctk_tree_view_accessible_edit (GtkCellAccessibleParent *parent,
                               GtkCellAccessible       *cell)
{
  GtkTreeView *treeview;
...
static void
ctk_tree_view_accessible_update_relationset (GtkCellAccessibleParent *parent,
                                             GtkCellAccessible       *cell,
...
static void
ctk_tree_view_accessible_get_cell_position (GtkCellAccessibleParent *parent,
                                            GtkCellAccessible       *cell,
                                            gint                    *row,
                                            gint                    *column)
{
...
static GPtrArray *
ctk_tree_view_accessible_get_column_header_cells (GtkCellAccessibleParent *parent,
                                                  GtkCellAccessible       *cell)
{
  GtkWidget *widget;
  GtkTreeViewAccessibleCellInfo *cell_info;
...
static void
ctk_cell_accessible_parent_interface_init (GtkCellAccessibleParentIface *iface)
{
...
void
_ctk_tree_view_accessible_reorder (GtkTreeView *treeview)
{
  GtkTreeViewAccessible *accessible;

  accessible = GTK_TREE_VIEW_ACCESSIBLE (_ctk_widget_peek_accessible (GTK_WIDGET (treeview)));
  if (accessible == NULL)
    return;
...
void
_ctk_tree_view_accessible_add (GtkTreeView *treeview,
                               GtkRBTree   *tree,
                               GtkRBNode   *node)
{
  GtkTreeViewAccessible *accessible;
...
void
_ctk_tree_view_accessible_remove (GtkTreeView *treeview,
                                  GtkRBTree   *tree,
                                  GtkRBNode   *node)
{
  GtkTreeViewAccessibleCellInfo *cell_info;
...
void
_ctk_tree_view_accessible_changed (GtkTreeView *treeview,
                                   GtkRBTree   *tree,
                                   GtkRBNode   *node)
{
  GtkTreeViewAccessible *accessible;
  ...
  accessible = GTK_TREE_VIEW_ACCESSIBLE (ctk_widget_get_accessible (GTK_WIDGET (treeview)));
                      ...
                      guint        id)
{
  ...
  for (i = 0; i < id; i++)
    {
      GtkTreeViewColumn *column = ctk_tree_view_get_column (treeview, i);
...
static void
ctk_tree_view_accessible_do_add_column (GtkTreeViewAccessible *accessible,
                                        GtkTreeView           *treeview,
                                        GtkTreeViewColumn     *column,
                                        guint                  id)
{
...
void
_ctk_tree_view_accessible_add_column (GtkTreeView       *treeview,
                                      GtkTreeViewColumn *column,
                                      guint              id)
{
  AtkObject *obj;
  ...
  obj = _ctk_widget_peek_accessible (GTK_WIDGET (treeview));
  if (obj == NULL)
    return;

  ctk_tree_view_accessible_do_add_column (GTK_TREE_VIEW_ACCESSIBLE (obj),
                                          treeview,
                                          column,
...
static void
ctk_tree_view_accessible_do_remove_column (GtkTreeViewAccessible *accessible,
                                           GtkTreeView           *treeview,
                                           GtkTreeViewColumn     *column,
                                           guint                  id)
{
...
void
_ctk_tree_view_accessible_remove_column (GtkTreeView       *treeview,
                                         GtkTreeViewColumn *column,
                                         guint              id)
{
  AtkObject *obj;
  ...
  ctk_tree_view_accessible_do_remove_column (GTK_TREE_VIEW_ACCESSIBLE (obj),
                                             treeview,
                                             column,
                                             to_visible_column_id (treeview, id));
}
...
void
_ctk_tree_view_accessible_reorder_column (GtkTreeView       *treeview,
                                          GtkTreeViewColumn *column)
{
  AtkObject *obj;
...
void
_ctk_tree_view_accessible_toggle_visibility (GtkTreeView       *treeview,
                                             GtkTreeViewColumn *column)
{
  AtkObject *obj;
  guint i, id;
  ...
  if (ctk_tree_view_column_get_visible (column))
  ...
  else
    {
      ...
      for (i = 0; i < ctk_tree_view_get_n_columns (treeview); i++)
        {
          GtkTreeViewColumn *cur = ctk_tree_view_get_column (treeview, i);
...
void
_ctk_tree_view_accessible_update_focus_column (GtkTreeView       *treeview,
  ...
  if (old_focus)
    {
      ...
      if (cell != NULL)
        _ctk_cell_accessible_state_changed (cell, GTK_CELL_RENDERER_FOCUSED, 0);
    }
  ...
  if (new_focus)
    {
      ...
      if (cell != NULL)
        _ctk_cell_accessible_state_changed (cell, 0, GTK_CELL_RENDERER_FOCUSED);
      else
...
void
_ctk_tree_view_accessible_add_state (GtkTreeView          *treeview,
                                     GtkRBTree            *tree,
                                     GtkRBNode            *node,
...
void
_ctk_tree_view_accessible_remove_state (GtkTreeView          *treeview,
                                        GtkRBTree            *tree,
                                        GtkRBNode            *node,
                                        GtkCellRendererState  state)
{
