 ...
 * |[<!-- language="C" -->
 * GtkWidget *menu_item = ctk_menu_item_new_with_label ("Example Menu Item");
 *
 * GtkWidget *child = ctk_bin_get_child (GTK_BIN (menu_item));
 * ctk_label_set_markup (GTK_LABEL (child), "<i>new label</i> with <b>markup</b>");
 * ctk_accel_label_set_accel (GTK_ACCEL_LABEL (child), GDK_KEY_1, 0);
 * ]|
 *
...
static void ctk_menu_item_get_property   (GObject          *object,
                                          ...
                                          GValue           *value,
                                          GParamSpec       *pspec);
static void ctk_menu_item_destroy        (GtkWidget        *widget);
static void ctk_menu_item_realize        (GtkWidget        *widget);
static void ctk_menu_item_unrealize      (GtkWidget        *widget);
static void ctk_menu_item_map            (GtkWidget        *widget);
static void ctk_menu_item_unmap          (GtkWidget        *widget);
static gboolean ctk_menu_item_enter      (GtkWidget        *widget,
                                          GdkEventCrossing *event);
static gboolean ctk_menu_item_leave      (GtkWidget        *widget,
                                          GdkEventCrossing *event);
static void ctk_menu_item_parent_set     (GtkWidget        *widget,
                                          GtkWidget        *previous_parent);
static void ctk_menu_item_direction_changed (GtkWidget        *widget,
                                             GtkTextDirection  previous_dir);


static void ctk_real_menu_item_select               (GtkMenuItem *item);
static void ctk_real_menu_item_deselect             (GtkMenuItem *item);
static void ctk_real_menu_item_activate             (GtkMenuItem *item);
static void ctk_real_menu_item_activate_item        (GtkMenuItem *item);
static void ctk_real_menu_item_toggle_size_request  (GtkMenuItem *menu_item,
                                                     gint        *requisition);
static void ctk_real_menu_item_toggle_size_allocate (GtkMenuItem *menu_item,
                                                     gint         allocation);
static gboolean ctk_menu_item_mnemonic_activate     (GtkWidget   *widget,
                                                     gboolean     group_cycling);

static void ctk_menu_item_ensure_label   (GtkMenuItem      *menu_item);
static gint ctk_menu_item_popup_timeout  (gpointer          data);
static void ctk_menu_item_show_all       (GtkWidget        *widget);

static void ctk_menu_item_forall         (GtkContainer    *container,
                                          gboolean         include_internals,
                                          GtkCallback      callback,
                                          gpointer         callback_data);

static gboolean ctk_menu_item_can_activate_accel (GtkWidget *widget,
                                                  guint      signal_id);

static void ctk_real_menu_item_set_label (GtkMenuItem     *menu_item,
                                          const gchar     *label);
static const gchar * ctk_real_menu_item_get_label (GtkMenuItem *menu_item);

static void ctk_menu_item_buildable_interface_init (GtkBuildableIface   *iface);
static void ctk_menu_item_buildable_add_child      (GtkBuildable        *buildable,
                                                    GtkBuilder          *builder,
                                                    GObject             *child,
                                                    const gchar         *type);
static void ctk_menu_item_buildable_custom_finished(GtkBuildable        *buildable,
                                                    GtkBuilder          *builder,
                                                    GObject             *child,
                                                    const gchar         *tagname,
                                                    gpointer             user_data);

static void ctk_menu_item_actionable_interface_init  (GtkActionableInterface *iface);
static void ctk_menu_item_activatable_interface_init (GtkActivatableIface  *iface);
static void ctk_menu_item_update                     (GtkActivatable       *activatable,
                                                      GtkAction            *action,
                                                      const gchar          *property_name);
static void ctk_menu_item_sync_action_properties     (GtkActivatable       *activatable,
                                                      GtkAction            *action);
static void ctk_menu_item_set_related_action         (GtkMenuItem          *menu_item, 
                                                      GtkAction            *action);
static void ctk_menu_item_set_use_action_appearance  (GtkMenuItem          *menu_item, 
...
G_DEFINE_TYPE_WITH_CODE (GtkMenuItem, ctk_menu_item, GTK_TYPE_BIN,
...
static void
ctk_menu_item_set_action_name (GtkActionable *actionable,
...
static void
ctk_menu_item_set_action_target_value (GtkActionable *actionable,
...
static const gchar *
ctk_menu_item_get_action_name (GtkActionable *actionable)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (actionable);
...
static GVariant *
ctk_menu_item_get_action_target_value (GtkActionable *actionable)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (actionable);
...
static void
ctk_menu_item_actionable_interface_init (GtkActionableInterface *iface)
{
...
static gboolean
ctk_menu_item_render (GtkCssGadget *gadget,
                      cairo_t      *cr,
                      int           x,
                      int           y,
                      int           width,
                      ...
                      gpointer      data)
{
  ...
  GTK_WIDGET_CLASS (ctk_menu_item_parent_class)->draw (widget, cr);

  return FALSE;
}

static gboolean
ctk_menu_item_draw (GtkWidget *widget,
                    cairo_t   *cr)
{
  ctk_css_gadget_draw (GTK_MENU_ITEM (widget)->priv->gadget, cr);

  return FALSE;
}

static void
ctk_menu_item_allocate (GtkCssGadget        *gadget,
                        ...
                        gpointer             data)
{
  ...
  if (GTK_IS_MENU_BAR (parent))
    {
      child_pack_dir = ctk_menu_bar_get_child_pack_direction (GTK_MENU_BAR (parent));
    }
  else
    {
  ...
  if (priv->submenu)
    ctk_menu_reposition (GTK_MENU (priv->submenu));
}

static void
ctk_menu_item_size_allocate (GtkWidget     *widget,
...
static void
ctk_menu_item_accel_width_foreach (GtkWidget *widget,
                                   gpointer   data)
{
  guint *width = data;
  ...
  if (GTK_IS_ACCEL_LABEL (widget))
    {
      ...
      w = ctk_accel_label_get_accel_width (GTK_ACCEL_LABEL (widget));
      *width = MAX (*width, w);
    }
...
static void
ctk_menu_item_real_get_width (GtkWidget *widget,
                              gint      *minimum_size,
                              gint      *natural_size)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  ...
  accel_width = 0;
  ctk_container_foreach (GTK_CONTAINER (menu_item),
                         ctk_menu_item_accel_width_foreach,
                         &accel_width);
...
static void
ctk_menu_item_real_get_height (GtkWidget *widget,
                               gint       for_size,
                               gint      *minimum_size,
                               gint      *natural_size)
{
...
static void
ctk_menu_item_measure (GtkCssGadget   *gadget,
...
static void
ctk_menu_item_get_preferred_width (GtkWidget *widget,
                                   gint      *minimum_size,
                                   gint      *natural_size)
{
  ctk_css_gadget_get_preferred_size (GTK_MENU_ITEM (widget)->priv->gadget,
                                     GTK_ORIENTATION_HORIZONTAL,
                                     -1,
...
static void
ctk_menu_item_get_preferred_height (GtkWidget *widget,
                                    gint      *minimum_size,
                                    gint      *natural_size)
{
  ctk_css_gadget_get_preferred_size (GTK_MENU_ITEM (widget)->priv->gadget,
...
static void
ctk_menu_item_get_preferred_height_for_width (GtkWidget *widget,
                                              gint       for_size,
                                              gint      *minimum_size,
                                              gint      *natural_size)
{
...
static void
ctk_menu_item_class_init (GtkMenuItemClass *klass)
{
  ...
  ctk_widget_class_set_accessible_type (widget_class, GTK_TYPE_MENU_ITEM_ACCESSIBLE);
...
static void
ctk_menu_item_init (GtkMenuItem *menu_item)
{
  ...
  ctk_widget_set_has_window (GTK_WIDGET (menu_item), FALSE);
  ...
  if (ctk_widget_get_direction (GTK_WIDGET (menu_item)) == GTK_TEXT_DIR_RTL)
  ...
  widget_node = ctk_widget_get_css_node (GTK_WIDGET (menu_item));
...
GtkCssGadget *
_ctk_menu_item_get_gadget (GtkMenuItem *menu_item)
{
  ...
  if (priv->action)
    {
      ...
      ctk_activatable_do_set_related_action (GTK_ACTIVATABLE (menu_item), NULL);
      G_GNUC_END_IGNORE_DEPRECATIONS;
      priv->action = NULL;
    }
...
static void
ctk_menu_item_do_set_right_justified (GtkMenuItem *menu_item,
                            ...
                            GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_ACTION_NAME:
      ctk_menu_item_set_action_name (GTK_ACTIONABLE (menu_item), g_value_get_string (value));
      break;
    case PROP_ACTION_TARGET:
      ctk_menu_item_set_action_target_value (GTK_ACTIONABLE (menu_item), g_value_get_variant (value));
      break;
    default:
...
static void
ctk_menu_item_destroy (GtkWidget *widget)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;
  ...
  GTK_WIDGET_CLASS (ctk_menu_item_parent_class)->destroy (widget);
}

static void
ctk_menu_item_detacher (GtkWidget *widget,
                        GtkMenu   *menu)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;
...
static void
ctk_menu_item_buildable_interface_init (GtkBuildableIface *iface)
{
...
static void
ctk_menu_item_buildable_add_child (GtkBuildable *buildable,
                                   GtkBuilder   *builder,
                                   GObject      *child,
                                   const gchar  *type)
{
  if (type && strcmp (type, "submenu") == 0)
        ctk_menu_item_set_submenu (GTK_MENU_ITEM (buildable),
                                   GTK_WIDGET (child));
  else
...
static void
ctk_menu_item_buildable_custom_finished (GtkBuildable *buildable,
                                         GtkBuilder   *builder,
                                         GObject      *child,
                                         const gchar  *tagname,
                                         gpointer      user_data)
  ...
  if (strcmp (tagname, "accelerator") == 0)
    {
      ...
      menu_shell = GTK_MENU_SHELL (ctk_widget_get_parent (GTK_WIDGET (buildable)));
      if (menu_shell)
        {
          while (GTK_IS_MENU (menu_shell) &&
                 (attach = ctk_menu_get_attach_widget (GTK_MENU (menu_shell))) != NULL)
            menu_shell = GTK_MENU_SHELL (ctk_widget_get_parent (attach));

          toplevel = ctk_widget_get_toplevel (GTK_WIDGET (menu_shell));
        }
      else
        {
          /* Fall back to something ... */
          toplevel = ctk_widget_get_toplevel (GTK_WIDGET (buildable));
...
static void
ctk_menu_item_activatable_interface_init (GtkActivatableIface *iface)
{
...
activatable_update_label (GtkMenuItem *menu_item, GtkAction *action)
{
  ...
  child = ctk_bin_get_child (GTK_BIN (menu_item));
 ...
 **/
static gboolean
ctk_menu_is_empty (GtkWidget *menu)
{
  GList *children, *cur;
...
static void
ctk_menu_item_update (GtkActivatable *activatable,
                      GtkAction      *action,
                      const gchar    *property_name)
{
  ...
  if (strcmp (property_name, "visible") == 0)
    {
      G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
      _ctk_action_sync_menu_visible (action, GTK_WIDGET (menu_item),
    ...
    {
      G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
      ctk_widget_set_sensitive (GTK_WIDGET (menu_item), ctk_action_is_sensitive (action));
      G_GNUC_END_IGNORE_DEPRECATIONS;
    }
...
static void
ctk_menu_item_sync_action_properties (GtkActivatable *activatable,
                                      GtkAction      *action)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (activatable);
  GtkMenuItemPrivate *priv = menu_item->priv;
  ...
  if (!priv->use_action_appearance || !action)
    {
      label = ctk_bin_get_child (GTK_BIN (menu_item));

      if (GTK_IS_ACCEL_LABEL (label))
        ctk_accel_label_set_accel_widget (GTK_ACCEL_LABEL (label), GTK_WIDGET (menu_item));
    }
  ...
  if (priv->use_action_appearance)
    {
      ...
      if (label && !GTK_IS_LABEL (label))
        {
          ctk_container_remove (GTK_CONTAINER (menu_item), label);
          label = NULL;
        }
      ...
      G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
      if (GTK_IS_ACCEL_LABEL (label) && ctk_action_get_accel_path (action))
        {
          ctk_accel_label_set_accel_widget (GTK_ACCEL_LABEL (label), NULL);
          ctk_accel_label_set_accel_closure (GTK_ACCEL_LABEL (label),
...
static void
ctk_menu_item_set_related_action (GtkMenuItem *menu_item,
                                  GtkAction   *action)
{
    GtkMenuItemPrivate *priv = menu_item->priv;
    ...
    ctk_activatable_do_set_related_action (GTK_ACTIVATABLE (menu_item), action);
...
static void
ctk_menu_item_set_use_action_appearance (GtkMenuItem *menu_item,
                                         gboolean     use_appearance)
{
    GtkMenuItemPrivate *priv = menu_item->priv;
    ...
    if (priv->use_action_appearance != use_appearance)
      {
        ...
        G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
        ctk_activatable_sync_action_properties (GTK_ACTIVATABLE (menu_item), priv->action);
        G_GNUC_END_IGNORE_DEPRECATIONS;
      }
}
...
update_node_classes (GtkMenuItem *menu_item)
{
  ...
  if (ctk_widget_get_direction (GTK_WIDGET (menu_item)) == GTK_TEXT_DIR_RTL)
    {
      ctk_css_node_add_class (arrow_node, g_quark_from_static_string (GTK_STYLE_CLASS_LEFT));
      ctk_css_node_remove_class (arrow_node, g_quark_from_static_string (GTK_STYLE_CLASS_RIGHT));
    ...
    {
      ctk_css_node_remove_class (arrow_node, g_quark_from_static_string (GTK_STYLE_CLASS_LEFT));
      ctk_css_node_add_class (arrow_node, g_quark_from_static_string (GTK_STYLE_CLASS_RIGHT));
...
update_arrow_gadget (GtkMenuItem *menu_item)
{
  ...
  should_have_gadget = priv->reserve_indicator ||
    (priv->submenu && !GTK_IS_MENU_BAR (ctk_widget_get_parent (widget)));
 ...
 */
void
ctk_menu_item_set_submenu (GtkMenuItem *menu_item,
                           GtkWidget   *submenu)
{
  GtkWidget *widget;
  GtkMenuItemPrivate *priv;
  ...
  if (priv->submenu != submenu)
    {
      if (priv->submenu)
        {
          ctk_menu_detach (GTK_MENU (priv->submenu));
          priv->submenu = NULL;
        }
      ...
      if (submenu)
        {
          ...
          ctk_menu_attach_to_widget (GTK_MENU (submenu),
                                     widget,
 ...
 */
GtkWidget *
ctk_menu_item_get_submenu (GtkMenuItem *menu_item)
{
...
void
_ctk_menu_item_set_placement (GtkMenuItem         *menu_item,
                             GtkSubmenuPlacement  placement)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
 ...
 */
void
ctk_menu_item_select (GtkMenuItem *menu_item)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
 ...
 */
void
ctk_menu_item_deselect (GtkMenuItem *menu_item)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
 ...
 */
void
ctk_menu_item_activate (GtkMenuItem *menu_item)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
 ...
 */
void
ctk_menu_item_toggle_size_request (GtkMenuItem *menu_item,
                                   gint        *requisition)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
 ...
 */
void
ctk_menu_item_toggle_size_allocate (GtkMenuItem *menu_item,
                                    gint         allocation)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
...
static void
ctk_menu_item_realize (GtkWidget *widget)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;
  GtkAllocation allocation;
...
static void
ctk_menu_item_unrealize (GtkWidget *widget)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;
  ...
  GTK_WIDGET_CLASS (ctk_menu_item_parent_class)->unrealize (widget);
}

static void
ctk_menu_item_map (GtkWidget *widget)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;

  GTK_WIDGET_CLASS (ctk_menu_item_parent_class)->map (widget);

  gdk_window_show (priv->event_window);
}

static void
ctk_menu_item_unmap (GtkWidget *widget)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;
  ...
  GTK_WIDGET_CLASS (ctk_menu_item_parent_class)->unmap (widget);
}

static gboolean
ctk_menu_item_enter (GtkWidget        *widget,
                     GdkEventCrossing *event)
{
...
static gboolean
ctk_menu_item_leave (GtkWidget        *widget,
                     GdkEventCrossing *event)
{
  g_return_val_if_fail (event != NULL, FALSE);
...
static void
ctk_real_menu_item_select (GtkMenuItem *menu_item)
{
  GtkMenuItemPrivate *priv = menu_item->priv;
  ...
  if ((!source_device ||
    ...
    {
      _ctk_menu_item_popup_submenu (GTK_WIDGET (menu_item), TRUE);
    }

  ctk_widget_set_state_flags (GTK_WIDGET (menu_item),
                              GTK_STATE_FLAG_PRELIGHT, FALSE);
  ctk_widget_queue_draw (GTK_WIDGET (menu_item));
}

static void
ctk_real_menu_item_deselect (GtkMenuItem *menu_item)
{
  GtkMenuItemPrivate *priv = menu_item->priv;

  if (priv->submenu)
    _ctk_menu_item_popdown_submenu (GTK_WIDGET (menu_item));

  ctk_widget_unset_state_flags (GTK_WIDGET (menu_item),
                                GTK_STATE_FLAG_PRELIGHT);
  ctk_widget_queue_draw (GTK_WIDGET (menu_item));
}

static gboolean
ctk_menu_item_mnemonic_activate (GtkWidget *widget,
                                 gboolean   group_cycling)
{
  GtkWidget *parent;
  ...
  if (GTK_IS_MENU_SHELL (parent))
    _ctk_menu_shell_set_keyboard_mode (GTK_MENU_SHELL (parent), TRUE);
  ...
  if (group_cycling &&
    ...
    {
      ctk_menu_shell_select_item (GTK_MENU_SHELL (parent), widget);
    }
  else
...
static void
ctk_real_menu_item_activate (GtkMenuItem *menu_item)
{
  GtkMenuItemPrivate *priv = menu_item->priv;
...
static void
ctk_real_menu_item_activate_item (GtkMenuItem *menu_item)
{
  GtkMenuItemPrivate *priv = menu_item->priv;
  GtkWidget *parent;
  GtkWidget *widget;
  ...
  if (parent && GTK_IS_MENU_SHELL (parent))
    {
      ...
      if (priv->submenu == NULL)
      ...
      else
        {
          ...
          ctk_menu_shell_select_first (GTK_MENU_SHELL (priv->submenu), TRUE);
        }
    }
}

static void
ctk_real_menu_item_toggle_size_request (GtkMenuItem *menu_item,
                                        gint        *requisition)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
...
static void
ctk_real_menu_item_toggle_size_allocate (GtkMenuItem *menu_item,
                                         gint         allocation)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
...
static void
ctk_real_menu_item_set_label (GtkMenuItem *menu_item,
                              const gchar *label)
{
  GtkWidget *child;
  ...
  if (GTK_IS_LABEL (child))
    {
      ctk_label_set_label (GTK_LABEL (child), label ? label : "");
...
static const gchar *
ctk_real_menu_item_get_label (GtkMenuItem *menu_item)
{
  GtkWidget *child;
  ...
  child = ctk_bin_get_child (GTK_BIN (menu_item));
  if (GTK_IS_LABEL (child))
    return ctk_label_get_label (GTK_LABEL (child));
...
{
  GtkWidget *parent = ctk_widget_get_parent (GTK_WIDGET (menu_item));
...
static void
ctk_menu_item_real_popup_submenu (GtkWidget      *widget,
  ...
  if (ctk_widget_is_sensitive (priv->submenu) && parent)
    {
      ...
      take_focus = ctk_menu_shell_get_take_focus (GTK_MENU_SHELL (parent));
      ctk_menu_shell_set_take_focus (GTK_MENU_SHELL (priv->submenu), take_focus);
      ...
      if (ctk_widget_get_window (widget))
        {
          switch (priv->submenu_placement)
            ...
            case GTK_TOP_BOTTOM:
              ...
              ctk_menu_popup_at_widget (GTK_MENU (priv->submenu),
                                        widget,
        ...
        ctk_menu_popup_at_pointer (GTK_MENU (priv->submenu), trigger_event);
    }
...
ctk_menu_item_popup_timeout (gpointer data)
{
  ...
  if ((GTK_IS_MENU_SHELL (parent) && GTK_MENU_SHELL (parent)->priv->active) ||
    ...
    {
      ctk_menu_item_real_popup_submenu (GTK_WIDGET (menu_item), info->trigger_event, TRUE);
...
get_popup_delay (GtkWidget *widget)
{
  ...
  parent = ctk_widget_get_parent (widget);
  if (GTK_IS_MENU_SHELL (parent))
    return _ctk_menu_shell_get_popup_delay (GTK_MENU_SHELL (parent));
  else
    return MENU_POPUP_DELAY;
}

void
_ctk_menu_item_popup_submenu (GtkWidget *widget,
                              gboolean   with_delay)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;
...
void
_ctk_menu_item_popdown_submenu (GtkWidget *widget)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;
  ...
  if (priv->submenu)
    {
      ...
      if (priv->timer)
        ...
        ctk_menu_popdown (GTK_MENU (priv->submenu));
 ...
 **/
void
ctk_menu_item_set_right_justified (GtkMenuItem *menu_item,
                                   gboolean     right_justified)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
 ...
 **/
gboolean
ctk_menu_item_get_right_justified (GtkMenuItem *menu_item)
{
...
static void
ctk_menu_item_show_all (GtkWidget *widget)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenuItemPrivate *priv = menu_item->priv;
...
static gboolean
ctk_menu_item_can_activate_accel (GtkWidget *widget,
                                  guint      signal_id)
{
  GtkWidget *parent;
...
static void
ctk_menu_item_accel_name_foreach (GtkWidget *widget,
                                  gpointer   data)
{
  ...
  if (!*path_p)
    {
      if (GTK_IS_LABEL (widget))
        {
          *path_p = ctk_label_get_text (GTK_LABEL (widget));
...
static void
ctk_menu_item_parent_set (GtkWidget *widget,
                          GtkWidget *previous_parent)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  GtkMenu *menu;
  ...
  if (GTK_WIDGET_CLASS (ctk_menu_item_parent_class)->parent_set)
    GTK_WIDGET_CLASS (ctk_menu_item_parent_class)->parent_set (widget, previous_parent);
}

static void
ctk_menu_item_direction_changed (GtkWidget        *widget,
                                 GtkTextDirection  previous_dir)
{
  GtkMenuItem *menu_item = GTK_MENU_ITEM (widget);
  ...
  GTK_WIDGET_CLASS (ctk_menu_item_parent_class)->direction_changed (widget, previous_dir);
}

void
_ctk_menu_item_refresh_accel_path (GtkMenuItem   *menu_item,
                                   const gchar   *prefix,
 ...
 */
void
ctk_menu_item_set_accel_path (GtkMenuItem *menu_item,
                              const gchar *accel_path)
{
  GtkMenuItemPrivate *priv = menu_item->priv;
  GtkWidget *parent;
 ...
 */
const gchar *
ctk_menu_item_get_accel_path (GtkMenuItem *menu_item)
{
  g_return_val_if_fail (GTK_IS_MENU_ITEM (menu_item), NULL);
...
static void
ctk_menu_item_forall (GtkContainer *container,
                      gboolean      include_internals,
                      GtkCallback   callback,
                      gpointer      callback_data)
{
...
gboolean
_ctk_menu_item_is_selectable (GtkWidget *menu_item)
{
  if ((!ctk_bin_get_child (GTK_BIN (menu_item)) &&
...
static void
ctk_menu_item_ensure_label (GtkMenuItem *menu_item)
{
  ...
  if (!ctk_bin_get_child (GTK_BIN (menu_item)))
    {
      ...
      ctk_widget_set_halign (accel_label, GTK_ALIGN_FILL);
      ctk_widget_set_valign (accel_label, GTK_ALIGN_CENTER);

      ctk_container_add (GTK_CONTAINER (menu_item), accel_label);
      ctk_accel_label_set_accel_widget (GTK_ACCEL_LABEL (accel_label),
                                        GTK_WIDGET (menu_item));
 ...
 */
void
ctk_menu_item_set_label (GtkMenuItem *menu_item,
                         const gchar *label)
{
  g_return_if_fail (GTK_IS_MENU_ITEM (menu_item));
 ...
 */
const gchar *
ctk_menu_item_get_label (GtkMenuItem *menu_item)
{
  g_return_val_if_fail (GTK_IS_MENU_ITEM (menu_item), NULL);
 ...
 */
void
ctk_menu_item_set_use_underline (GtkMenuItem *menu_item,
                                 gboolean     setting)
{
  GtkWidget *child;
  ...
  child = ctk_bin_get_child (GTK_BIN (menu_item));
  if (GTK_IS_LABEL (child) &&
      ctk_label_get_use_underline (GTK_LABEL (child)) != setting)
    {
      ctk_label_set_use_underline (GTK_LABEL (child), setting);
 ...
 */
gboolean
ctk_menu_item_get_use_underline (GtkMenuItem *menu_item)
{
  GtkWidget *child;
  ...
  child = ctk_bin_get_child (GTK_BIN (menu_item));
  if (GTK_IS_LABEL (child))
    return ctk_label_get_use_underline (GTK_LABEL (child));
 ...
 */
void
ctk_menu_item_set_reserve_indicator (GtkMenuItem *menu_item,
                                     gboolean     reserve)
{
  GtkMenuItemPrivate *priv;
 ...
 */
gboolean
ctk_menu_item_get_reserve_indicator (GtkMenuItem *menu_item)
{
  g_return_val_if_fail (GTK_IS_MENU_ITEM (menu_item), FALSE);
