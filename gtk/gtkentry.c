...
/* GObject methods
 */
static void   ctk_entry_editable_init        (GtkEditableInterface *iface);
static void   ctk_entry_cell_editable_init   (GtkCellEditableIface *iface);
...
/* GtkWidget methods
 */
static void   ctk_entry_destroy              (GtkWidget        *widget);
static void   ctk_entry_realize              (GtkWidget        *widget);
static void   ctk_entry_unrealize            (GtkWidget        *widget);
static void   ctk_entry_map                  (GtkWidget        *widget);
static void   ctk_entry_unmap                (GtkWidget        *widget);
static void   ctk_entry_get_preferred_width  (GtkWidget        *widget,
                                              gint             *minimum,
                                              gint             *natural);
static void   ctk_entry_get_preferred_height (GtkWidget        *widget,
                                              gint             *minimum,
                                              gint             *natural);
static void  ctk_entry_get_preferred_height_and_baseline_for_width (GtkWidget *widget,
                                                                    gint       width,
                                                                    gint      *minimum_height,
                                                                    gint      *natural_height,
                                                                    gint      *minimum_baseline,
                                                                    gint      *natural_baseline);
static void   ctk_entry_size_allocate        (GtkWidget        *widget,
					      GtkAllocation    *allocation);
static gint   ctk_entry_draw                 (GtkWidget        *widget,
                                              cairo_t          *cr);
static gboolean ctk_entry_event              (GtkWidget        *widget,
                                              GdkEvent         *event);
static gint   ctk_entry_enter_notify         (GtkWidget        *widget,
                                              GdkEventCrossing *event);
static gint   ctk_entry_leave_notify         (GtkWidget        *widget,
                                              GdkEventCrossing *event);
static gint   ctk_entry_key_press            (GtkWidget        *widget,
					      GdkEventKey      *event);
static gint   ctk_entry_key_release          (GtkWidget        *widget,
					      GdkEventKey      *event);
static gint   ctk_entry_focus_in             (GtkWidget        *widget,
					      GdkEventFocus    *event);
static gint   ctk_entry_focus_out            (GtkWidget        *widget,
					      GdkEventFocus    *event);
static void   ctk_entry_grab_focus           (GtkWidget        *widget);
static void   ctk_entry_style_updated        (GtkWidget        *widget);
static gboolean ctk_entry_query_tooltip      (GtkWidget        *widget,
                                              gint              x,
                                              gint              y,
                                              gboolean          keyboard_tip,
                                              GtkTooltip       *tooltip);
static void   ctk_entry_direction_changed    (GtkWidget        *widget,
					      GtkTextDirection  previous_dir);
static void   ctk_entry_state_flags_changed  (GtkWidget        *widget,
					      GtkStateFlags     previous_state);
static void   ctk_entry_screen_changed       (GtkWidget        *widget,
					      GdkScreen        *old_screen);

static gboolean ctk_entry_drag_drop          (GtkWidget        *widget,
                                              GdkDragContext   *context,
                                              gint              x,
                                              gint              y,
                                              guint             time);
static gboolean ctk_entry_drag_motion        (GtkWidget        *widget,
					      GdkDragContext   *context,
					      gint              x,
					      gint              y,
					      guint             time);
static void     ctk_entry_drag_leave         (GtkWidget        *widget,
					      GdkDragContext   *context,
					      guint             time);
static void     ctk_entry_drag_data_received (GtkWidget        *widget,
					      GdkDragContext   *context,
					      gint              x,
					      gint              y,
					      ...
					      guint             info,
					      guint             time);
static void     ctk_entry_drag_data_get      (GtkWidget        *widget,
					      GdkDragContext   *context,
					      GtkSelectionData *selection_data,
					      guint             info,
					      guint             time);
static void     ctk_entry_drag_data_delete   (GtkWidget        *widget,
					      GdkDragContext   *context);
static void     ctk_entry_drag_begin         (GtkWidget        *widget,
                                              GdkDragContext   *context);
static void     ctk_entry_drag_end           (GtkWidget        *widget,
                                              GdkDragContext   *context);


/* GtkEditable method implementations
 */
static void     ctk_entry_insert_text          (GtkEditable *editable,
						const gchar *new_text,
						gint         new_text_length,
						gint        *position);
static void     ctk_entry_delete_text          (GtkEditable *editable,
						gint         start_pos,
						gint         end_pos);
static gchar *  ctk_entry_get_chars            (GtkEditable *editable,
						gint         start_pos,
						gint         end_pos);
static void     ctk_entry_real_set_position    (GtkEditable *editable,
						gint         position);
static gint     ctk_entry_get_position         (GtkEditable *editable);
static void     ctk_entry_set_selection_bounds (GtkEditable *editable,
						gint         start,
						gint         end);
static gboolean ctk_entry_get_selection_bounds (GtkEditable *editable,
						gint        *start,
						gint        *end);

/* GtkCellEditable method implementations
 */
static void ctk_entry_start_editing (GtkCellEditable *cell_editable,
				     GdkEvent        *event);

/* Default signal handlers
 */
static void ctk_entry_real_insert_text   (GtkEditable     *editable,
					  const gchar     *new_text,
					  gint             new_text_length,
					  gint            *position);
static void ctk_entry_real_delete_text   (GtkEditable     *editable,
					  gint             start_pos,
					  gint             end_pos);
static void ctk_entry_move_cursor        (GtkEntry        *entry,
					  GtkMovementStep  step,
					  gint             count,
					  gboolean         extend_selection);
static void ctk_entry_insert_at_cursor   (GtkEntry        *entry,
					  const gchar     *str);
static void ctk_entry_delete_from_cursor (GtkEntry        *entry,
					  GtkDeleteType    type,
					  gint             count);
static void ctk_entry_backspace          (GtkEntry        *entry);
static void ctk_entry_cut_clipboard      (GtkEntry        *entry);
static void ctk_entry_copy_clipboard     (GtkEntry        *entry);
static void ctk_entry_paste_clipboard    (GtkEntry        *entry);
static void ctk_entry_toggle_overwrite   (GtkEntry        *entry);
static void ctk_entry_insert_emoji       (GtkEntry        *entry);
static void ctk_entry_select_all         (GtkEntry        *entry);
static void ctk_entry_real_activate      (GtkEntry        *entry);
static gboolean ctk_entry_popup_menu     (GtkWidget       *widget);
 ...
 */
static void     ctk_entry_commit_cb               (GtkIMContext *context,
						   const gchar  *str,
						   GtkEntry     *entry);
static void     ctk_entry_preedit_changed_cb      (GtkIMContext *context,
						   GtkEntry     *entry);
static gboolean ctk_entry_retrieve_surrounding_cb (GtkIMContext *context,
						   GtkEntry     *entry);
static gboolean ctk_entry_delete_surrounding_cb   (GtkIMContext *context,
						   gint          offset,
						   gint          n_chars,
						   GtkEntry     *entry);

/* Event controller callbacks */
static void   ctk_entry_multipress_gesture_pressed (GtkGestureMultiPress *gesture,
                                                    gint                  n_press,
                                                    gdouble               x,
                                                    gdouble               y,
                                                    GtkEntry             *entry);
static void   ctk_entry_drag_gesture_update        (GtkGestureDrag *gesture,
                                                    gdouble         offset_x,
                                                    gdouble         offset_y,
                                                    GtkEntry       *entry);
static void   ctk_entry_drag_gesture_end           (GtkGestureDrag *gesture,
                                                    gdouble         offset_x,
                                                    gdouble         offset_y,
                                                    GtkEntry       *entry);
...
/* Internal routines
 */
static void         ctk_entry_draw_text                (GtkEntry       *entry,
                                                        cairo_t        *cr);
static void         ctk_entry_draw_cursor              (GtkEntry       *entry,
                                                        cairo_t        *cr,
							CursorType      type);
static PangoLayout *ctk_entry_ensure_layout            (GtkEntry       *entry,
                                                        gboolean        include_preedit);
static void         ctk_entry_reset_layout             (GtkEntry       *entry);
static void         ctk_entry_recompute                (GtkEntry       *entry);
static gint         ctk_entry_find_position            (GtkEntry       *entry,
							gint            x);
static void         ctk_entry_get_cursor_locations     (GtkEntry       *entry,
							CursorType      type,
							gint           *strong_x,
							gint           *weak_x);
static void         ctk_entry_adjust_scroll            (GtkEntry       *entry);
static gint         ctk_entry_move_visually            (GtkEntry       *editable,
							gint            start,
							gint            count);
static gint         ctk_entry_move_logically           (GtkEntry       *entry,
							gint            start,
							gint            count);
static gint         ctk_entry_move_forward_word        (GtkEntry       *entry,
							gint            start,
                                                        gboolean        allow_whitespace);
static gint         ctk_entry_move_backward_word       (GtkEntry       *entry,
							gint            start,
                                                        gboolean        allow_whitespace);
static void         ctk_entry_delete_whitespace        (GtkEntry       *entry);
static void         ctk_entry_select_word              (GtkEntry       *entry);
static void         ctk_entry_select_line              (GtkEntry       *entry);
static void         ctk_entry_paste                    (GtkEntry       *entry,
							GdkAtom         selection);
static void         ctk_entry_update_primary_selection (GtkEntry       *entry);
static void         ctk_entry_do_popup                 (GtkEntry       *entry,
							const GdkEvent *event);
static gboolean     ctk_entry_mnemonic_activate        (GtkWidget      *widget,
							gboolean        group_cycling);
static void         ctk_entry_grab_notify              (GtkWidget      *widget,
                                                        gboolean        was_grabbed);
static void         ctk_entry_check_cursor_blink       (GtkEntry       *entry);
static void         ctk_entry_pend_cursor_blink        (GtkEntry       *entry);
static void         ctk_entry_reset_blink_time         (GtkEntry       *entry);
static void         ctk_entry_get_text_area_size       (GtkEntry       *entry,
							gint           *x,
							gint           *y,
							gint           *width,
							gint           *height);
static void         ctk_entry_get_frame_size           (GtkEntry       *entry,
							gint           *x,
							gint           *y,
							gint           *width,
							gint           *height);
static void         get_frame_size                     (GtkEntry       *entry,
							...
							gint           *width,
							gint           *height);
static void         ctk_entry_move_adjustments         (GtkEntry             *entry);
static void         ctk_entry_update_cached_style_values(GtkEntry      *entry);
static gboolean     get_middle_click_paste             (GtkEntry *entry);
static void         ctk_entry_get_scroll_limits        (GtkEntry       *entry,
                                                        gint           *min_offset,
                                                        gint           *max_offset);

/* GtkTextHandle handlers */
static void         ctk_entry_handle_drag_started      (GtkTextHandle         *handle,
                                                        GtkTextHandlePosition  pos,
                                                        GtkEntry              *entry);
static void         ctk_entry_handle_dragged           (GtkTextHandle         *handle,
                                                        GtkTextHandlePosition  pos,
                                                        gint                   x,
                                                        gint                   y,
                                                        GtkEntry              *entry);
static void         ctk_entry_handle_drag_finished     (GtkTextHandle         *handle,
                                                        GtkTextHandlePosition  pos,
                                                        GtkEntry              *entry);

static void         ctk_entry_selection_bubble_popup_set   (GtkEntry *entry);
static void         ctk_entry_selection_bubble_popup_unset (GtkEntry *entry);
...
static void     ctk_entry_measure  (GtkCssGadget        *gadget,
                                    ...
                                    gpointer             data);
static void     ctk_entry_allocate (GtkCssGadget        *gadget,
                                    const GtkAllocation *allocation,
                                    int                  baseline,
                                    GtkAllocation       *out_clip,
                                    gpointer             data);
static gboolean ctk_entry_render   (GtkCssGadget        *gadget,
                                    cairo_t             *cr,
                                    int                  x,
                                    int                  y,
                                    int                  width,
...
G_DEFINE_TYPE_WITH_CODE (GtkEntry, ctk_entry, GTK_TYPE_WIDGET,
...
static void
ctk_entry_class_init (GtkEntryClass *class)
{
 ...
 /* Action signals */
  ...
  ctk_widget_class_set_accessible_type (widget_class, GTK_TYPE_ENTRY_ACCESSIBLE);
  ctk_widget_class_set_css_name (widget_class, "entry");
}

static void
ctk_entry_editable_init (GtkEditableInterface *iface)
{
...
static void
ctk_entry_cell_editable_init (GtkCellEditableIface *iface)
{
...
static void
ctk_entry_do_set_inner_border (GtkEntry *entry,
...
ctk_entry_do_get_inner_border (GtkEntry *entry)
{
  ...
  switch (prop_id)
    ...
    case PROP_IM_MODULE:
      ...
      if (GTK_IS_IM_MULTICONTEXT (priv->im_context))
        ctk_im_multicontext_set_context_id (GTK_IM_MULTICONTEXT (priv->im_context), priv->im_module);
      g_object_notify_by_pspec (object, pspec);
      break;
    ...
    case PROP_COMPLETION:
      ctk_entry_set_completion (entry, GTK_ENTRY_COMPLETION (g_value_get_object (value)));
      break;
  ...
  switch (prop_id)
    ...
    case PROP_ACTIVATABLE_PRIMARY:
      g_value_set_boolean (value,
                           ctk_entry_get_icon_activatable (entry, GTK_ENTRY_ICON_PRIMARY));
      break;

    case PROP_ACTIVATABLE_SECONDARY:
      g_value_set_boolean (value,
                           ctk_entry_get_icon_activatable (entry, GTK_ENTRY_ICON_SECONDARY));
      break;

    case PROP_SENSITIVE_PRIMARY:
      g_value_set_boolean (value,
                           ctk_entry_get_icon_sensitive (entry, GTK_ENTRY_ICON_PRIMARY));
      break;

    case PROP_SENSITIVE_SECONDARY:
      g_value_set_boolean (value,
                           ctk_entry_get_icon_sensitive (entry, GTK_ENTRY_ICON_SECONDARY));
      break;

    case PROP_TOOLTIP_TEXT_PRIMARY:
      g_value_take_string (value,
                           ctk_entry_get_icon_tooltip_text (entry, GTK_ENTRY_ICON_PRIMARY));
      break;

    case PROP_TOOLTIP_TEXT_SECONDARY:
      g_value_take_string (value,
                           ctk_entry_get_icon_tooltip_text (entry, GTK_ENTRY_ICON_SECONDARY));
      break;

    case PROP_TOOLTIP_MARKUP_PRIMARY:
      g_value_take_string (value,
                           ctk_entry_get_icon_tooltip_markup (entry, GTK_ENTRY_ICON_PRIMARY));
      break;

    case PROP_TOOLTIP_MARKUP_SECONDARY:
      g_value_take_string (value,
                           ctk_entry_get_icon_tooltip_markup (entry, GTK_ENTRY_ICON_SECONDARY));
      break;
...
ctk_entry_init (GtkEntry *entry)
{
  ...
  ctk_widget_set_can_focus (GTK_WIDGET (entry), TRUE);
  ctk_widget_set_has_window (GTK_WIDGET (entry), FALSE);
  ...
  ctk_drag_dest_set (GTK_WIDGET (entry), 0, NULL, 0,
                     GDK_ACTION_COPY | GDK_ACTION_MOVE);
  ctk_drag_dest_add_text_targets (GTK_WIDGET (entry));
  ...
  priv->drag_gesture = ctk_gesture_drag_new (GTK_WIDGET (entry));
  g_signal_connect (priv->drag_gesture, "drag-update",
                    G_CALLBACK (ctk_entry_drag_gesture_update), entry);
  g_signal_connect (priv->drag_gesture, "drag-end",
                    G_CALLBACK (ctk_entry_drag_gesture_end), entry);
  ctk_gesture_single_set_button (GTK_GESTURE_SINGLE (priv->drag_gesture), 0);
  ctk_gesture_single_set_exclusive (GTK_GESTURE_SINGLE (priv->drag_gesture), TRUE);

  priv->multipress_gesture = ctk_gesture_multi_press_new (GTK_WIDGET (entry));
  ...
  ctk_gesture_single_set_exclusive (GTK_GESTURE_SINGLE (priv->multipress_gesture), TRUE);

  widget_node = ctk_widget_get_css_node (GTK_WIDGET (entry));
  ...
  for (i = 0; i < 2; i++)
    {
      ...
      ctk_css_node_add_class (priv->undershoot_node[i], g_quark_from_static_string (i == 0 ? GTK_STYLE_CLASS_LEFT : GTK_STYLE_CLASS_RIGHT));
      ctk_css_node_set_parent (priv->undershoot_node[i], widget_node);
      ctk_css_node_set_state (priv->undershoot_node[i], ctk_css_node_get_state (widget_node) & ~GTK_STATE_FLAG_DROP_ACTIVE);
      g_object_unref (priv->undershoot_node[i]);
    }
}

static void
ctk_entry_ensure_magnifier (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  ...
  priv->magnifier = _ctk_magnifier_new (GTK_WIDGET (entry));
  ctk_widget_set_size_request (priv->magnifier, 100, 60);
  _ctk_magnifier_set_magnification (GTK_MAGNIFIER (priv->magnifier), 2.0);
  priv->magnifier_popover = ctk_popover_new (GTK_WIDGET (entry));
  ctk_style_context_add_class (ctk_widget_get_style_context (priv->magnifier_popover),
                               "magnifier");
  ctk_popover_set_modal (GTK_POPOVER (priv->magnifier_popover), FALSE);
  ctk_container_add (GTK_CONTAINER (priv->magnifier_popover),
                     priv->magnifier);
  ctk_container_set_border_width (GTK_CONTAINER (priv->magnifier_popover), 4);
  ctk_widget_show (priv->magnifier);
}

static void
ctk_entry_ensure_text_handles (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  ...
  priv->text_handle = _ctk_text_handle_new (GTK_WIDGET (entry));
...
static void
ctk_entry_destroy (GtkWidget *widget)
{
  ...
  if (priv->magnifier)
    _ctk_magnifier_set_inspected (GTK_MAGNIFIER (priv->magnifier), NULL);

  GTK_WIDGET_CLASS (ctk_entry_parent_class)->destroy (widget);
}
...
ctk_entry_dispose (GObject *object)
{
  ...
  ctk_entry_set_icon_from_pixbuf (entry, GTK_ENTRY_ICON_PRIMARY, NULL);
  ctk_entry_set_icon_tooltip_markup (entry, GTK_ENTRY_ICON_PRIMARY, NULL);
  ctk_entry_set_icon_from_pixbuf (entry, GTK_ENTRY_ICON_SECONDARY, NULL);
  ctk_entry_set_icon_tooltip_markup (entry, GTK_ENTRY_ICON_SECONDARY, NULL);
  ...
  keymap = gdk_keymap_get_for_display (ctk_widget_get_display (GTK_WIDGET (object)));
...
ctk_entry_finalize (GObject *object)
{
  ...
  if (priv->tick_id)
    ctk_widget_remove_tick_callback (GTK_WIDGET (entry), priv->tick_id);
...
static DisplayMode
ctk_entry_get_display_mode (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
gchar*
_ctk_entry_get_display_text (GtkEntry *entry,
                             gint      start_pos,
                             gint      end_pos)
{
  ...
  for (i = 0; i < MAX_ICONS; i++)
    {
      if ((icon_info = priv->icons[i]) != NULL)
        {
          if (!_ctk_icon_helper_get_is_empty (GTK_ICON_HELPER (icon_info->gadget)) && 
...
update_node_state (GtkEntry *entry)
{
  ...
  state = ctk_widget_get_state_flags (GTK_WIDGET (entry));
...
update_node_ordering (GtkEntry *entry)
{
  ...
  if (ctk_widget_get_direction (GTK_WIDGET (entry)) == GTK_TEXT_DIR_RTL)
                     ...
                     GtkEntryIconPosition  icon_pos)
{
  ...
  _ctk_icon_helper_set_force_scale_pixbuf (GTK_ICON_HELPER (icon_info->gadget), TRUE);
...
static void
ctk_entry_map (GtkWidget *widget)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
  EntryIconInfo *icon_info = NULL;
  ...
  GTK_WIDGET_CLASS (ctk_entry_parent_class)->map (widget);
...
static void
ctk_entry_unmap (GtkWidget *widget)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
  EntryIconInfo *icon_info = NULL;
  ...
  GTK_WIDGET_CLASS (ctk_entry_parent_class)->unmap (widget);
}

static void
ctk_entry_realize (GtkWidget *widget)
{
  GtkEntry *entry;
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
  ...
  GTK_WIDGET_CLASS (ctk_entry_parent_class)->realize (widget);
...
static void
ctk_entry_unrealize (GtkWidget *widget)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
  ...
  GTK_WIDGET_CLASS (ctk_entry_parent_class)->unrealize (widget);
...
static void
ctk_entry_get_preferred_width (GtkWidget *widget,
                               gint      *minimum,
                               gint      *natural)
{
  ctk_css_gadget_get_preferred_size (GTK_ENTRY (widget)->priv->gadget,
                                     GTK_ORIENTATION_HORIZONTAL,
                                     -1,
                                     minimum, natural,
                                     NULL, NULL);
...
static void
ctk_entry_get_preferred_height (GtkWidget *widget,
                                gint      *minimum,
                                gint      *natural)
{
  ctk_css_gadget_get_preferred_size (GTK_ENTRY (widget)->priv->gadget,
...
static void
ctk_entry_get_preferred_height_and_baseline_for_width (GtkWidget *widget,
						       gint       width,
						       gint      *minimum,
						       gint      *natural,
						       gint      *minimum_baseline,
...
static void
ctk_entry_measure (GtkCssGadget   *gadget,
                   GtkOrientation  orientation,
                   int             for_size,
                   int            *minimum,
                   int            *natural,
      ...
      if (priv->width_chars < 0)
        {
          if (GTK_IS_SPIN_BUTTON (entry))
            min = ctk_spin_button_get_text_width (GTK_SPIN_BUTTON (entry));
          else
            min = MIN_ENTRY_WIDTH;
        }
      else
...
static void
ctk_entry_get_text_area_size (GtkEntry *entry,
                              gint     *x,
			      gint     *y,
			      gint     *width,
			      gint     *height)
{
  ...
  ctk_widget_get_allocation (GTK_WIDGET (entry), &widget_allocation);
...
static void
ctk_entry_get_frame_size (GtkEntry *entry,
                          gint     *x,
                          gint     *y,
                          gint     *width,
                          gint     *height)
  ...
  if (!relative_to_window)
    {
      ...
      ctk_widget_get_allocation (GTK_WIDGET (entry), &allocation);
...
static void
ctk_entry_size_allocate (GtkWidget     *widget,
			 GtkAllocation *allocation)
{
  GdkRectangle clip;
  ...
  ctk_css_gadget_allocate (GTK_ENTRY (widget)->priv->gadget,
                           allocation,
...
static void
ctk_entry_allocate (GtkCssGadget        *gadget,
                    const GtkAllocation *allocation,
                    int                  baseline,
                    GtkAllocation       *out_clip,
                    gpointer             data)
  ...
  for (i = 0; i < MAX_ICONS; i++)
    {
      ...
      if ((ctk_widget_get_direction (widget) == GTK_TEXT_DIR_RTL && i == GTK_ENTRY_ICON_PRIMARY) ||
          (ctk_widget_get_direction (widget) == GTK_TEXT_DIR_LTR && i == GTK_ENTRY_ICON_SECONDARY))
        {
...
static gboolean
ctk_entry_draw (GtkWidget *widget,
		cairo_t   *cr)
{
  ctk_css_gadget_draw (GTK_ENTRY (widget)->priv->gadget, cr);
...
static void
ctk_entry_draw_undershoot (GtkEntry *entry,
                           cairo_t  *cr)
{
  GtkEntryPrivate *priv = entry->priv;
  ...
  rtl = ctk_widget_get_direction (GTK_WIDGET (entry)) == GTK_TEXT_DIR_RTL;
...
static gboolean
ctk_entry_render (GtkCssGadget *gadget,
                  cairo_t      *cr,
                  int           x,
                  int           y,
                  int           width,
                  ...
                  gpointer      data)
{
  ...
  if (priv->dnd_position != -1)
    ctk_entry_draw_cursor (GTK_ENTRY (widget), cr, CURSOR_DND);

  ctk_entry_draw_text (GTK_ENTRY (widget), cr);
  ...
  if (ctk_entry_get_display_mode (entry) != DISPLAY_BLANK &&
    ...
    ctk_entry_draw_cursor (GTK_ENTRY (widget), cr, CURSOR_STANDARD);
...
static gint
ctk_entry_enter_notify (GtkWidget        *widget,
                        GdkEventCrossing *event)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
...
static gint
ctk_entry_leave_notify (GtkWidget        *widget,
                        GdkEventCrossing *event)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_get_pixel_ranges (GtkEntry  *entry,
			    gint     **ranges,
			    gint      *n_ranges)
{
  gint start_char, end_char;

  if (ctk_editable_get_selection_bounds (GTK_EDITABLE (entry), &start_char, &end_char))
    {
...
static void
ctk_entry_move_handle (GtkEntry              *entry,
                       GtkTextHandlePosition  pos,
                       gint                   x,
                       gint                   y,
                       gint                   height)
...
static gint
ctk_entry_get_selection_bound_location (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  PangoLayout *layout;
  PangoRectangle pos;
...
static void
ctk_entry_update_handles (GtkEntry          *entry,
  ...
  if (mode == GTK_TEXT_HANDLE_MODE_SELECTION)
    {
      ...
      ctk_entry_move_handle (entry, GTK_TEXT_HANDLE_POSITION_SELECTION_START,
                             start, 0, height);
      ctk_entry_move_handle (entry, GTK_TEXT_HANDLE_POSITION_SELECTION_END,
                             end, 0, height);
    }
  else
    ctk_entry_move_handle (entry, GTK_TEXT_HANDLE_POSITION_CURSOR,
                           cursor, 0, height);
}

static gboolean
ctk_entry_event (GtkWidget *widget,
                 GdkEvent  *event)
{
                                     ...
                                     gint             *y)
{
  ...
  ctk_gesture_get_point (GTK_GESTURE (gesture), sequence, &px, &py);
...
static void
ctk_entry_multipress_gesture_pressed (GtkGestureMultiPress *gesture,
                                      gint                  n_press,
                                      gdouble               widget_x,
                                      gdouble               widget_y,
                                      GtkEntry             *entry)
{
  ...
  event = ctk_gesture_get_last_event (GTK_GESTURE (gesture), current);

  ctk_gesture_set_sequence_state (GTK_GESTURE (gesture), current,
...
static gchar *
_ctk_entry_get_selected_text (GtkEntry *entry)
{
  GtkEditable *editable = GTK_EDITABLE (entry);
...
static void
ctk_entry_show_magnifier (GtkEntry *entry,
                          gint      x,
                          gint      y)
{
  GtkAllocation allocation;
  ...
  _ctk_magnifier_set_coords (GTK_MAGNIFIER (priv->magnifier), rect.x,
                             rect.y + rect.height / 2);
  rect.x = CLAMP (rect.x, 0, allocation.width);
  ctk_popover_set_pointing_to (GTK_POPOVER (priv->magnifier_popover),
                               &rect);
  ctk_popover_popup (GTK_POPOVER (priv->magnifier_popover));
}

static void
ctk_entry_drag_gesture_update (GtkGestureDrag *gesture,
                               gdouble         offset_x,
                               gdouble         offset_y,
                               GtkEntry       *entry)
{
...
static void
ctk_entry_drag_gesture_end (GtkGestureDrag *gesture,
                            gdouble         offset_x,
                            gdouble         offset_y,
                            GtkEntry       *entry)
{
  ...
  if (priv->magnifier_popover)
    ctk_popover_popdown (GTK_POPOVER (priv->magnifier_popover));
  ...
  if (in_drag)
    {
      ...
      ctk_editable_set_position (GTK_EDITABLE (entry), tmp_pos);
    }

  if (is_touchscreen &&
      !ctk_editable_get_selection_bounds (GTK_EDITABLE (entry), NULL, NULL))
    ctk_entry_update_handles (entry, GTK_TEXT_HANDLE_MODE_CURSOR);
...
static void
ctk_entry_obscure_mouse_cursor (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
static gint
ctk_entry_key_press (GtkWidget   *widget,
		     GdkEventKey *event)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
  ...
  if (GTK_WIDGET_CLASS (ctk_entry_parent_class)->key_press_event (widget, event))
    {
...
static gint
ctk_entry_key_release (GtkWidget   *widget,
		       GdkEventKey *event)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
  ...
  retval = GTK_WIDGET_CLASS (ctk_entry_parent_class)->key_release_event (widget, event);
...
static gint
ctk_entry_focus_in (GtkWidget     *widget,
		    GdkEventFocus *event)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
...
static gint
ctk_entry_focus_out (GtkWidget     *widget,
		     GdkEventFocus *event)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
...
void
_ctk_entry_grab_focus (GtkEntry  *entry,
                       gboolean   select_all)
{
  if (!ctk_widget_get_can_focus (GTK_WIDGET (entry)))
    return;

  if (!ctk_widget_is_sensitive (GTK_WIDGET (entry)))
    return;

  GTK_WIDGET_CLASS (ctk_entry_parent_class)->grab_focus (GTK_WIDGET (entry));
...
static void
ctk_entry_grab_focus (GtkWidget *widget)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
 ...
 */
void
ctk_entry_grab_focus_without_selecting (GtkEntry *entry)
{
...
static void
ctk_entry_direction_changed (GtkWidget        *widget,
                             GtkTextDirection  previous_dir)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  ...
  GTK_WIDGET_CLASS (ctk_entry_parent_class)->direction_changed (widget, previous_dir);
}

static void
ctk_entry_state_flags_changed (GtkWidget     *widget,
                               GtkStateFlags  previous_state)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
  ...
  if (!ctk_widget_is_sensitive (widget))
    {
      /* Clear any selection */
      ctk_editable_select_region (GTK_EDITABLE (entry), priv->current_pos, priv->current_pos);
    }
...
static void
ctk_entry_screen_changed (GtkWidget *widget,
			  GdkScreen *old_screen)
{
  ctk_entry_recompute (GTK_ENTRY (widget));
}

/* GtkEditable method implementations
 */
static void
ctk_entry_insert_text (GtkEditable *editable,
		       const gchar *new_text,
		       gint         new_text_length,
		       gint        *position)
{
...
static void
ctk_entry_delete_text (GtkEditable *editable,
		       gint         start_pos,
		       gint         end_pos)
{
  g_object_ref (editable);
...
static gchar *    
ctk_entry_get_chars      (GtkEditable   *editable,
			  gint           start_pos,
			  gint           end_pos)
{
...
static void
ctk_entry_real_set_position (GtkEditable *editable,
			     gint         position)
{
  GtkEntry *entry = GTK_ENTRY (editable);
  GtkEntryPrivate *priv = entry->priv;
...
static gint
ctk_entry_get_position (GtkEditable *editable)
{
  GtkEntry *entry = GTK_ENTRY (editable);
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_set_selection_bounds (GtkEditable *editable,
				gint         start,
				gint         end)
{
  GtkEntry *entry = GTK_ENTRY (editable);
...
static gboolean
ctk_entry_get_selection_bounds (GtkEditable *editable,
				gint        *start,
				gint        *end)
{
  GtkEntry *entry = GTK_ENTRY (editable);
...
static void
ctk_entry_update_cached_style_values (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void 
ctk_entry_style_updated (GtkWidget *widget)
{
  GtkEntry *entry = GTK_ENTRY (widget);

  GTK_WIDGET_CLASS (ctk_entry_parent_class)->style_updated (widget);
 ...
 */
static void
ctk_cell_editable_entry_activated (GtkEntry *entry, gpointer data)
{
...
static gboolean
ctk_cell_editable_key_press_event (GtkEntry    *entry,
...
static void
ctk_entry_start_editing (GtkCellEditable *cell_editable,
			 GdkEvent        *event)
{
...
static void
ctk_entry_password_hint_free (GtkEntryPasswordHint *password_hint)
{
...
ctk_entry_remove_password_hint (gpointer data)
{
  ...
  ctk_entry_recompute (GTK_ENTRY (data));
 ...
 */
static void
ctk_entry_real_insert_text (GtkEditable *editable,
			    const gchar *new_text,
			    gint         new_text_length,
			    gint        *position)
{
  ...
  n_inserted = ctk_entry_buffer_insert_text (get_buffer (GTK_ENTRY (editable)), *position, new_text, n_chars);

  end_change (GTK_ENTRY (editable));

  if (n_inserted != n_chars)
      ctk_widget_error_bell (GTK_WIDGET (editable));

  *position += n_inserted;
}

static void
ctk_entry_real_delete_text (GtkEditable *editable,
                            gint         start_pos,
                            gint         end_pos)
{
  /*
  ...
  ctk_entry_buffer_delete_text (get_buffer (GTK_ENTRY (editable)), start_pos, end_pos - start_pos);
                      ...
                      GtkEntry       *entry)
{
  ...
  if (n_chars == 1 && !priv->visible)
    {
      g_object_get (ctk_widget_get_settings (GTK_WIDGET (entry)),
...
{
  GtkEntryPrivate *priv = entry->priv;
  GdkKeymap *keymap = gdk_keymap_get_for_display (ctk_widget_get_display (GTK_WIDGET (entry)));
...
static void
ctk_entry_move_cursor (GtkEntry       *entry,
		       GtkMovementStep step,
		       gint            count,
		       gboolean        extend_selection)
{
          ...
          if (priv->current_pos == new_pos)
            {
              if (!extend_selection)
                {
                  if (!ctk_widget_keynav_failed (GTK_WIDGET (entry),
                                                 count > 0 ?
                                                 GTK_DIR_RIGHT : GTK_DIR_LEFT))
                    {
                      GtkWidget *toplevel = ctk_widget_get_toplevel (GTK_WIDGET (entry));
                ...
                {
                  ctk_widget_error_bell (GTK_WIDGET (entry));
                }
            }
	  break;
  ...
  if (extend_selection)
    ctk_editable_select_region (GTK_EDITABLE (entry), priv->selection_bound, new_pos);
  else
    ctk_editable_set_position (GTK_EDITABLE (entry), new_pos);
  
  ctk_entry_pend_cursor_blink (entry);
}

static void
ctk_entry_insert_at_cursor (GtkEntry    *entry,
			    const gchar *str)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkEditable *editable = GTK_EDITABLE (entry);
...
static void
ctk_entry_delete_from_cursor (GtkEntry       *entry,
			      GtkDeleteType   type,
			      gint            count)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_backspace (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkEditable *editable = GTK_EDITABLE (entry);
  gint prev_pos;
...
static void
ctk_entry_copy_clipboard (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkEditable *editable = GTK_EDITABLE (entry);
  gint start, end;
  ...
  if (ctk_editable_get_selection_bounds (editable, &start, &end))
    {
      ...
      ctk_clipboard_set_text (ctk_widget_get_clipboard (GTK_WIDGET (entry),
...
static void
ctk_entry_cut_clipboard (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkEditable *editable = GTK_EDITABLE (entry);
  gint start, end;
...
static void
ctk_entry_paste_clipboard (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_delete_cb (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkEditable *editable = GTK_EDITABLE (entry);
  gint start, end;
...
static void
ctk_entry_toggle_overwrite (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;

  priv->overwrite_mode = !priv->overwrite_mode;
  ctk_entry_pend_cursor_blink (entry);
  ctk_widget_queue_draw (GTK_WIDGET (entry));
}

static void
ctk_entry_select_all (GtkEntry *entry)
{
  ctk_entry_select_line (entry);
}

static void
ctk_entry_real_activate (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkWindow *window;
...
static void
ctk_entry_commit_cb (GtkIMContext *context,
		     const gchar  *str,
		     GtkEntry     *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void 
ctk_entry_preedit_changed_cb (GtkIMContext *context,
			      GtkEntry     *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
static gboolean
ctk_entry_retrieve_surrounding_cb (GtkIMContext *context,
                                   GtkEntry     *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  gchar *text;
...
static gboolean
ctk_entry_delete_surrounding_cb (GtkIMContext *slave,
				 gint          offset,
				 gint          n_chars,
				 GtkEntry     *entry)
{
  ...
  if (priv->editable)
    ctk_editable_delete_text (GTK_EDITABLE (entry),
...
void
ctk_entry_enter_text (GtkEntry       *entry,
                      const gchar    *str)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkEditable *editable = GTK_EDITABLE (entry);
      ...
      if (priv->overwrite_mode)
        {
          ...
          if (priv->current_pos < text_length)
            ctk_entry_delete_from_cursor (entry, GTK_DELETE_CHARS, 1);
        }
    }
 ...
 */
void
ctk_entry_set_positions (GtkEntry *entry,
			 gint      current_pos,
			 gint      selection_bound)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_reset_layout (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_recompute (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkTextHandleMode handle_mode;
...
static void
ctk_entry_get_placeholder_text_color (GtkEntry   *entry,
                                      PangoColor *color)
{
  GtkWidget *widget = GTK_WIDGET (entry);
  GtkStyleContext *context;
...
show_placeholder_text (GtkEntry *entry)
{
  ...
  if (!ctk_widget_has_focus (GTK_WIDGET (entry)) &&
...
static PangoLayout *
ctk_entry_create_layout (GtkEntry *entry,
			 gboolean  include_preedit)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkWidget *widget = GTK_WIDGET (entry);
...
static PangoLayout *
ctk_entry_ensure_layout (GtkEntry *entry,
                         gboolean  include_preedit)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_draw_text (GtkEntry *entry,
                     cairo_t  *cr)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkWidget *widget = GTK_WIDGET (entry);
  ...
  if (ctk_editable_get_selection_bounds (GTK_EDITABLE (entry), &start_pos, &end_pos))
    {
      const char *text = pango_layout_get_text (layout);
...
static void
ctk_entry_draw_cursor (GtkEntry  *entry,
                       cairo_t   *cr,
		       CursorType type)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_handle_dragged (GtkTextHandle         *handle,
                          GtkTextHandlePosition  pos,
                          gint                   x,
                          gint                   y,
                          GtkEntry              *entry)
...
static void
ctk_entry_handle_drag_started (GtkTextHandle         *handle,
                               GtkTextHandlePosition  pos,
                               GtkEntry              *entry)
{
...
static void
ctk_entry_handle_drag_finished (GtkTextHandle         *handle,
                                GtkTextHandlePosition  pos,
                                GtkEntry              *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  ...
  if (!priv->cursor_handle_dragged && !priv->selection_handle_dragged)
    {
      ...
      settings = ctk_widget_get_settings (GTK_WIDGET (entry));
      g_object_get (settings, "gtk-double-click-time", &double_click_time, NULL);
      if (g_get_monotonic_time() - priv->handle_place_time < double_click_time * 1000)
        {
          ctk_entry_select_word (entry);
          ctk_entry_update_handles (entry, GTK_TEXT_HANDLE_MODE_SELECTION);
        }
      else
        ctk_entry_selection_bubble_popup_set (entry);
    }
 ...
 */
void
ctk_entry_reset_im_context (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
 ...
 */
gboolean
ctk_entry_im_context_filter_keypress (GtkEntry    *entry,
                                      GdkEventKey *event)
{
  GtkEntryPrivate *priv;
...
GtkIMContext*
_ctk_entry_get_im_context (GtkEntry *entry)
{
  return entry->priv->im_context;
}

GtkCssGadget *
ctk_entry_get_gadget (GtkEntry  *entry)
{
  return entry->priv->gadget;
}

static gint
ctk_entry_find_position (GtkEntry *entry,
			 gint      x)
{
  GtkEntryPrivate *priv = entry->priv;
  PangoLayout *layout;
...
static void
ctk_entry_get_cursor_locations (GtkEntry   *entry,
				CursorType  type,
				gint       *strong_x,
				gint       *weak_x)
{
...
static gboolean
ctk_entry_get_is_selection_handle_dragged (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkTextHandlePosition pos;
  ...
  if (_ctk_text_handle_get_mode (priv->text_handle) != GTK_TEXT_HANDLE_MODE_SELECTION)
    return FALSE;
...
static void
ctk_entry_get_scroll_limits (GtkEntry *entry,
                             gint     *min_offset,
                             gint     *max_offset)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_adjust_scroll (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  gint min_offset, max_offset;
...
static void
ctk_entry_move_adjustments (GtkEntry *entry)
{
  GtkWidget *widget = GTK_WIDGET (entry);
  GtkAllocation allocation;
...
static gint
ctk_entry_move_visually (GtkEntry *entry,
			 gint      start,
			 gint      count)
{
  GtkEntryPrivate *priv = entry->priv;
...
static gint
ctk_entry_move_logically (GtkEntry *entry,
			  gint      start,
			  gint      count)
{
  gint new_pos = start;
...
static gint
ctk_entry_move_forward_word (GtkEntry *entry,
			     gint      start,
                             gboolean  allow_whitespace)
{
  gint new_pos = start;
...
static gint
ctk_entry_move_backward_word (GtkEntry *entry,
			      gint      start,
                              gboolean  allow_whitespace)
{
  gint new_pos = start;
...
static void
ctk_entry_delete_whitespace (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  PangoLayout *layout = ctk_entry_ensure_layout (entry, FALSE);
  PangoLogAttr *log_attrs;
  ...
  if (start != end)
    ctk_editable_delete_text (GTK_EDITABLE (entry), start, end);
}


static void
ctk_entry_select_word (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  ...
  ctk_editable_select_region (GTK_EDITABLE (entry), start_pos, end_pos);
}

static void
ctk_entry_select_line (GtkEntry *entry)
{
  ctk_editable_select_region (GTK_EDITABLE (entry), 0, -1);
}
		...
		gpointer      data)
{
  ...
  button = ctk_gesture_single_get_current_button (GTK_GESTURE_SINGLE (priv->multipress_gesture));
...
static void
ctk_entry_paste (GtkEntry *entry,
		 GdkAtom   selection)
{
  g_object_ref (entry);
  ctk_clipboard_request_text (ctk_widget_get_clipboard (GTK_WIDGET (entry), selection),
		...
		gpointer          data)
{
  ...
  if (ctk_editable_get_selection_bounds (GTK_EDITABLE (entry), &start, &end))
    {
...
static void
ctk_entry_update_primary_selection (GtkEntry *entry)
{
  GtkTargetList *list;
  ...
  clipboard = ctk_widget_get_clipboard (GTK_WIDGET (entry), GDK_SELECTION_PRIMARY);
...
static void
ctk_entry_clear_icon (GtkEntry             *entry,
                      GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv = entry->priv;
  EntryIconInfo *icon_info = priv->icons[icon_pos];
 ...
 */
GtkWidget*
ctk_entry_new_with_buffer (GtkEntryBuffer *buffer)
{
 ...
 */
GtkEntryBuffer*
ctk_entry_get_buffer (GtkEntry *entry)
{
 ...
 */
void
ctk_entry_set_buffer (GtkEntry       *entry,
                      GtkEntryBuffer *buffer)
{
  GtkEntryPrivate *priv;
  GObject *obj;
  ...
  if (had_buffer)
    {
      ctk_editable_set_position (GTK_EDITABLE (entry), 0);
      ctk_entry_recompute (entry);
    }
}
 ...
 **/
void
ctk_entry_get_text_area (GtkEntry     *entry,
 ...
 */
void
ctk_entry_set_text (GtkEntry    *entry,
		    const gchar *text)
{
  gint tmp_pos;
  GtkEntryCompletion *completion;
  ...
  begin_change (entry);
  ctk_editable_delete_text (GTK_EDITABLE (entry), 0, -1);
  tmp_pos = 0;
  ctk_editable_insert_text (GTK_EDITABLE (entry), text, strlen (text), &tmp_pos);
  end_change (entry);
 ...
 */
void
ctk_entry_set_visibility (GtkEntry *entry,
			  gboolean visible)
{
  GtkEntryPrivate *priv;
 ...
 **/
gboolean
ctk_entry_get_visibility (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), FALSE);
 ...
 **/
void
ctk_entry_set_invisible_char (GtkEntry *entry,
                              gunichar  ch)
{
  GtkEntryPrivate *priv;
 ...
 **/
gunichar
ctk_entry_get_invisible_char (GtkEntry *entry)
{
 ...
 **/
void
ctk_entry_unset_invisible_char (GtkEntry *entry)
{
  GtkEntryPrivate *priv;
  gunichar ch;
 ...
 **/
void
ctk_entry_set_overwrite_mode (GtkEntry *entry,
                              gboolean  overwrite)
{
  GtkEntryPrivate *priv = entry->priv;
 ...
 **/
gboolean
ctk_entry_get_overwrite_mode (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), FALSE);
 ...
 **/
const gchar*
ctk_entry_get_text (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), NULL);
 ...
 **/
void
ctk_entry_set_max_length (GtkEntry     *entry,
                          gint          max)
{
  g_return_if_fail (GTK_IS_ENTRY (entry));
 ...
 **/
gint
ctk_entry_get_max_length (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), 0);
 ...
 **/
guint16
ctk_entry_get_text_length (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), 0);
 ...
 **/
void
ctk_entry_set_activates_default (GtkEntry *entry,
                                 gboolean  setting)
{
  GtkEntryPrivate *priv;
 ...
 */
gboolean
ctk_entry_get_activates_default (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), FALSE);
 ...
 **/
void
ctk_entry_set_width_chars (GtkEntry *entry,
                           gint      n_chars)
{
  GtkEntryPrivate *priv;
  ...
  if (priv->width_chars != n_chars)
    {
      ...
      ctk_widget_queue_resize (GTK_WIDGET (entry));
    }
}
 ...
 **/
gint
ctk_entry_get_width_chars (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), 0);
 ...
 */
void
ctk_entry_set_max_width_chars (GtkEntry *entry,
                               gint      n_chars)
{
  GtkEntryPrivate *priv;
 ...
 */
gint
ctk_entry_get_max_width_chars (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), 0);
 ...
 **/
void
ctk_entry_set_has_frame (GtkEntry *entry,
                         gboolean  setting)
{
  GtkStyleContext *context;
  ...
  context = ctk_widget_get_style_context (GTK_WIDGET (entry));
  if (setting)
    ctk_style_context_remove_class (context, GTK_STYLE_CLASS_FLAT);
  else
    ctk_style_context_add_class (context, GTK_STYLE_CLASS_FLAT);
  ctk_widget_queue_draw (GTK_WIDGET (entry));
 ...
 **/
gboolean
ctk_entry_get_has_frame (GtkEntry *entry)
{
  GtkStyleContext *context;
  ...
  return !ctk_style_context_has_class (context, GTK_STYLE_CLASS_FLAT);
}
 ...
 **/
void
ctk_entry_set_inner_border (GtkEntry        *entry,
                            const GtkBorder *border)
{
  g_return_if_fail (GTK_IS_ENTRY (entry));
 ...
 **/
const GtkBorder *
ctk_entry_get_inner_border (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), NULL);
 ...
 **/
PangoLayout*
ctk_entry_get_layout (GtkEntry *entry)
{
  PangoLayout *layout;
 ...
 **/
gint
ctk_entry_layout_index_to_text_index (GtkEntry *entry,
                                      gint      layout_index)
{
  GtkEntryPrivate *priv;
  PangoLayout *layout;
 ...
 **/
gint
ctk_entry_text_index_to_layout_index (GtkEntry *entry,
                                      gint      text_index)
{
  GtkEntryPrivate *priv;
  PangoLayout *layout;
 ...
 **/
void
ctk_entry_get_layout_offsets (GtkEntry *entry,
                              gint     *x,
                              gint     *y)
{
  GtkEntryPrivate *priv;
 ...
 **/
void
ctk_entry_set_alignment (GtkEntry *entry, gfloat xalign)
{
  GtkEntryPrivate *priv;
 ...
 **/
gfloat
ctk_entry_get_alignment (GtkEntry *entry)
{
 ...
 */
void
ctk_entry_set_icon_from_pixbuf (GtkEntry             *entry,
                                GtkEntryIconPosition  icon_pos,
                                GdkPixbuf            *pixbuf)
{
  GtkEntryPrivate *priv;
  ...
  if (pixbuf)
    {
      _ctk_icon_helper_set_pixbuf (GTK_ICON_HELPER (icon_info->gadget), pixbuf);
      _ctk_icon_helper_set_icon_size (GTK_ICON_HELPER (icon_info->gadget),
                                      GTK_ICON_SIZE_MENU);
      ...
      if (ctk_widget_get_mapped (GTK_WIDGET (entry)))
          gdk_window_show_unraised (icon_info->window);
  ...
  if (ctk_widget_get_visible (GTK_WIDGET (entry)))
    ctk_widget_queue_resize (GTK_WIDGET (entry));
 ...
 */
void
ctk_entry_set_icon_from_stock (GtkEntry             *entry,
                               GtkEntryIconPosition  icon_pos,
                               const gchar          *stock_id)
{
  ...
  if (stock_id != NULL)
    {
      _ctk_icon_helper_set_stock_id (GTK_ICON_HELPER (icon_info->gadget), stock_id, GTK_ICON_SIZE_MENU);
 ...
 */
void
ctk_entry_set_icon_from_icon_name (GtkEntry             *entry,
                                   GtkEntryIconPosition  icon_pos,
                                   const gchar          *icon_name)
{
  ...
  if (icon_name != NULL)
    {
      _ctk_icon_helper_set_icon_name (GTK_ICON_HELPER (icon_info->gadget), icon_name, GTK_ICON_SIZE_MENU);
 ...
 */
void
ctk_entry_set_icon_from_gicon (GtkEntry             *entry,
                               GtkEntryIconPosition  icon_pos,
                               GIcon                *icon)
{
  GtkEntryPrivate *priv;
  ...
  if (icon)
    {
      _ctk_icon_helper_set_gicon (GTK_ICON_HELPER (icon_info->gadget), icon, GTK_ICON_SIZE_MENU);
 ...
 */
void
ctk_entry_set_icon_activatable (GtkEntry             *entry,
                                GtkEntryIconPosition  icon_pos,
 ...
 */
gboolean
ctk_entry_get_icon_activatable (GtkEntry             *entry,
                                GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
 ...
 */
GdkPixbuf *
ctk_entry_get_icon_pixbuf (GtkEntry             *entry,
                           GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
  ...
  _ctk_icon_helper_get_size (GTK_ICON_HELPER (icon_info->gadget), &width, &height);
  surface = ctk_icon_helper_load_surface (GTK_ICON_HELPER (icon_info->gadget), 1);
 ...
 */
GIcon *
ctk_entry_get_icon_gicon (GtkEntry             *entry,
                          GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
  ...
  return _ctk_icon_helper_peek_gicon (GTK_ICON_HELPER (icon_info->gadget));
}
 ...
 */
const gchar *
ctk_entry_get_icon_stock (GtkEntry             *entry,
                          GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
  ...
  return _ctk_icon_helper_get_stock_id (GTK_ICON_HELPER (icon_info->gadget));
}
 ...
 */
const gchar *
ctk_entry_get_icon_name (GtkEntry             *entry,
                         GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
  ...
  return _ctk_icon_helper_get_icon_name (GTK_ICON_HELPER (icon_info->gadget));
}
 ...
 */
void
ctk_entry_set_icon_sensitive (GtkEntry             *entry,
                              GtkEntryIconPosition  icon_pos,
                              gboolean              sensitive)
{
  GtkEntryPrivate *priv;
 ...
 */
gboolean
ctk_entry_get_icon_sensitive (GtkEntry             *entry,
                              GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
 ...
 **/
GtkImageType
ctk_entry_get_icon_storage_type (GtkEntry             *entry,
                                 GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
  ...
  return _ctk_icon_helper_get_storage_type (GTK_ICON_HELPER (icon_info->gadget));
}
 ...
 */
gint
ctk_entry_get_icon_at_pos (GtkEntry *entry,
                           gint      x,
                           gint      y)
{
  GtkEntryPrivate *priv;
 ...
 */
void
ctk_entry_set_icon_drag_source (GtkEntry             *entry,
                                GtkEntryIconPosition  icon_pos,
                                GtkTargetList        *target_list,
                                GdkDragAction         actions)
{
 ...
 */
gint
ctk_entry_get_current_icon_drag_source (GtkEntry *entry)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info = NULL;
  gint i;
 ...
 */
void
ctk_entry_get_icon_area (GtkEntry             *entry,
                         GtkEntryIconPosition  icon_pos,
...
{
  gchar *text = ctk_widget_get_tooltip_text (GTK_WIDGET (entry));
  ...
  ctk_widget_set_has_tooltip (GTK_WIDGET (entry), has_tooltip);
}
 ...
 */
gchar *
ctk_entry_get_icon_tooltip_text (GtkEntry             *entry,
                                 GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
 ...
 * ctk_widget_set_tooltip_markup(), this sets GtkWidget:has-tooltip to %FALSE,
 * which suppresses icon tooltips too. You can resolve this by then calling
 * ctk_widget_set_has_tooltip() to set GtkWidget:has-tooltip back to %TRUE, or
 * setting at least one non-empty tooltip on any icon achieves the same result.
 *
 * Since: 2.16
 */
void
ctk_entry_set_icon_tooltip_text (GtkEntry             *entry,
                                 GtkEntryIconPosition  icon_pos,
                                 const gchar          *tooltip)
{
  GtkEntryPrivate *priv;
 ...
 */
gchar *
ctk_entry_get_icon_tooltip_markup (GtkEntry             *entry,
                                   GtkEntryIconPosition  icon_pos)
{
  GtkEntryPrivate *priv;
  EntryIconInfo *icon_info;
 ...
 */
void
ctk_entry_set_icon_tooltip_markup (GtkEntry             *entry,
                                   GtkEntryIconPosition  icon_pos,
                                   const gchar          *tooltip)
{
  GtkEntryPrivate *priv;
...
static gboolean
ctk_entry_query_tooltip (GtkWidget  *widget,
                         gint        x,
                         gint        y,
                         gboolean    keyboard_tip,
                         GtkTooltip *tooltip)
{
  ...
  return GTK_WIDGET_CLASS (ctk_entry_parent_class)->query_tooltip (widget,
                                                                   x, y,
                                                                   keyboard_tip,
                                                                   tooltip);
}
...
static gboolean
ctk_entry_mnemonic_activate (GtkWidget *widget,
			     gboolean   group_cycling)
{
  ctk_widget_grab_focus (widget);
  return GDK_EVENT_STOP;
...
{
  if (!info->device ||
      !ctk_widget_device_is_shadowed (GTK_WIDGET (entry), info->device))
    return;
...
static void
ctk_entry_grab_notify (GtkWidget *widget,
                       gboolean   was_grabbed)
{
  GtkEntryPrivate *priv;
  gint i;
...
append_action_signal (GtkEntry     *entry,
		      ...
		      const gchar  *signal,
                      gboolean      sensitive)
{
  GtkWidget *menuitem = ctk_menu_item_new_with_mnemonic (label);
			...
			gpointer          user_data)
{
  ...
  if (ctk_widget_get_realized (GTK_WIDGET (entry)))
	...
	ctk_widget_destroy (info_entry_priv->popup_menu);
      ...
      ctk_menu_attach_to_widget (GTK_MENU (menu), GTK_WIDGET (entry), popup_menu_detach);
      ...
      if (info_entry_priv->show_emoji_icon ||
          (ctk_entry_get_input_hints (entry) & GTK_INPUT_HINT_NO_EMOJI) == 0)
        {
      ...
      if (info->trigger_event && gdk_event_triggers_context_menu (info->trigger_event))
        ctk_menu_popup_at_pointer (GTK_MENU (menu), info->trigger_event);
      else
        {
          ...
          ctk_menu_popup_at_rect (GTK_MENU (menu),
...
static void
ctk_entry_do_popup (GtkEntry       *entry,
                    const GdkEvent *event)
{
  ...
  ctk_clipboard_request_contents (ctk_widget_get_clipboard (GTK_WIDGET (entry), GDK_SELECTION_CLIPBOARD),
...
static gboolean
ctk_entry_popup_menu (GtkWidget *widget)
{
  ctk_entry_do_popup (GTK_ENTRY (widget), NULL);
  return GDK_EVENT_STOP;
}
                      ...
                      GtkEntry   *entry)
{
  ...
  if (mode == GTK_TEXT_HANDLE_MODE_CURSOR)
    {
      _ctk_text_handle_set_visible (handle, GTK_TEXT_HANDLE_POSITION_CURSOR, !visible);
    }
  else if (mode == GTK_TEXT_HANDLE_MODE_SELECTION)
    {
      _ctk_text_handle_set_visible (handle, GTK_TEXT_HANDLE_POSITION_SELECTION_START, !visible);
      _ctk_text_handle_set_visible (handle, GTK_TEXT_HANDLE_POSITION_SELECTION_END, !visible);
    }
}
                      ...
                      gboolean      sensitive)
{
  ...
  ctk_container_add (GTK_CONTAINER (toolbar), item);
}
                         ...
                         gpointer          user_data)
{
  ...
  has_selection = ctk_editable_get_selection_bounds (GTK_EDITABLE (entry), &start, &end);
  length = ctk_entry_buffer_get_length (get_buffer (entry));
  ...
  priv->selection_bubble = ctk_popover_new (GTK_WIDGET (entry));
  ctk_style_context_add_class (ctk_widget_get_style_context (priv->selection_bubble),
                               GTK_STYLE_CLASS_TOUCH_SELECTION);
  ctk_popover_set_position (GTK_POPOVER (priv->selection_bubble), GTK_POS_BOTTOM);
  ctk_popover_set_modal (GTK_POPOVER (priv->selection_bubble), FALSE);
  ...
  ctk_widget_show (box);
  toolbar = ctk_box_new (GTK_ORIENTATION_HORIZONTAL, 5);
  ctk_widget_show (toolbar);
  ctk_container_add (GTK_CONTAINER (priv->selection_bubble), box);
  ctk_container_add (GTK_CONTAINER (box), toolbar);
  ...
  ctk_popover_set_pointing_to (GTK_POPOVER (priv->selection_bubble), &rect);
...
static void
ctk_entry_selection_bubble_popup_unset (GtkEntry *entry)
{
  GtkEntryPrivate *priv;
...
static void
ctk_entry_selection_bubble_popup_set (GtkEntry *entry)
{
  GtkEntryPrivate *priv;
...
static void
ctk_entry_drag_begin (GtkWidget      *widget,
                      GdkDragContext *context)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
          ...
          if (icon_info->in_drag) 
            {
              ctk_drag_set_icon_definition (context,
                                            ctk_icon_helper_get_definition (GTK_ICON_HELPER (icon_info->gadget)),
                                            -2, -2);
              return;
            }
        }
...
static void
ctk_entry_drag_end (GtkWidget      *widget,
                    GdkDragContext *context)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_drag_leave (GtkWidget        *widget,
		      GdkDragContext   *context,
		      guint             time)
{
  GtkEntry *entry = GTK_ENTRY (widget);
...
static gboolean
ctk_entry_drag_drop  (GtkWidget        *widget,
		      GdkDragContext   *context,
		      gint              x,
		      gint              y,
		      guint             time)
...
static gboolean
ctk_entry_drag_motion (GtkWidget        *widget,
		       GdkDragContext   *context,
		       gint              x,
		       gint              y,
		       guint             time)
{
  ...
  if (priv->editable &&
      ctk_drag_dest_find_target (widget, context, NULL) != GDK_NONE)
    {
      ...
      if (!ctk_editable_get_selection_bounds (GTK_EDITABLE (entry), &sel1, &sel2) ||
...
static void
ctk_entry_drag_data_received (GtkWidget        *widget,
			      GdkDragContext   *context,
			      gint              x,
			      gint              y,
			      GtkSelectionData *selection_data,
...
static void
ctk_entry_drag_data_get (GtkWidget        *widget,
			 GdkDragContext   *context,
			 GtkSelectionData *selection_data,
			 guint             info,
			 guint             time)
{
  ...
  if (ctk_editable_get_selection_bounds (editable, &sel_start, &sel_end))
    {
      gchar *str = _ctk_entry_get_display_text (GTK_ENTRY (widget), sel_start, sel_end);
...
static void
ctk_entry_drag_data_delete (GtkWidget      *widget,
			    GdkDragContext *context)
{
  GtkEntry *entry = GTK_ENTRY (widget);
  GtkEntryPrivate *priv = entry->priv;
...
{
  GtkSettings *settings = ctk_widget_get_settings (GTK_WIDGET (entry));
  gint time;
...
static void
ctk_entry_check_cursor_blink (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_pend_cursor_blink (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
...
static void
ctk_entry_reset_blink_time (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
 ...
 */
void
ctk_entry_set_completion (GtkEntry           *entry,
                          GtkEntryCompletion *completion)
{
 ...
 */
GtkEntryCompletion *
ctk_entry_get_completion (GtkEntry *entry)
{
  GtkEntryCompletion *completion;
 ...
 */
void
ctk_entry_set_cursor_hadjustment (GtkEntry      *entry,
                                  GtkAdjustment *adjustment)
{
  g_return_if_fail (GTK_IS_ENTRY (entry));
  if (adjustment)
 ...
 */
GtkAdjustment*
ctk_entry_get_cursor_hadjustment (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), NULL);
...
static void
ctk_entry_ensure_progress_gadget (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  ...
  ctk_css_gadget_set_state (priv->progress_gadget,
      ctk_css_node_get_state (ctk_widget_get_css_node (GTK_WIDGET (entry))));

  update_node_ordering (entry);
}

static void
ctk_entry_start_pulse_mode (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  ...
  ctk_css_gadget_add_class (priv->progress_gadget, GTK_STYLE_CLASS_PULSE);
  ...
  priv->tick_id = ctk_widget_add_tick_callback (GTK_WIDGET (entry), tick_cb, NULL, NULL);
...
static void
ctk_entry_stop_pulse_mode (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  ...
  if (priv->progress_pulse_mode)
    {
      ...
      ctk_css_gadget_remove_class (priv->progress_gadget, GTK_STYLE_CLASS_PULSE);
...
static void
ctk_entry_update_pulse (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
 ...
 */
void
ctk_entry_set_progress_fraction (GtkEntry *entry,
                                 gdouble   fraction)
{
  GtkWidget       *widget;
 ...
 */
gdouble
ctk_entry_get_progress_fraction (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), 0.0);
 ...
 */
void
ctk_entry_set_progress_pulse_step (GtkEntry *entry,
                                   gdouble   fraction)
{
  GtkEntryPrivate *private;
 ...
 */
gdouble
ctk_entry_get_progress_pulse_step (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), 0.0);
 ...
 */
void
ctk_entry_progress_pulse (GtkEntry *entry)
{
  g_return_if_fail (GTK_IS_ENTRY (entry));
 ...
 **/
void
ctk_entry_set_placeholder_text (GtkEntry    *entry,
                                const gchar *text)
{
  GtkEntryPrivate *priv;
 ...
 **/
const gchar *
ctk_entry_get_placeholder_text (GtkEntry *entry)
{
  GtkEntryPrivate *priv;
                        ...
                        const gchar *text)
{
  ...
  if (ctk_entry_get_icon_storage_type (entry, GTK_ENTRY_ICON_SECONDARY) == GTK_IMAGE_EMPTY)
    {
      ctk_entry_set_icon_from_icon_name (entry, GTK_ENTRY_ICON_SECONDARY, "caps-lock-symbolic");
      ctk_entry_set_icon_activatable (entry, GTK_ENTRY_ICON_SECONDARY, FALSE);
      priv->caps_lock_warning_shown = TRUE;
    }

  if (priv->caps_lock_warning_shown)
    ctk_entry_set_icon_tooltip_text (entry, GTK_ENTRY_ICON_SECONDARY, text);
  else
 ...
 */
void
ctk_entry_set_input_purpose (GtkEntry        *entry,
 ...
 */
GtkInputPurpose
ctk_entry_get_input_purpose (GtkEntry *entry)
{
  GtkInputPurpose purpose;
 ...
 */
void
ctk_entry_set_input_hints (GtkEntry      *entry,
                           GtkInputHints  hints)
 ...
 */
GtkInputHints
ctk_entry_get_input_hints (GtkEntry *entry)
{
  GtkInputHints hints;
 ...
 */
void
ctk_entry_set_attributes (GtkEntry      *entry,
                          PangoAttrList *attrs)
{
  GtkEntryPrivate *priv = entry->priv;
  g_return_if_fail (GTK_IS_ENTRY (entry));
 ...
 */
PangoAttrList *
ctk_entry_get_attributes (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), NULL);
...
void
ctk_entry_set_tabs (GtkEntry      *entry,
                    PangoTabArray *tabs)
{
  GtkEntryPrivate *priv;
  g_return_if_fail (GTK_IS_ENTRY (entry));
...
PangoTabArray *
ctk_entry_get_tabs (GtkEntry *entry)
{
  g_return_val_if_fail (GTK_IS_ENTRY (entry), NULL);
...
static void
ctk_entry_insert_emoji (GtkEntry *entry)
{
  GtkWidget *chooser;
  GdkRectangle rect;

  if (ctk_entry_get_input_hints (entry) & GTK_INPUT_HINT_NO_EMOJI)
    return;

  if (ctk_widget_get_ancestor (GTK_WIDGET (entry), GTK_TYPE_EMOJI_CHOOSER) != NULL)
    return;
  ...
  if (!chooser)
    {
      ...
      ctk_popover_set_relative_to (GTK_POPOVER (chooser), GTK_WIDGET (entry));
      if (entry->priv->show_emoji_icon)
        {
          ctk_entry_get_icon_area (entry, GTK_ENTRY_ICON_SECONDARY, &rect);
          ctk_popover_set_pointing_to (GTK_POPOVER (chooser), &rect);
        }
      g_signal_connect_swapped (chooser, "emoji-picked", G_CALLBACK (ctk_entry_enter_text), entry);
    }

  ctk_popover_popup (GTK_POPOVER (chooser));
}
...
set_enable_emoji_completion (GtkEntry *entry,
                             gboolean  value)
{
  GtkEntryPrivate *priv = ctk_entry_get_instance_private (entry);
