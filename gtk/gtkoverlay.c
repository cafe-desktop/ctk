...
G_DEFINE_TYPE_WITH_CODE (GtkOverlay, ctk_overlay, GTK_TYPE_BIN,
				      ...
				      GtkAllocation *widget_allocation)
{
  ...
  ctk_widget_get_allocation (GTK_WIDGET (overlay), &overlay_allocation);
                                        ...
                                        GtkAllocation *main_alloc_out)
{
  ...
  main_widget = ctk_bin_get_child (GTK_BIN (overlay));
  ...
  if (GTK_IS_SCROLLED_WINDOW (main_widget))
    {
      ...
      grandchild = ctk_bin_get_child (GTK_BIN (main_widget));
  ...
  else
    {
      main_alloc.x = 0;
      main_alloc.y = 0;
      main_alloc.width = ctk_widget_get_allocated_width (GTK_WIDGET (overlay));
      main_alloc.height = ctk_widget_get_allocated_height (GTK_WIDGET (overlay));
    }
                                        ...
                                        GtkAllocation *child_allocation)
{
  ...
  has_left = ctk_style_context_has_class (context, GTK_STYLE_CLASS_LEFT);
  has_right = ctk_style_context_has_class (context, GTK_STYLE_CLASS_RIGHT);
  has_top = ctk_style_context_has_class (context, GTK_STYLE_CLASS_TOP);
  has_bottom = ctk_style_context_has_class (context, GTK_STYLE_CLASS_BOTTOM);
                            ...
                            GtkOverlayChild *child)
{
  ...
  if (ctk_widget_get_mapped (GTK_WIDGET (overlay)))
    {
                           ...
                           GtkAllocation *allocation)
{
  ...
  GTK_WIDGET_CLASS (ctk_overlay_parent_class)->size_allocate (widget, allocation);
...
ctk_overlay_realize (GtkWidget *widget)
{
  ...
  GTK_WIDGET_CLASS (ctk_overlay_parent_class)->realize (widget);
...
ctk_overlay_unrealize (GtkWidget *widget)
{
  ...
  GTK_WIDGET_CLASS (ctk_overlay_parent_class)->unrealize (widget);
}
...
ctk_overlay_map (GtkWidget *widget)
{
  ...
  GTK_WIDGET_CLASS (ctk_overlay_parent_class)->map (widget);
...
ctk_overlay_unmap (GtkWidget *widget)
{
  ...
  GTK_WIDGET_CLASS (ctk_overlay_parent_class)->unmap (widget);
}
                    ...
                    GtkWidget    *widget)
{
      ...
      if (child->widget == widget)
        {
          if (child->window != NULL)
            {
              ctk_widget_unregister_window (GTK_WIDGET (container), child->window);
              gdk_window_destroy (child->window);
            }
  ...
  if (!removed)
    GTK_CONTAINER_CLASS (ctk_overlay_parent_class)->remove (container, widget);
}
                             ...
                             int         index_)
{
  ...
  if (ctk_widget_get_visible (child) &&
      ctk_widget_get_visible (GTK_WIDGET (overlay)))
    ctk_widget_queue_resize (GTK_WIDGET (overlay));
}
				...
				GParamSpec   *pspec)
{
  ...
  switch (property_id)
    ...
    case CHILD_PROP_INDEX:
      if (child_info != NULL)
	ctk_overlay_reorder_overlay (GTK_OVERLAY (container),
				     child,
...
ctk_overlay_init (GtkOverlay *overlay)
{
  ...
  ctk_widget_set_has_window (GTK_WIDGET (overlay), FALSE);
}
                                 ...
                                 const gchar  *type)
{
  if (type && strcmp (type, "overlay") == 0)
    ctk_overlay_add_overlay (GTK_OVERLAY (buildable), GTK_WIDGET (child));
  else if (!type)
                         ...
                         GtkWidget  *widget)
{
  ...
  if (ctk_widget_get_realized (GTK_WIDGET (overlay)))
    {
      child->window = ctk_overlay_create_child_window (overlay, child);
      ctk_widget_set_parent (widget, GTK_WIDGET (overlay));
    }
  else
    ctk_widget_set_parent (widget, GTK_WIDGET (overlay));
				      ...
				      gboolean    pass_through)
{
  ...
  ctk_container_child_set (GTK_CONTAINER (overlay), widget,
				      ...
				      GtkWidget  *widget)
{
  ...
  ctk_container_child_get (GTK_CONTAINER (overlay), widget,
			   "pass-through", &pass_through,
			   NULL);
