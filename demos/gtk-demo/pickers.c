...
do_pickers (GtkWidget *do_widget)
{
  ...
  if (!window)
  {
    ...
    ctk_window_set_title (GTK_WINDOW (window), "Pickers");
    ...
    table = ctk_grid_new ();
    ctk_grid_set_row_spacing (GTK_GRID (table), 3);
    ctk_grid_set_column_spacing (GTK_GRID (table), 10);
    ctk_container_add (GTK_CONTAINER (window), table);
    ...
    ctk_grid_attach (GTK_GRID (table), picker, 1, 0, 1, 1);
    ...
    ctk_grid_attach (GTK_GRID (table), picker, 1, 1, 1, 1);
    ...
    ctk_file_chooser_set_local_only (GTK_FILE_CHOOSER (picker), FALSE);
    ctk_grid_attach (GTK_GRID (table), label, 0, 2, 1, 1);
    ctk_grid_attach (GTK_GRID (table), picker, 1, 2, 1, 1);
    ...
    ctk_grid_attach (GTK_GRID (table), label, 0, 3, 1, 1);
    ctk_grid_attach (GTK_GRID (table), picker, 1, 3, 1, 1);
    ...
    ctk_app_chooser_button_set_show_dialog_item (GTK_APP_CHOOSER_BUTTON (picker), TRUE);
    ctk_grid_attach (GTK_GRID (table), label, 0, 4, 1, 1);
    ctk_grid_attach (GTK_GRID (table), picker, 1, 4, 1, 1);
  }
